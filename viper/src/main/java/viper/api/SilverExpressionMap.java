...
public class SilverExpressionMap<T, E> implements ASTMapping<E> {
    ...
    public E map(OperatorExpression e) {
        ...
        switch (e.operator()) {
            ...
            case SubSet:
                create.and(o, create.any_set_subset(o, e1, e2), create.lt(o, create.size(o, e1), create.size(o, e2)));
            case SubSetEq:
        ...
        switch (e.getKind()) {
            ...
            case Reserved:
                switch (e.reserved()) {
                    ...
                    case FullPerm:
                        return create.write_perm(o);
                    case ReadPerm:
                        return create.read_perm(o);
                    case NoPerm:
                        return create.no_perm(o);
                    case Result:
                        return create.result(o, e.getType().apply(type));
                    default:
                }
