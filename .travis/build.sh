...
./.travis/travis_fold.sh test "Executing tests" "vercors --progress --test=$TRAVIS_BUILD_DIR/examples --travis-test-output --test-workers=1 --tool=silicon --exclude-suite=slow,medium,problem-fail,skip-travis --enable-test-coverage --coverage-output-file=sync/${TRAVIS_BUILD_NUMBER}/${TRAVIS_JOB_NUMBER}.xml"
