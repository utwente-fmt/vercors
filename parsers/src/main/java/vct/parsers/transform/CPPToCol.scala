...
import vct.col.origin.{DiagnosticOrigin, SourceNameOrigin}
import vct.col.ref.{Ref, UnresolvedRef}
import vct.col.util.AstBuildHelpers.{const, ff, tt}
...
case class CPPToCol[G](override val originProvider: OriginProvider, override val blameProvider: BlameProvider, override val errors: Seq[(Token, Token, ExpectedError)])
  ...
  def convert(unit: TranslationUnitContext): Seq[GlobalDeclaration[G]] = unit match {
    case TranslationUnit0(declSeq, _) =>
      declSeq.map(convert(_)).getOrElse(Nil)
  }
  ...
  def convert(implicit expr: PrimaryExpressionContext): Expr[G] = expr match {
    ...
    case PrimaryExpression2(_) => AmbiguousThis() // TODO OS: Should this be a CPPThis or an AmbiguousThis?
    case PrimaryExpression3(_, inner, _) => convert(inner)
    case PrimaryExpression4(id) => ??(expr) // TODO OS: It is a bit of a question what this can/should be. Is it only identifiers or more?
    case PrimaryExpression5(lambda) => ??(expr)
  }

  def convert(implicit expr: LiteralContext): Expr[G] = expr match {
    case Literal0(integer) => IntegerValue(Integer.parseInt(integer))
    case Literal1(character) => ??(expr)
    case Literal2(floating) => ??(expr)
    case Literal3(string) => ??(expr)
    case Literal4(boolean) => BooleanValue(boolean.toBoolean)
    case Literal5(pointer) => ??(expr)
    case Literal6(userDefined) => ??(expr)
  }

  def convert(implicit expr: AnnotatedPrimaryExpressionContext): Expr[G] = expr match {
    case AnnotatedPrimaryExpression0(pre, pexpr, post) =>
      convertEmbedWith(pre, convertEmbedThen(post, convert(pexpr)))
  }

  def convert(implicit expr: IdExpressionContext): Expr[G] = expr match {
    case _ => ??(expr)
  }

  def convert(implicit id: UnqualifiedIdContext): String = id match {
    case UnqualifiedId0(cppid) => convert(cppid)
    case UnqualifiedId1(_) => ??(id)
    case UnqualifiedId2(_) => ??(id)
    case UnqualifiedId3(_) => ??(id)
    case UnqualifiedId4(_) => ??(id)
    case UnqualifiedId5(_) => ??(id)
    case UnqualifiedId6(_) => ??(id)
  }



  def convert(implicit id: QualifiedIdContext): Expr[G] = id match {
    case QualifiedId0(namespaces, _, identifier) => local(id, convert(identifier), convert(namespaces))
  }


  def convert(implicit spec: NestedNameSpecifierContext): Seq[String] = spec match {
    case NestedNameSpecifier0(Some(typeName), _) => ??(spec)
    case NestedNameSpecifier1(Some(namespaceName), _) => Seq(convert(namespaceName))
    case NestedNameSpecifier3(namespaces, cppId, _) => convert(namespaces) ++ Seq(convert(cppId))
    case _ => ??(spec)
  }

  def convert(implicit name: NamespaceNameContext): String = name match {
    case NamespaceName0(OriginalNamespaceName0(cppid)) => convert(cppid)
    case NamespaceName1(NamespaceAlias0(cppid)) => convert(cppid)
  }

  def convert(implicit expr: LambdaExpressionContext): Expr[G] = expr match {
    case _ => ??(expr)
  }

  def convert(implicit expr: LambdaIntroducerContext): Expr[G] = expr match {
    case _ => ??(expr)
  }

  def convert(implicit cap: LambdaCaptureContext): Expr[G] = cap match {
    case _ => ??(cap)
  }
  ...
  def convert(implicit expr: PostfixExpressionContext): Expr[G] = expr match {
    ...
    case PostfixExpression2(arr, _, idx, _) => AmbiguousSubscript(convert(arr), convert(idx))(blame(expr))
    case PostfixExpression3(f, _, args, _, given, yields) =>
      CPPInvocation(convert(f), args.map(convert(_)) getOrElse Nil,
        convertEmbedGiven(given), convertEmbedYields(yields))(blame(expr))
    case PostfixExpression4(typeSpec, _, args, _, given, yields) => ??(expr)
    case PostfixExpression5(typeSpec, bracedInitList, given, yields) => ??(expr)
    case PostfixExpression6(typeSpec, _, args, _, given, yields) => ??(expr)
    case PostfixExpression7(typeSpec, bracedInitList, given, yields) => ??(expr)
    case PostfixExpression8(postexpr, _, _, field) => ??(expr)
    case PostfixExpression9(postexpr, _, destructor) => ??(expr)
    case PostfixExpression10(postexpr, _, _, field) => ??(expr)
    case PostfixExpression11(postexpr, _, destructor) => ??(expr)
    case PostfixExpression12(targetNode, _) =>
      val target = convert(targetNode)
      PostAssignExpression(target, col.Plus(target, const(1)))(blame(expr))
    case PostfixExpression13(targetNode, _) =>
      val target = convert(targetNode)
      PostAssignExpression(target, col.Minus(target, const(1)))(blame(expr))
    case PostfixExpression14(e, SpecPostfix0(postfix)) => convert(postfix, convert(e))



  }

  def convert(implicit list: ExpressionListContext): Seq[Expr[G]] = list match {
    case ExpressionList0(inner) => convert(inner)
  }

  def convert(implicit unary: UnaryExpressionContext): Expr[G] = unary match {
    case UnaryExpression0(inner) => convert(inner)
    case UnaryExpression1(_, arg) =>
      val target = convert(arg)
      PreAssignExpression(target, col.Plus(target, const(1)))(blame(unary))
    case UnaryExpression2(_, arg) =>
      val target = convert(arg)
      PreAssignExpression(target, col.Minus(target, const(1)))(blame(unary))
    case UnaryExpression3(op, arg) => op match {
      case UnaryOperator0(_) => ??(unary)
      case UnaryOperator1(_) => DerefPointer(convert(arg))(blame(unary))
      case UnaryOperator2(_) => AddrOf(convert(arg))
      case UnaryOperator3(_) => convert(arg)
      case UnaryOperator4(_) => BitNot(convert(arg))
      case UnaryOperator5(_) => UMinus(convert(arg))
      case UnaryOperator6(_) => col.Not(convert(arg))
    }
    case UnaryExpression4(_, _) => ??(unary)
    case UnaryExpression5(_, _, _, _) => ??(unary)
    case UnaryExpression6(_, _, _, _, _) => ??(unary)
    case UnaryExpression7(_, _, _, _) => ??(unary)
    case UnaryExpression8(_) => ??(unary)
    case UnaryExpression8(_) => ??(unary)
    case UnaryExpression8(_) => ??(unary)

  }

  def convert(implicit expr: NewExpressionContext): Expr[G] = expr match {
    case _ => ??(expr)
  }
...
//    case PointerMemberExpression1(pointMemExpr, _, prependExpr) => ???
//    case PointerMemberExpression2(pointMemExpr, _, prependExpr) => ???
  }


  def convert(implicit expr: MultiplicativeExpressionContext): Expr[G] = expr match {
    case MultiplicativeExpression0(inner, _) => convert(inner)
  ...
  def convert(implicit expr: ShiftExpressionContext): Expr[G] = expr match {
    ...
    case ShiftExpression1(left, _, right) => BitShr(convert(left), convert(right))
    case ShiftExpression2(left, _, right) => BitShl(convert(left), convert(right))
  }
  ...
  def convert(implicit expr: RelationalExpressionContext): Expr[G] = expr match {
    ...
    case RelationalExpression1(left, RelationalOp1(specOp), right) => convert(specOp, convert(left), convert(right))
  }
  ...
  def convert(implicit expr: AssignmentExpressionContext): Expr[G] = expr match {
    ...
    case AssignmentExpression1(pre, targetNode, op, valueNode, post) =>
      ...
      val res = PreAssignExpression(target, op match {
        case AssignmentOperator0(_) => value
        case AssignmentOperator1(_) => Mult(target, value)
        case AssignmentOperator2(_) => FloorDiv(target, value)(blame(expr))
        case AssignmentOperator3(_) => col.Mod(target, value)(blame(expr))
        case AssignmentOperator4(_) => col.Plus(target, value)
        case AssignmentOperator5(_) => col.Minus(target, value)
        case AssignmentOperator6(_) => BitShr(target, value)
        case AssignmentOperator7(_) => BitShl(target, value)
        case AssignmentOperator8(_) => BitAnd(target, value)
        case AssignmentOperator9(_) => BitXor(target, value)
        case AssignmentOperator10(_) => BitOr(target, value)
      })(blame(expr))


      convertEmbedWith(pre, convertEmbedThen(post, res))
  }
  ...
  def convert(implicit expr: ConstantExpressionContext): Expr[G] = expr match {
    case ConstantExpression0(inner) => convert(inner)
  }

  def convert(implicit expr: StatementContext): Statement[G] = expr match {
    case Statement0(inner) => convert(inner)
    case Statement1(maybeAttrSepcSeq, inner) => convert(inner)
    case Statement2(maybeAttrSepcSeq, inner) => convert(inner)
    case Statement3(maybeAttrSepcSeq, inner) => convert(inner)
    case Statement4(maybeAttrSepcSeq, inner) => convert(inner)
    case Statement5(maybeAttrSepcSeq, inner) => convert(inner)
    case Statement6(maybeAttrSepcSeq, inner) => convert(inner)
    case Statement7(declStmt) => convert(declStmt)
    case Statement8(inner) => convert(inner)
    case Statement8(inner) => convert(inner)
  }

  def convert(implicit block: TryBlockContext): Statement[G] = ??(block)


  def convert(implicit expr: CppIdentifierContext): String = expr match {
    case CppIdentifier0(id) => id
    case CppIdentifier1(valid) => convert(valid)
  }


  def convert(implicit lbldStmt: LabeledStatementContext): Statement[G] = lbldStmt match {
    case _ => ??(lbldStmt)
  }

  def convert(implicit stmt: ExpressionStatementContext): Statement[G] = stmt match {
    case ExpressionStatement0(None, _) => Block(Nil)
    case ExpressionStatement0(Some(expr), _) => Eval(convert(expr))
  }

  def convert(implicit stmt: CompoundStatementContext): Statement[G] = stmt match {
    case CompoundStatement0(_, stats, _) =>
      Scope(Nil, Block(stats.map(convert(_)) getOrElse Nil))
  }

  def convert(implicit stmt: StatementSeqContext): Seq[Statement[G]] = stmt match {
    case StatementSeq0(stats) => stats.map(convert(_))
  }

  def convert(implicit stmt: SelectionStatementContext): Statement[G] = stmt match {
    case SelectionStatement0(_, _, cond, _, whenTrue, _, whenFalse) =>
      val colCond = cond match {
        case Condition0(expr) => convert(expr)
        case _ => ??(cond)
      }
      Branch(Seq((colCond, convert(whenTrue)), (tt, convert(whenFalse))))
    case SelectionStatement1(_, _, Condition0(expr), _, whenTrue) =>
      Branch(Seq((convert(expr), convert(whenTrue))))
    case SelectionStatement2(_, _, cond, _, whenTrue) => ??(stmt)
  }

  def convert(implicit stmt: IterationStatementContext): Statement[G] = stmt match {
    case IterationStatement0(contract1, _, _, cond, _, contract2, body) => withContract(contract1, contract2, c => {
      Scope(Nil, Loop[G](Block(Nil), convert(cond), Block(Nil), c.consumeLoopContract(stmt), convert(body)))
    })
    case IterationStatement2(contract1, _, _, init, maybeCond, _, update, _, contract2, body) =>
      withContract(contract1, contract2, c => {
        Scope(Nil, Loop[G](convert(init), maybeCond.map(convert(_)) getOrElse tt, evalOrNop(update), c.consumeLoopContract(stmt), convert(body)))
      })
    case _ => ??(stmt)
  }

  def convert(implicit expr: ConditionContext): Expr[G] = expr match {
    case Condition0(expr) => convert(expr)
    case Condition1(_, declSpecSeq, decl, _, init) => ???
  ...
  def convert(implicit stmt: ForInitStatementContext): Statement[G] = stmt match {
    case ForInitStatement0(expr) => convert(expr)
    case ForInitStatement1(_) => ??(stmt)
  }


  def convert(implicit stmt: DeclarationStatementContext): Statement[G] = stmt match {
    case DeclarationStatement0(inner) => CPPDeclarationStatement(convert(inner))
  }

  def convert(implicit declSeq: DeclarationseqContext): Seq[GlobalDeclaration[G]] = declSeq match {
    case Declarationseq0(declSeqs) => declSeqs.flatMap(convert(_)).map(new CPPGlobalDeclaration[G](_))
  }

  def convert(implicit decl: DeclarationContext): Seq[CPPDeclaration[G]] = decl match {
    case Declaration0(block) => Seq(convert(block))
    case Declaration1(func) => Seq(convert(func))
    case Declaration2(template) => ??(decl)
    case Declaration3(explInstantiation) => ??(decl)
    case Declaration4(explSpecialization) => ??(decl)
    case Declaration5(linkSpec) => ??(decl)
    case Declaration6(namespace) => ??(decl)
    case Declaration7(emptyDecl) => Nil
    case Declaration8(attr) => ??(decl)
  }

  def convert(implicit block: BlockDeclarationContext): CPPDeclaration[G] = block match {
    case BlockDeclaration0(simplDecl) => ??(block)
    case BlockDeclaration1(asmDef) => ??(block)
    case BlockDeclaration2(nsalias) => ??(block)
    case BlockDeclaration3(usingDecl) => ??(block)
    case BlockDeclaration4(usingDirective) => ??(block)
    case BlockDeclaration5(staticAssert) => ??(block)
    case BlockDeclaration6(aliasDecl) => ??(block)
    case BlockDeclaration7(enumDecl) => ??(block)
  }

  def convert(implicit func: FunctionDefinitionContext): CPPFunctionDefinition[G] = func match {
    case FunctionDefinition0(maybeContract, maybeAttrSpecSeq, maybeDeclSpecSeq, decl, virtualSpecSeq, body) => ??(func)
  }

  def convert(implicit init: InitializerClauseContext): Expr[G] = init match {
    case InitializerClause0(inner) => convert(inner)
    case InitializerClause1(inner) => convert(inner)
  }

  def convert(implicit list: BracedInitListContext): Expr[G] = list match {
    case BracedInitList0(_, initlist, _, _) => CPPBracedInitList(convert(initlist))
    case BracedInitList1(_, _) => CPPBracedInitList(Nil)
  }
  ...
  def convert(implicit list: InitializerListContext): Seq[Expr[G]] = list match {
    ...
    case InitializerList1(initClause, None, _, rest) => Seq(convert(initClause)) ++ convert(rest)
    case _ => ??(list)
  }

  def convert(implicit jump: JumpStatementContext): Statement[G] = jump match {
    case JumpStatement0(_, _) => col.Break(None)
    case JumpStatement1(_, _) => col.Continue(None)
    case JumpStatement2(_, value, _) => col.Return(convert(value))
    case JumpStatement3(_, list, _) => col.Return(convert(list))
    case JumpStatement4(_, _) => col.Return(col.Void())
    case JumpStatement5(_, label, _) => CPPGoto(convert(label))
  }
  ...
  /****************
    *** CPPParser **
    *****************/
  def convert(expr: LangExprContext): Expr[G] = ??(expr)

  def convert(stat: LangStatementContext): Statement[G] = ??(stat)

  def convert(implicit t: LangTypeContext): Type[G] = ??(t)

  def convert(id: LangIdContext): String = ??(id)
  ...
  def convert(decl: LangGlobalDeclContext): Seq[GlobalDeclaration[G]] = decl match {
    case LangGlobalDecl0(decl) => convert(decl).map(x => new CPPGlobalDeclaration[G](x)(x.o))
  }


  def local(ctx: ParserRuleContext, name: String, namespace: Seq[String] = Nil): Expr[G] =
    CPPLocal(name, namespace)(blame(ctx))(origin(ctx))
