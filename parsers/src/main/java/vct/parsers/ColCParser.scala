...
case class ColCParser() extends Parser {
  ...
  override def parse(stream: InputStream, originProvider: OriginProvider, blameProvider: BlameProvider): Seq[GlobalDeclaration] = {
  ...
  override def parse(f: File)(originProvider: OriginProvider = null,
                              blameProvider: BlameProvider = null): Seq[GlobalDeclaration] = {
