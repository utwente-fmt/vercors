...
case class PVLtoCOL(fileName: String, tokens: CommonTokenStream, parser: PVLParser)
  ...
  def expr(tree: ParserRuleContext): ASTNode = origin(tree, tree match {
    ...
    case SeqAddExpr0(unaryExp, None) => expr(unaryExp)
    case SeqAddExpr0(x, Some(SeqPrependExpr0("::", xs))) => create expression(PrependSingle, expr(x), expr(xs))
    case SeqAddExpr1(xs, "++", SeqAppendExpr0(ys)) => create expression(AppendSingle, expr(xs), expr(ys))
    case SeqAddExpr1(xs, "++", SeqAppendExpr1("(", a, ",", b, ")")) => create expression(MapBuild, expr(xs), expr(a), expr(b))
  ...
  def convertParUnitList(tree: ParUnitListContext): Seq[ParallelBlock] = tree match {
    case ParUnitList0(x, None) => Seq(convertParUnit(x))
    case ParUnitList0(x, Some(ParUnitListTail0("and", xs))) => convertParUnit(x) +: convertParUnitList(xs)
  }
