...
import vct.col.ast.{Assign, Declaration, Deref, DerefVeyMontThread, Expr, Node, ProcedureInvocation, RunMethod, Statement, VeyMontAssignExpression, VeyMontCommExpression, VeyMontSeqProg}
          ...
          target match {
            case Deref(obj, _) => obj match {
              case DerefVeyMontThread(ref) => { //target is correct ref to thread
                ...
                else throw AddVeyMontNodesError(value, "The value of this assignment is not allowed to refer to multiple threads!")
              }
              case _ => throw AddVeyMontNodesError(target, "The target of this assignment must refer to a thread")
            }
            case _ => throw AddVeyMontNodesError(st, "The target of this assignment must be a dereference to a thread")
          }
