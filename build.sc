...
object external extends Module {
  object z3 extends Module {
    def url =
      T {
        "https://www.sosy-lab.org/ivy/org.sosy_lab/javasmt-solver-z3/com.microsoft.z3-4.8.7.jar"
      }
...
object viper extends ScalaModule {
  ...
  object silver extends ScalaModule {
    ...
    override def sources =
      T.sources { repo.filteredRepo() / "src" / "main" / "scala" }
    override def resources =
      T.sources { repo.filteredRepo() / "src" / "main" / "resources" }
    override def ivyDeps =
      settings.deps.log ++ Agg(
  ...
  object silicon extends ScalaModule {
    ...
    object common extends ScalaModule {
      ...
      override def sources =
        T.sources {
          silicon.repo.filteredRepo() / "common" / "src" / "main" / "scala"
        }
...
object vercors extends Module {
  ...
  object col extends VercorsModule {
    object helpers extends Module {
      ...
      def structureClasspathDir: Path =
        settings.meta / "structureClassPath.dest"
      ...
      def allDefinitions =
        T { read[Seq[NodeDefinition]](allDefinitionsSource()) }

      // Step 1: compile
      object generators extends VercorsModule {
        def key = "helpers"
        def deps: T[Agg[Dep]] = T { Agg(ivy"org.scalameta::scalameta:4.8.15") }
        def structureClasspath =
          T.source(structureClasspathDir / "classpath.json")
        def unmanagedClasspath =
          T { read[Seq[Path]](structureClasspath()).map(PathRef(_)) }
      }

      def instantiate[T](name: String): Task[T] =
        T.task {
          mill.api.ClassLoader.create(
            ...
            .asInstanceOf[Class[T]].getDeclaredConstructor().newInstance()
        }
      ...
      object implTraits extends Module {
        ...
        def inputs =
          T.sources(
            os.walk(root).filter(_.last.endsWith("Impl.scala")).map(PathRef(_))
          )
        ...
        def generator =
          T.worker(
            instantiate[structure.api.ImplTraitGeneratorApi]("ImplTrait")
          )

        def allDefinitionsSet = T { allDefinitions().map(_.name.base).toSet }

        def allFamiliesSet =
          T {
            val allFamiliesVal = allFamilies()
            (allFamiliesVal.declaredFamilies ++
              allFamiliesVal.structuralFamilies).map(_.base).toSet
          }
        ...
        def fix =
          T {
            ...
            inputs().foreach { input =>
              ...
              val opsNames =
                (if (defnSet.contains(name))
                   Seq(name + "Ops")
                 else
                   Nil) ++
                  (if (familySet.contains(name))
                     Seq(name + "FamilyOps")
                   else
                     Nil)

              try { gen.fix(input.path.toNIO, write(opsNames)) }
              catch {
        ...
        def generate: T[Unit] =
          T {
            ...
            if (ungenerated.nonEmpty)
              T.log.error({
                val items = ungenerated.map(name => s" - ${name}Impl")
                  .mkString("\n")
                s"""
                 ...
                 |If not, compilation will fail after this message."""
                  .stripMargin
              })
      ...
      def allFamiliesGenerators =
        T.worker {
          Seq(
            instantiate[structure.api.AllFamiliesGeneratorApi](
              "AbstractRewriter"
            )(),
            ...
            instantiate[structure.api.AllFamiliesGeneratorApi](
              "BaseCoercingRewriter"
            )(),
            instantiate[structure.api.AllFamiliesGeneratorApi](
              "BaseNonLatchingRewriter"
            )(),
            instantiate[structure.api.AllFamiliesGeneratorApi](
              "SuccessorsProvider"
            )(),
      ...
      def familyGenerators =
        T.worker {
          Seq(
            instantiate[structure.api.FamilyGeneratorApi](
              "DeserializeFamily"
            )(),
        ...
        def generate =
          T {
            ...
            allFamiliesGenerators().foreach { gen =>
              ...
              gen.generate(
                T.dest.toNIO,
                write(declarationFamilies),
                write(structuralFamilies),
              )
            }
            protoAuxTypes()
              .foreach(_.generate(T.dest.toNIO, write(definitions)))
            PathRef(T.dest, quick = true)
          }
      }

      implicit object NameSegments
          extends Cross.ToSegments[structure.Name](v => v.parts.toList)
      implicit object FamilySegments
          extends Cross.ToSegments[
            (structure.Name, structure.NodeKind, Seq[structure.Name])
          ](v => implicitly[Cross.ToSegments[structure.Name]].convert(v._1))

      object family extends Cross[FamilyCross](familyCross)
      trait FamilyCross extends Cross.Module[Name] {
        def source =
          T.source(PathRef(
            analysis / "cross-family" / crossValue.path / "family.json",
            quick = true,
          ))
        ...
        def generate =
          T {
            ...
            familyGenerators().foreach { gen =>
              T.log.debug(
                s"Generating ${gen.getClass.getSimpleName} for ${defn.name.base}"
              )
              gen.generate(
                T.dest.toNIO,
                write(defn.name),
      ...
      trait NodeCross extends Cross.Module[Name] {
        def source =
          T.source(PathRef(
            analysis / "cross-node" / crossValue.path / "node.json",
            quick = true,
          ))
      ...
      def generatedSources: T[Seq[PathRef]] =
        T {
          T.traverse(nodeCross)(node(_).generate)() ++
            T.traverse(familyCross)(family(_).generate)() :+ global.generate()
        }

      def megacol: T[PathRef] =
        T.source(global.generate().path / "vct" / "col" / "ast" / "col.proto")

      def sources: T[PathRef] =
        T.persistent {
  ...
  object serialize extends VercorsModule with ScalaPBModule {
    override def scalaPBSources = T { col.helpers.sources() +: this.sources() }
  ...
  object parsers extends VercorsModule {
    ...
    val includeVcllvmCross = interp.watchValue {
      if (os.exists(settings.root / ".include-vcllvm")) { Seq("vcllvm") }
      else { Seq.empty[String] }
    }

    object vcllvmDep extends Cross[VcllvmDep](includeVcllvmCross)
    trait VcllvmDep extends Cross.Module[String] {
      def path = T { vcllvm.compile().path / os.up }
    }

    override def bareResourcePaths =
      T { T.traverse(includeVcllvmCross.map(vcllvmDep(_)))(_.path)() }
    ...
    trait GenModule extends Module {
      ...
      def deps: Seq[String]
      final def depsRef =
        T.sources { deps.map(dep => base() / dep).map(PathRef(_)) }
      ...
      def generate =
        T {
          ...
          runAntlr(
            base() / parser,
            args = Seq("-listener", "-visitor", "-scala-extractor-objects"),
          )
          PathRef(T.dest)
        }
    }
    ...
    object llvm extends GenModule {
      ...
      def deps =
        Seq(
          "SpecParser.g4",
          "SpecLexer.g4",
          "LangLLVMSpecParser.g4",
          "LangLLVMSpecLexer.g4",
        )
    }
  }
  ...
  object viperApi extends VercorsModule {
    def key = "viper"
    def deps = Agg(ivy"org.scalatest::scalatest:3.2.7")
    override def moduleDeps = Seq(hre, col, parsers, viper)

    object test extends Tests
  }

  object tools extends VercorsModule {
    def key = "tools"
    def deps = Agg[Dep]()
    def moduleDeps = Seq(hre, col, rewrite, parsers, viperApi)
  }

  object lsp extends VercorsModule {
    def key = "lsp"
    def deps = Agg(ivy"com.lihaoyi::upickle:4.0.0")
  }

  object cli extends VercorsModule {
    def key = "cli"
    def deps = Agg[Dep]()
  }
  ...
  object main extends VercorsModule {
    ...
    def githubReleaseOutputs() =
      T.command {
        System.out.println(s"TAG_NAME=${buildInfo.gitVersionTag().get}")
        val releaseName =
          if (buildInfo.gitIsPrerelease())
            "VerCors Nightly"
          else
            s"${name()} ${version()}"
        System.out.println(s"RELEASE_NAME=$releaseName")
        System.out.println(s"BODY=${if (buildInfo.gitIsPrerelease())
            "Nightly Build"
          else
            "..."}")
        System.out.println(s"PRERELEASE=${if (buildInfo.gitIsPrerelease())
            "true"
          else
            "false"}")
      }

    def deps = Agg(ivy"com.github.scopt::scopt:4.0.1")
    override def moduleDeps =
      Seq(hre, buildInfo, tools)
    override def mainClass = Some("vct.main.Main")
    override def runScriptClasses =
      T {
        Map(
    ...
    object buildInfo extends BuildInfo with ScalaModule {
      def callOrElse(command: Shellable*)(alt: => String): String =
        try { os.proc(command: _*).call().out.text().trim }
        catch { case _: SubprocessException => alt }

      def gitBranch =
        T.input {
          callOrElse("git", "rev-parse", "--abbrev-ref=strict", "HEAD")(
            "unknown"
          )
        }
      def gitCommit =
        T.input { callOrElse("git", "rev-parse", "HEAD")("unknown") }
      def gitShortCommit =
        T.input {
          callOrElse("git", "rev-parse", "--short=8", "HEAD")("unknown")
        }
      def gitHasChanges =
        T.input {
          callOrElse("git", "diff-index", "--name-only", "HEAD")("dummyChanges")
            .nonEmpty
        }

      def gitVersionTag =
        T.input {
          val tags = callOrElse("git", "tag", "--points-at", "HEAD")("")
            .split("\n")
          tags.collectFirst {
      ...
      def gitIsPrerelease =
        T.input { gitVersionTag().contains("dev-prerelease") }
  ...
  object vcllvm extends CppExecutableModule {
    ...
    def moduleDeps =
      Seq(origin, passes, transform, util, llvm, proto, protobuf.libprotobuf)
    ...
    object protobuf extends CMakeModule {
      ...
      override def cMakeSetupBuild: T[os.Path] =
        T {
    ...
    object proto extends CppModule {
      ...
      def generate =
        T {
          os.proc(
            protobuf.protoc.executable().path,
            protoPath().map(p => "-I=" + p.path.toString),
            "--cpp_out=" + T.dest.toString,
            (Seq(vercors.col.helpers.megacol()) ++
              os.walk(serialize.scalaPBUnpackProto().path).filter(path =>
                !path
                  .startsWith(serialize.scalaPBUnpackProto().path / "google") &&
                  path.ext == "proto"
              ).map(PathRef(_)) ++ os.walk(settings.src / "serialize")
                .filter(_.ext == "proto").map(PathRef(_))).map(_.path),
          ).call()
          T.dest
        }
      ...
      def precompileHeaders: T[PathRef] =
        T {
          def isHiddenFile(path: os.Path): Boolean = path.last.startsWith(".")

          val headers =
            for {
              root <- allSources() if os.exists(root.path)
              path <-
                if (os.isDir(root.path))
                  os.walk(root.path)
                else
                  Seq(root.path) if os.isFile(path)
      ...
      override def exportIncludePaths: T[Seq[PathRef]] =
        T { Seq(precompileHeaders()) }
  ...
  object allTests extends ScalaModule with ReleaseModule {
    ...
    override def moduleDeps: Seq[JavaModule] =
      Seq(col.test, viperApi.test, main.test)
