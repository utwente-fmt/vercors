...
class TestCompare extends AnyFlatSpec {
  ...
  it should "judge equal declarations without references as equal with mappings" in {
    val left = new Procedure[G](TVoid(), Nil, Nil, Nil, None, ApplicableContract(UnitAccountedPredicate(tt), UnitAccountedPredicate(tt), tt, Nil, Nil, Nil)(null))(null)
    val right = new Procedure[G](TVoid(), Nil, Nil, Nil, None, ApplicableContract(UnitAccountedPredicate(tt), UnitAccountedPredicate(tt), tt, Nil, Nil, Nil)(null))(null)
