...
case class This(c: Ref[Class])(implicit val o: Origin) extends Expr {
...
case class ActionApply(action: Ref[ModelAction], args: Seq[Expr])(implicit val o: Origin) extends ProcessExpr with Apply {
  override def ref: Ref[Applicable] = action.decl.ref
}
...
case class ModelThis(ref: Ref[Model])(implicit val o: Origin) extends Expr {
  override def t: Type = TModel(ref)
}
