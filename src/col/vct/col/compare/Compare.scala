...
case object Compare {
  ...
  ): LazyList[CompareResult[L, R]] =
    underEquivalence(left.compare(right))(toNormalForm)

  def underEquivalence[L, R](diffs: LazyList[CompareResult[L, R]])(
      toNormalForm: PartialFunction[Node[L], Node[L]]
  ): LazyList[CompareResult[L, R]] =
    diffs.flatMap {
      case diff @ StructuralDifference(left, right) =>
        (
          toNormalForm.lift(left),
          toNormalForm.lift(right.asInstanceOf[Node[L]])
            .asInstanceOf[Option[Node[R]]],
        ) match {
          case (None, None) => Seq(diff)
          case (maybeLeft, maybeRight) =>
            compare(maybeLeft.getOrElse(left), maybeRight.getOrElse(right))(
              toNormalForm
            )
        }
  ...
  def getIsomorphism[L, R](
      left: Node[L],
      right: Node[R],
      matchFreeVariables: Boolean = false,
  ): Either[Seq[(Node[L], Node[R])], Map[Declaration[L], Declaration[R]]] = {
    ...
    left.compare(right).foreach {
      case MatchingReference(left, right) =>
        define(
          left,
          right,
          mustMatch = matchFreeVariables && !boundVariables.contains(left),
        )
    ...
    if (irreconcilableDiffs.isEmpty)
      Right(isomorphism.toMap)
    else
      Left(irreconcilableDiffs.toSeq)
  }

  def isIsomorphic[L, R](
      left: Node[L],
      right: Node[R],
      matchFreeVariables: Boolean = false,
  ): Boolean = getIsomorphism(left, right, matchFreeVariables).isRight
}
