...
class ExpressionEqualityCheck[G](info: Option[AnnotationVariableInfo[G]]) {
  ...
  def equalExpressionsRecurse(lhs: Expr[G], rhs: Expr[G]): Boolean = {
    ...
    def commAssoc[B <: BinExpr[G]](e1: B, e2: B)(implicit
        tag: ClassTag[B]
    ): Boolean = {
...
class AnnotationVariableInfoGetter[G]() {
  ...
  def extractComparisons(e: Expr[G]): Unit = {
    e match {
      ...
      // n == m + 1 then m < n
      case Eq(v1: Local[G], Plus(v2: Local[G], IntegerValue(i))) =>
        varEqVarPlusInt(v1, v2, i)
      case Eq(v1: Local[G], Plus(IntegerValue(i), v2: Local[G])) =>
        varEqVarPlusInt(v1, v2, i)
      case Eq(Plus(v2: Local[G], IntegerValue(i)), v1: Local[G]) =>
        varEqVarPlusInt(v1, v2, i)
      case Eq(Plus(IntegerValue(i), v2: Local[G]), v1: Local[G]) =>
        varEqVarPlusInt(v1, v2, i)
  ...
  def getInfo(annotations: Seq[Expr[G]]): AnnotationVariableInfo[G] = {
    ...
    for (clause <- annotations) { extractComparisons(clause) }
