...
trait FieldLocationImpl[G] extends FieldLocationOps[G] with LocationImpl[G] {
  this: FieldLocation[G] =>
  override def check(context: CheckContext[G]): Seq[CheckError] =
    if (field.decl.flags.collectFirst { case Final() => () }.nonEmpty)
      Seq(FinalPermission(this)) ++ super.check(context)
    else
      super.check(context)
