...
trait JavaClassOrInterfaceImpl[G] {
  this: JavaClassOrInterface[G] =>
  def name: String
  ...
  def transSupportArrows(
      seen: Set[JavaClassOrInterface[G]]
  ): Seq[(JavaClassOrInterface[G], JavaClassOrInterface[G])] = {
  ...
  def findMethodByName[Decl <: ClassDeclaration[G]](
      name: String
  )(implicit tag: ClassTag[Decl]): Seq[Decl] = {
    def matches(decl: ClassDeclaration[G]): Boolean =
      decl match {
    ...
    decls.collect { case decl: Decl if matches(decl) => decl }
  }
}
