...
object JavaAnnotationImpl {
  case class AnnotationElementMissing(ann: JavaAnnotation[_], element: String)
      extends UserError {
    override def code: String = "annotationElementMissing"

    override def text: String =
      ann.o.messageInContext(s"The element $element should be present")
  }
}
...
object JavaAnnotationEx {
  def unapply[G](
      ja: JavaAnnotation[G]
  ): Option[(Type[G], Seq[(String, Expr[G])], JavaAnnotationData[G])] = {
    ja.data match {
...
trait JavaAnnotationImpl[G] extends JavaAnnotationOps[G] {
  this: JavaAnnotation[G] =>
  def get(n: String): Option[Expr[G]] = args.find(_._1 == n).map(_._2)

  def expect(elem: String): Expr[G] =
    get(elem) match {
      case Some(expr) => expr
  ...
  override def layout(implicit ctx: Ctx): Doc =
    Text("@") <> name <> "(" <> Doc.args(args.map { case (key, value) =>
      Text(key) <> "=" <> value
    }) <> ")"
}
