...
import vct.col.ast.ops.JavaInvocationOps

trait JavaInvocationImpl[G] extends JavaInvocationOps[G] {
  this: JavaInvocation[G] =>
  ...
  override def layout(implicit ctx: Ctx): Doc =
    obj.map(obj => assoc(obj) <> ".").getOrElse(Empty) <> method <>
