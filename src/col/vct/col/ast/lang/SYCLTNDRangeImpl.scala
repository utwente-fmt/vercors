...
trait SYCLTNDRangeImpl[G] { this: SYCLTNDRange[G] =>
  ...
  def findConstructor(genericArgs: Seq[CPPExprOrTypeSpecifier[G]], args: Seq[Expr[G]]): Option[CPPInvocationTarget[G]] = genericArgs match {
    case Seq(CPPExprOrTypeSpecifier(Some(IntegerValue(dim)), None)) if dim > 0 && dim <= 3 && Util.compatTypes(args, Seq(SYCLTRange[G](dim.toInt), SYCLTRange[G](dim.toInt))) => Some(RefSYCLConstructorDefinition(SYCLTNDRange(dim.toInt)))
