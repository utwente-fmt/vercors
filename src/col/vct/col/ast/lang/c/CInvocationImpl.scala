...
trait CInvocationImpl[G] extends CInvocationOps[G] {
  this: CInvocation[G] =>
    ...
    ref.get match {
      ...
      case BuiltinInstanceMethod(f) =>
        applicable match {
          ...
          case other =>
            throw Unreachable(
              "BuiltinInstanceMethod resolution of CInvocation must invoke a CStructAccess."
            )
        }
    }
  ...
  override def layout(implicit ctx: Ctx): Doc =
    Group(
      assoc(applicable) <> "(" <> Doc.args(args) <> ")" <>
