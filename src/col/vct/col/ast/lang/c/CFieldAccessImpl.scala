...
import vct.col.ast.{CFieldAccess, CPrimitiveType, CSpecificationType, TCInt, TEnum, TOpenCLVector, Type}
...
trait CFieldAccessImpl[G] extends CFieldAccessOps[G] { this: CFieldAccess[G] =>
  override lazy val t: Type[G] = ref.get match {
    ...
    case ref: RefOpenCLVectorMembers[G] =>
      val innerType = obj.t match {
        case CPrimitiveType(Seq(CSpecificationType(v: TOpenCLVector[G]))) => v.innerType
        case v: TOpenCLVector[G] => v.innerType
        case _ => ???
      }

      if(ref.idx.size == 1) innerType
      else if(ref.idx.size > 1) TOpenCLVector(ref.idx.size, innerType)
      else ???
