...
trait DeclarationImpl[G] extends NodeImpl[G] {
  this: Declaration[G] =>
  var debugRewriteState: DebugRewriteState = NotProcessed

  def drop(): Unit = debugRewriteState = Dropped

  def rewriteDefault[G1]()(
      implicit rw: AbstractRewriter[G, G1]
  ): Declaration[G1]

  /** Create a Ref to this declaration. This is often useful in a place where
    * the type of the ref can be directly inferred, e.g.
    * `FunctionInvocation(func.ref, ...)`. The witness to `this.type <:< T`
    * demands that the inferred T at least supports the type of this
    * declaration.
    */
  def ref[Decl <: Declaration[G]](
      implicit tag: ClassTag[Decl],
      witness: this.type <:< Decl,
  ): Ref[G, Decl] = new DirectRef[G, Decl](this)

  override def check(context: CheckContext[G]): Seq[CheckError] =
    try {
      NopCoercingRewriter()
        .coerce(this.asInstanceOf[Declaration[InitialGeneration]])
      Nil
    } catch {
  ...
  override def enterCheckContextDeclarationStack(
      context: CheckContext[G]
  ): Seq[Declaration[G]] = this +: context.declarationStack
}
