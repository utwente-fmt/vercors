...
trait ProcedureImpl[G] extends ProcedureOps[G] {
  this: Procedure[G] =>
  def layoutSilver(implicit ctx: Ctx): Doc =
    Group(
      Text("method") <+> ctx.name(this) <> "(" <> Doc.args(args) <> ")" <>
        (if (outArgs.nonEmpty)
           Text(" returns (") <> Doc.args(typeArgs.map(ctx.name).map(Text)) <>
             ")"
         else
           Empty)
    ) <+/> contract <>
      (if (body.nonEmpty)
         Line <> body.get.layoutAsBlock
       else
         Empty)
