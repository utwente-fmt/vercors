...
trait ClassImpl[G] extends Declarator[G] {
  this: Class[G] =>
  protected def transSupportArrows(
      seen: Set[Class[G]]
  ): Seq[(Class[G], Class[G])] =
    if (seen.contains(this))
      Nil
    else
      supports.map(other => (this, other.decl)) ++
        supports
          .flatMap(other => other.decl.transSupportArrows(Set(this) ++ seen))

  def transSupportArrows: Seq[(Class[G], Class[G])] =
    transSupportArrows(Set.empty)
    ...
    (if (intrinsicLockInvariant == tt[G])
      ...
      Text("class") <+> ctx.name(this) <>
        (if (supports.isEmpty)
           ...
           Text(" implements") <+>
             Doc.args(supports.map(ctx.name).map(Text))) <+> "{"
