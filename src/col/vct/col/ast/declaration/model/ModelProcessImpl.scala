...
trait ModelProcessImpl[G] extends ModelProcessOps[G] {
  this: ModelProcess[G] =>
  override def returnType: Type[G] = TProcess()
  override def body: Option[Node[G]] = Some(impl)
  override def check(context: CheckContext[G]): Seq[CheckError] =
    impl.checkSubType(TProcess()) ++ requires.checkSubType(TBool()) ++
      ensures.checkSubType(TBool())
  ...
  override def layout(implicit ctx: Ctx): Doc =
    DocUtil.clauses("requires", requires) <+/>
      ...
      (if (accessible.nonEmpty)
         ...
         Empty) <> DocUtil.clauses("ensures", ensures) <+/> Group(
        Group(
          Text("process") <+> ctx.name(this) <> "(" <> Doc.args(args) <> ") ="
        ) <+> impl
      )
}
