...
trait ParInvariantImpl[G] extends Declarator[G] with ParInvariantOps[G] { this: ParInvariant[G] =>
  override def declarations: Seq[Declaration[G]] = Seq(decl)

  override def layout(implicit ctx: Ctx): Doc =
    Group(Text("invariant") <+> ctx.name(decl) <> "(" <> Doc.arg(inv) <> ")") <+> content.layoutAsBlock
}