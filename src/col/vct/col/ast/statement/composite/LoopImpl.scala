...
trait LoopImpl[G] {
  ...
  def getVariableAndLowerBound(implicit
      o: Origin
  ...
  def getIterationContractData(implicit
      o: Origin
  ...
  def layoutControlElement(e: Show)(implicit ctx: Ctx): Option[Show] =
    Some(e match {
      case Eval(e) => e.show
      case a: Assign[G] => a.layoutAsExpr
      case e: VeyMontAssignExpression[G] =>
        return layoutControlElement(e.assign)
  ...
  def layoutControl(stat: Statement[G])(implicit ctx: Ctx): Doc =
    Group(Doc.args(
      stat.blockElementsForLayout
        .map(layoutControlElement(_).getOrElse(return stat.show))
    ))
