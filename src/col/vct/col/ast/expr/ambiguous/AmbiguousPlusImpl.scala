...
trait AmbiguousPlusImpl[G] {
  ...
  def isProcessOp: Boolean =
    CoercionUtils.getCoercion(left.t, TProcess()).isDefined
  ...
  def isPointerOp: Boolean =
    CoercionUtils.getAnyPointerCoercion(left.t).isDefined
  ...
  override lazy val t: Type[G] =
    if (isProcessOp)
      TProcess()
    else if (isSeqOp || isBagOp || isSetOp)
