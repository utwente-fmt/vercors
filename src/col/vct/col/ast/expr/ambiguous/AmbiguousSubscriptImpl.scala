...
trait AmbiguousSubscriptImpl[G] extends AmbiguousSubscriptOps[G] {
  this: AmbiguousSubscript[G] =>
  def isSeqOp: Boolean = CoercionUtils.getAnySeqCoercion(collection.t).isDefined
  def isVectorOp: Boolean =
    CoercionUtils.getAnyVectorCoercion(collection.t).isDefined
  def isArrayOp: Boolean =
    CoercionUtils.getAnyArrayCoercion(collection.t).isDefined
  def isCArrayOp: Boolean =
  ...
  override lazy val t: Type[G] =
    if (isSeqOp)
      collection.t.asSeq.get.element
    else if (isVectorOp)
      collection.t.asVector.get.element
    else if (isArrayOp)
