...
object AstBuildHelpers {
  ...
  implicit class ExprBuildHelpers[G](left: Expr[G]) {
    ...
    def /(
        right: Expr[G]
    )(implicit origin: Origin, blame: Blame[DivByZero]): FloorDiv[G] =
      FloorDiv(left, right)(blame)
    def /:/(
        right: Expr[G]
    )(implicit origin: Origin, blame: Blame[DivByZero]): Div[G] =
      Div(left, right)(blame)
    def %(
        right: Expr[G]
    ...
    def ~>(field: SilverField[G])(implicit
    ...
    def rewrite(
      ...
      contracted match {
        case function: Function[Pre] =>
          new RewriteFunction(function).rewrite(
            args = args,
        ...
        case function: InstanceFunction[Pre] =>
          new RewriteInstanceFunction(function).rewrite(
            args = args,
            returnType = returnType,
            typeArgs = typeArgs,
            inline = inline,
        ...
        case function: InstanceOperatorFunction[Pre] =>
          new RewriteInstanceOperatorFunction(function).rewrite(
            args = args,
            returnType = returnType,
            inline = inline,
            contract = contract,
        ...
        case function: LlvmSpecFunction[Pre] =>
          new RewriteLlvmSpecFunction(function).rewrite(
            args = args,
            returnType = returnType,
            typeArgs = typeArgs,
            inline = inline,
    ...
    def rewrite(
      ...
      method match {
        case procedure: Procedure[Pre] =>
          new RewriteProcedure(procedure).rewrite(
            args = args,
            returnType = returnType,
            body = body,
            inline = inline,
        ...
        case method: InstanceMethod[Pre] =>
          new RewriteInstanceMethod(method).rewrite(
            args = args,
            returnType = returnType,
            body = body,
            inline = inline,
        ...
        case method: InstanceOperatorMethod[Pre] =>
          new RewriteInstanceOperatorMethod(method).rewrite(
            returnType = returnType,
    ...
    def rewrite(
      ...
      predicate match {
        case predicate: Predicate[Pre] =>
          new RewritePredicate(predicate)
            .rewrite(args = args, inline = inline, threadLocal = threadLocal)
        case predicate: InstancePredicate[Pre] =>
          new RewriteInstancePredicate(predicate)
            .rewrite(args = args, inline = inline, threadLocal = threadLocal)
      }
  }

  implicit class ApplyBuildHelpers[Pre, Post](apply: Apply[Pre])(implicit
      rewriter: AbstractRewriter[Pre, Post]
  ) {
    def rewrite(
    ...
    def rewrite(
      ...
      apply match {
        ...
        case inv: InstancePredicateApply[Pre] =>
          new RewriteInstancePredicateApply(inv)
            .rewrite(args = args, perm = perm)
        case inv: CoalesceInstancePredicateApply[Pre] =>
          new RewriteCoalesceInstancePredicateApply(inv)
            .rewrite(args = args, perm = perm)
      }
  }
    ...
    def rewrite(
      ...
      apply match {
        case inv: FunctionInvocation[Pre] =>
          new RewriteFunctionInvocation(inv).rewrite(
            args = args,
            typeArgs = typeArgs,
            givenMap = givenMap,
            yields = yields,
        ...
        case inv: InstanceFunctionInvocation[Pre] =>
          new RewriteInstanceFunctionInvocation(inv).rewrite(
            args = args,
            typeArgs = typeArgs,
            givenMap = givenMap,
            yields = yields,
    ...
    def rewrite(
      ...
      apply match {
        case inv: ProcedureInvocation[Pre] =>
          new RewriteProcedureInvocation(inv).rewrite(
            args = args,
            outArgs = outArgs,
            typeArgs = typeArgs,
            givenMap = givenMap,
        ...
        case inv: MethodInvocation[Pre] =>
          new RewriteMethodInvocation(inv).rewrite(
            args = args,
            outArgs = outArgs,
            typeArgs = typeArgs,
            givenMap = givenMap,
    ...
    def rewrite(
      ...
      apply match {
        case inv: InvokeProcedure[Pre] =>
          new RewriteInvokeProcedure(inv).rewrite(
            args = args,
            outArgs = outArgs,
            typeArgs = typeArgs,
            givenMap = givenMap,
        ...
        case inv: InvokeMethod[Pre] =>
          new RewriteInvokeMethod(inv).rewrite(
            args = args,
            outArgs = outArgs,
            typeArgs = typeArgs,
            givenMap = givenMap,
  ...
  def assignLocal[G](local: Local[G], value: Expr[G])(implicit
      o: Origin
  ...
  def implies[G](conds: Seq[Expr[G]], body: Expr[G])(implicit
      o: Origin
  ): Expr[G] =
    conds match {
      case Nil => body
