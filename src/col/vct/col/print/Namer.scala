...
case class Namer[G](syntax: Ctx.Syntax) {
  ...
  private def nearestClass =
    nearest {
      case _: Class[G] | _: JavaClass[G] | _: VeyMontSeqProg[G] |
          _: JavaInterface[G] | _: JavaAnnotationInterface[G] =>
        ()
    }

  private def nearestVariableScope =
    nearest {
      case _: ParBlock[G] => ()
      case _: VecBlock[G] => ()
  ...
  def name(node: Node[G]): Unit = {
    ...
    node match {
      ...
      case decl: ModelDeclaration[G] =>
        nameKeyed(nearest { case _: Model[G] => () }, decl)
      case decl: EnumConstant[G] =>
        nameKeyed(nearest { case _: Enum[G] => () }, decl)
      case decl: Variable[G] => nameKeyed(nearestVariableScope, decl)
      case decl: LabelDecl[G] => nameKeyed(nearestCallable, decl)
      case decl: SendDecl[G] =>
        nameKeyed(nearest { case _: ParBlock[G] | _: Loop[G] => () }, decl)
      case decl: ParBlockDecl[G] =>
        nameKeyed(nearest { case _: ParBlock[G] => () }, decl)
      case decl: ParInvariantDecl[G] =>
        nameKeyed(nearest { case _: ParInvariant[G] => () }, decl)
      ...
      case decl: VeyMontThread[G] =>
        nameKeyed(nearest { case _: VeyMontSeqProg[G] => () }, decl)
  ...
  def finish: Map[Declaration[G], String] =
    Map(names.map { case (_, name, index) -> decl =>
      decl -> packName(name, index)
    }.toSeq: _*)
}
