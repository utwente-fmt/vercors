...
object DocUtil {
  ...
  def givenYieldsMapping(keyword: String, mapping: Seq[(Doc, Doc)])(
      implicit ctx: Ctx
  ): Doc =
    if (mapping.isEmpty)
      Empty
  ...
  def argsOutArgs[G](args: Seq[Expr[G]], outArgs: Seq[Expr[G]])(
      implicit ctx: Ctx
  ): Doc =
    outArgs match {
  ...
  def javaGenericArgs[G](args: Seq[Type[G]])(implicit ctx: Ctx): Doc =
    args match {
      case Seq() => Empty
      case _ => Text("<") <> Doc.args(args) <> ">"
    }

  def javaGenericParams[G](args: Seq[Variable[G]])(implicit ctx: Ctx): Doc =
    args match {
      case Seq() => Empty
      case _ => Text("<") <> Doc.args(args) <> ">"
    }
