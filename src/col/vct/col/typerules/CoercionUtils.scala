  ...
  def getCoercion[G](source: Type[G], target: Type[G]): Option[Coercion[G]] = {
    Some((source, target) match {
      ...
      case (source @ TOption(innerSource), target @ TOption(innerTarget)) =>
        CoerceMapOption(
          getCoercion(innerSource, innerTarget).getOrElse(return None),
          innerSource,
          innerTarget,
        )
      case (
      ...
      case (_ @CTArray(_, innerType), _ @TArray(element))
          if element == innerType =>
        CoerceCArrayPointer(element)
      case (_ @CPPTArray(_, innerType), _ @TArray(element))
          if element == innerType =>
      ...
      case (source @ TBoundedInt(_, _), target @ TFloat(_, _)) =>
      ...
      case (source: TBoundedInt[G], TRational()) =>
        CoercionSequence(Seq(CoerceUnboundInt(source), CoerceIntRat()))
      ...
      case (source @ TUnion(ts), target) =>
        CoerceJoinUnion(
          ts.map(getCoercion(_, target)).map {
      ...
      case (source, target @ TUnion(ts)) =>
        return ts.map(getCoercion(source, _)).zipWithIndex
