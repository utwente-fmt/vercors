...
case object CoercingRewriter {
  ...
  case class IncoercibleExplanation(blame: Expr[_], message: String)
      extends CoercionError
...
abstract class CoercingRewriter[Pre <: Generation]()
    extends AbstractRewriter[Pre, Rewritten[Pre]] with LazyLogging {
  ...
  /** Apply a particular coercion to an expression. SAFETY: all promoting
    ...
    */
  def applyCoercion(e: => Expr[Post], coercion: Coercion[Pre])(implicit
      o: Origin
  ): Expr[Post] = {
    coercion match {
  ...
  override final def dispatch(e: Expr[Pre]): Expr[Post] =
    e match {
  ...
  def postCoerce(region: ParRegion[Pre]): ParRegion[Post] =
    rewriteDefault(region)
  ...
  def postCoerce(node: Verification[Pre]): Verification[Post] =
    rewriteDefault(node)
  override final def dispatch(node: Verification[Pre]): Verification[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: VerificationContext[Pre]): VerificationContext[Pre] = node
  def postCoerce(node: VerificationContext[Pre]): VerificationContext[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: VerificationContext[Pre]
  ): VerificationContext[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: Program[Pre]): Program[Pre] = node
  def postCoerce(node: Program[Pre]): Program[Post] = rewriteDefault(node)
  override final def dispatch(node: Program[Pre]): Program[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: Type[Pre]): Type[Pre] = node
  def postCoerce(node: Type[Pre]): Type[Post] = rewriteDefault(node)
  override final def dispatch(node: Type[Pre]): Type[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: LoopContract[Pre]): LoopContract[Pre] = node
  def postCoerce(node: LoopContract[Pre]): LoopContract[Post] =
    rewriteDefault(node)
  override final def dispatch(node: LoopContract[Pre]): LoopContract[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: CatchClause[Pre]): CatchClause[Pre] = node
  def postCoerce(node: CatchClause[Pre]): CatchClause[Post] =
    rewriteDefault(node)
  override final def dispatch(node: CatchClause[Pre]): CatchClause[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: IterVariable[Pre]): IterVariable[Pre] = node
  def postCoerce(node: IterVariable[Pre]): IterVariable[Post] =
    rewriteDefault(node)
  override final def dispatch(node: IterVariable[Pre]): IterVariable[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: SignalsClause[Pre]): SignalsClause[Pre] = node
  def postCoerce(node: SignalsClause[Pre]): SignalsClause[Post] =
    rewriteDefault(node)
  override final def dispatch(node: SignalsClause[Pre]): SignalsClause[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: DecreasesClause[Pre]): DecreasesClause[Pre] = node
  def postCoerce(node: DecreasesClause[Pre]): DecreasesClause[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: DecreasesClause[Pre]
  ): DecreasesClause[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: ApplicableContract[Pre]): ApplicableContract[Pre] = node
  def postCoerce(node: ApplicableContract[Pre]): ApplicableContract[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: ApplicableContract[Pre]
  ): ApplicableContract[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: AccountedPredicate[Pre]): AccountedPredicate[Pre] = node
  def postCoerce(node: AccountedPredicate[Pre]): AccountedPredicate[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: AccountedPredicate[Pre]
  ): AccountedPredicate[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: FieldFlag[Pre]): FieldFlag[Pre] = node
  def postCoerce(node: FieldFlag[Pre]): FieldFlag[Post] = rewriteDefault(node)
  override final def dispatch(node: FieldFlag[Pre]): FieldFlag[Post] =
    postCoerce(coerce(preCoerce(node)))
  ...
  override final def dispatch(node: Location[Pre]): Location[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: CDeclarationSpecifier[Pre]): CDeclarationSpecifier[Pre] =
    node
  def postCoerce(
      node: CDeclarationSpecifier[Pre]
  ): CDeclarationSpecifier[Post] = rewriteDefault(node)
  override final def dispatch(
      node: CDeclarationSpecifier[Pre]
  ): CDeclarationSpecifier[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: CTypeQualifier[Pre]): CTypeQualifier[Pre] = node
  def postCoerce(node: CTypeQualifier[Pre]): CTypeQualifier[Post] =
    rewriteDefault(node)
  override final def dispatch(node: CTypeQualifier[Pre]): CTypeQualifier[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: CPointer[Pre]): CPointer[Pre] = node
  def postCoerce(node: CPointer[Pre]): CPointer[Post] = rewriteDefault(node)
  override final def dispatch(node: CPointer[Pre]): CPointer[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: CDeclarator[Pre]): CDeclarator[Pre] = node
  def postCoerce(node: CDeclarator[Pre]): CDeclarator[Post] =
    rewriteDefault(node)
  override final def dispatch(node: CDeclarator[Pre]): CDeclarator[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: CInit[Pre]): CInit[Pre] = node
  def postCoerce(node: CInit[Pre]): CInit[Post] = rewriteDefault(node)
  override final def dispatch(node: CInit[Pre]): CInit[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: CDeclaration[Pre]): CDeclaration[Pre] = node
  def postCoerce(node: CDeclaration[Pre]): CDeclaration[Post] =
    rewriteDefault(node)
  override final def dispatch(node: CDeclaration[Pre]): CDeclaration[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: GpuMemoryFence[Pre]): GpuMemoryFence[Pre] = node
  def postCoerce(node: GpuMemoryFence[Pre]): GpuMemoryFence[Post] =
    rewriteDefault(node)
  override final def dispatch(node: GpuMemoryFence[Pre]): GpuMemoryFence[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: CPPDeclarator[Pre]): CPPDeclarator[Pre] = node
  def postCoerce(node: CPPDeclarator[Pre]): CPPDeclarator[Post] =
    rewriteDefault(node)
  override final def dispatch(node: CPPDeclarator[Pre]): CPPDeclarator[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(
      node: CPPDeclarationSpecifier[Pre]
  ): CPPDeclarationSpecifier[Pre] = node
  def postCoerce(
      node: CPPDeclarationSpecifier[Pre]
  ): CPPDeclarationSpecifier[Post] = rewriteDefault(node)
  override final def dispatch(
      node: CPPDeclarationSpecifier[Pre]
  ): CPPDeclarationSpecifier[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: CPPDeclaration[Pre]): CPPDeclaration[Pre] = node
  def postCoerce(node: CPPDeclaration[Pre]): CPPDeclaration[Post] =
    rewriteDefault(node)
  override final def dispatch(node: CPPDeclaration[Pre]): CPPDeclaration[Post] =
    postCoerce(coerce(preCoerce(node)))
  ...
  override final def dispatch(node: JavaImport[Pre]): JavaImport[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: JavaModifier[Pre]): JavaModifier[Pre] = node
  def postCoerce(node: JavaModifier[Pre]): JavaModifier[Post] =
    rewriteDefault(node)
  override final def dispatch(node: JavaModifier[Pre]): JavaModifier[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(
      node: JavaVariableDeclaration[Pre]
  ): JavaVariableDeclaration[Pre] = node
  def postCoerce(
      node: JavaVariableDeclaration[Pre]
  ): JavaVariableDeclaration[Post] = rewriteDefault(node)
  override final def dispatch(
      node: JavaVariableDeclaration[Pre]
  ): JavaVariableDeclaration[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: Operator[Pre]): Operator[Pre] = node
  def postCoerce(node: Operator[Pre]): Operator[Post] = rewriteDefault(node)
  override final def dispatch(node: Operator[Pre]): Operator[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: BipPortType[Pre]): BipPortType[Pre] = node
  def postCoerce(node: BipPortType[Pre]): BipPortType[Post] =
    rewriteDefault(node)
  override final def dispatch(node: BipPortType[Pre]): BipPortType[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(
      node: BipTransitionSignature[Pre]
  ): BipTransitionSignature[Pre] = node
  def postCoerce(
      node: BipTransitionSignature[Pre]
  ): BipTransitionSignature[Post] = rewriteDefault(node)
  override final def dispatch(
      node: BipTransitionSignature[Pre]
  ): BipTransitionSignature[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: BipGlueDataWire[Pre]): BipGlueDataWire[Pre] = node
  def postCoerce(node: BipGlueDataWire[Pre]): BipGlueDataWire[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: BipGlueDataWire[Pre]
  ): BipGlueDataWire[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: BipGlueRequires[Pre]): BipGlueRequires[Pre] = node
  def postCoerce(node: BipGlueRequires[Pre]): BipGlueRequires[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: BipGlueRequires[Pre]
  ): BipGlueRequires[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: BipGlueAccepts[Pre]): BipGlueAccepts[Pre] = node
  def postCoerce(node: BipGlueAccepts[Pre]): BipGlueAccepts[Post] =
    rewriteDefault(node)
  override final def dispatch(node: BipGlueAccepts[Pre]): BipGlueAccepts[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: JavaBipGlueElement[Pre]): JavaBipGlueElement[Pre] = node
  def postCoerce(node: JavaBipGlueElement[Pre]): JavaBipGlueElement[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: JavaBipGlueElement[Pre]
  ): JavaBipGlueElement[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: JavaBipGlueName[Pre]): JavaBipGlueName[Pre] = node
  def postCoerce(node: JavaBipGlueName[Pre]): JavaBipGlueName[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: JavaBipGlueName[Pre]
  ): JavaBipGlueName[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: LlvmFunctionContract[Pre]): LlvmFunctionContract[Pre] =
    node
  def postCoerce(node: LlvmFunctionContract[Pre]): LlvmFunctionContract[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: LlvmFunctionContract[Pre]
  ): LlvmFunctionContract[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: LlvmLoopContract[Pre]): LlvmLoopContract[Pre] = node
  def postCoerce(node: LlvmLoopContract[Pre]): LlvmLoopContract[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: LlvmLoopContract[Pre]
  ): LlvmLoopContract[Post] = postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: ProverLanguage[Pre]): ProverLanguage[Pre] = node
  def postCoerce(node: ProverLanguage[Pre]): ProverLanguage[Post] =
    rewriteDefault(node)
  override final def dispatch(node: ProverLanguage[Pre]): ProverLanguage[Post] =
    postCoerce(coerce(preCoerce(node)))

  def preCoerce(node: SmtlibFunctionSymbol[Pre]): SmtlibFunctionSymbol[Pre] =
    node
  def postCoerce(node: SmtlibFunctionSymbol[Pre]): SmtlibFunctionSymbol[Post] =
    rewriteDefault(node)
  override final def dispatch(
      node: SmtlibFunctionSymbol[Pre]
  ): SmtlibFunctionSymbol[Post] = postCoerce(coerce(preCoerce(node)))

  def coerce(value: Expr[Pre], target: Type[Pre]): Expr[Pre] =
    ApplyCoercion(
      value,
      CoercionUtils.getCoercion(value.t, target) match {
  ...
  def coerceArgs(
    ...
    args.zip(app.args).map { case (value, arg) =>
      coerce(value, arg.t.particularize(app.typeArgs.zip(tArgs).toMap))
    }

  def coerceGiven(
      givenMap: Seq[(Ref[Pre, Variable[Pre]], Expr[Pre])]
  ): Seq[(Ref[Pre, Variable[Pre]], Expr[Pre])] =
    givenMap.map { case (Ref(v), e) => (v.ref, coerce(e, v.t)) }
  ...
  def firstOk[T](
    ...
    Left(Nil).onCoercionError(alt1).onCoercionError(alt2).onCoercionError(alt3)
      ...
      .onCoercionError(alt10).onCoercionError(alt11) match {
      case Left(errs) =>
  ...
  def coerce(e: Expr[Pre]): Expr[Pre] = {
    ...
    e match {
      case ApplyCoercion(_, _) =>
        throw Unreachable(
          "All instances of ApplyCoercion should be immediately rewritten by CoercingRewriter.dispatch."
        )
      ...
      case minus @ AmbiguousMinus(left, right) =>
        firstOk(
          e,
          s"Expected both operands to be numeric, a set or a bag; or a pointer and integer, but got ${left
              .t} and ${right.t}.",
      ...
      case AmbiguousMult(left, right) =>
        firstOk(
          e,
          s"Expected both operands to be numeric, a process, a set or a bag but got ${left
              .t} and ${right.t}.",
      ...
      case plus @ AmbiguousPlus(left, right) =>
        firstOk(
          e,
          s"Expected both operands to be numeric, a process, a sequence, set, bag, or string; or a pointer and integer, but got ${left
              .t} and ${right.t}.",
      ...
      case sub @ AmbiguousSubscript(collection, index) =>
        firstOk(
          e,
          s"Expected collection to be a sequence, array, pointer or map, but got ${collection.t}.",
      ...
      case CastFloat(e, t) => CastFloat(float(e), t)
      ...
      case CStructDeref(struct, field) => CStructDeref(struct, field)
      ...
      case inv @ FunctionInvocation(ref, args, typeArgs, givenMap, yields) =>
        FunctionInvocation(
          ref,
          coerceArgs(args, ref.decl, typeArgs),
          typeArgs,
          coerceGiven(givenMap),
      ...
      case ThisSeqProg(ref) => ThisSeqProg(ref)
  ...
  def coerce(stat: Statement[Pre]): Statement[Pre] = {
    ...
    stat match {
      ...
      case a @ Assign(target, value) =>
        try { Assign(target, coerce(value, target.t))(a.blame) }
        catch {
      ...
      case VeyMontCommExpression(r, s, t, a) =>
        VeyMontCommExpression(r, s, t, a)
    }
  }
  ...
  def coerce(decl: Declaration[Pre]): Declaration[Pre] = {
    ...
    decl match {
      ...
      case namespace: CPPNamespaceDefinition[Pre] => namespace
  ...
  def coerce(node: Type[Pre]): Type[Pre] = node
