...
/** Defines how to construct a [[Rewriter]] for the transformation stage, when
  * the rewriter takes an argument.
  *
  * Similarly to [[RewriterBuilder]], for case classes this shape of `apply` is
  * implicitly defined:
  *
  * {{{
  ...
  *
  * @tparam T
  *   The type of the argument the rewriter takes.
  */
  ...
  def withArg(x: T): RewriterBuilder =
    new RewriterBuilder {
      override def apply[Pre <: Generation]()
        ...
        RewriterBuilderArg.this.apply(x)
...
trait RewriterBuilderArg2[T, U] {
  ...
  def withArg(t: T, u: U): RewriterBuilder =
    new RewriterBuilder {
      override def apply[Pre <: Generation]()
          : AbstractRewriter[Pre, _ <: Generation] =
        RewriterBuilderArg2.this.apply(t, u)
