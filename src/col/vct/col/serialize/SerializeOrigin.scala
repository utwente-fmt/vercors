...
object SerializeOrigin {
  def deserialize(@unused origin: ser.Origin): Origin =
    Origin(origin.content.map(_.content).map {
      ...
      case ser.OriginContent.Content.Context(context) => DeserializedContext(context.context, context.inlineContext, context.shortPosition)
    })
  ...
  def serialize(@unused origin: Origin): ser.Origin =
    ser.Origin(origin.originContents.flatMap {
      ...
      case DeserializedContext(context, inlineContext, shortPosition) => Seq(ser.OriginContent.Content.Context(ser.Context(context, inlineContext, shortPosition)))
      case _ => Nil
...
case class DeserializedContext(context: String, inlineContext: String, shortPosition: String) extends Context {
  ...
  override protected def inlineContextHere(tail: Origin, compress: Boolean): (String, Origin) = (inlineContext, tail)
