...
/** * This trait is used to box information about Origins in a structured
  * manner.
  */
trait OriginContent {

  /** Indicates the preferred name or required name for this node. The parts of
    * a preferred name can be joined as desired for the kind of declaration.
    */
  def name(tail: Origin): Option[Name] = None

  /** The reason that this node exists, typeset in a user-friendly way. The
    * contexts are stacked, so context (i) can say something about (i+1). For
    * example: Seq("interpreted c source", "Interpreted from:", "c source"), or
    * Seq("check_sat method for:", "pvl source"). For this method the messages
    * can be multiline, but should not have a trailing or preceding newline.
    */
  def context(tail: Origin): Option[Seq[String]] = None

  /** Should have the same shape as the context method, but instead provides a
    * sequence of breadcrumbs, e.g.: Seq("check_sat", "void test() { }") The
    * part must not contain newlines. Consumers of the inline context should
    * make sure to shorten breadcrumbs and add an ellipsis where approriate,
    * whereas the definition should not try to shorten the breadcrumb (within
    * reason).
    */
  def inlineContext(
      tail: Origin,
  ...
  /** Short description of the source position indicated by this origin. This
    * should refer to the oldest cq original position; not the most recent. If
    * no OriginContent indicates a shortPosition, it should be assumed the node
    * is generated apart from any source input.
    */
  def shortPosition(tail: Origin): Option[String] = None
}

/** Leaf name provider, should consider modifiers like NamePrefix that follow
  * it.
  */
...
case class PreferredName(preferredName: Seq[String]) extends NameStrategy {
  override def name(tail: Origin): Option[Name] =
    Some(Name.Preferred(tail.span[NameStrategy]._1.originContents.collect {
...
case class IndirectName(name: Name) extends NameStrategy {
  override def name(tail: Origin): Option[Name] =
    Some(
      (prefix(tail).map(Seq(_)).getOrElse(Seq()) ++ Seq(name)).reduce(Name.Join)
    )
...
trait Context extends OriginContent {
  ...
  protected def inlineContextHere(
      tail: Origin,
      compress: Boolean,
  ): (String, Origin)
...
/** One or two lowercase words define the context, inline context and short
  * position. For more extended explanations you should implement a case of
  * Context yourself.
  */
...
trait Source extends Context {
  ...
  override protected def inlineContextHere(
      tail: Origin,
      compress: Boolean,
  ): (String, Origin) = (genericInlineContext, tail)
  override protected def shortPositionHere(tail: Origin): (String, Origin) =
    (genericShortPosition, tail)
}
...
case class ReadableOrigin(readable: Readable) extends Source {
  ...
  def inlinePositionContext(
      position: PositionRange,
      compress: Boolean,
  ): String = InputOrigin.inlineContext(readable, position, compress)
...
case class InlineBipContext(bipContext: String) extends Source {
  def positionContext(position: PositionRange): String =
    InputOrigin.contextLines(LiteralReadable("literal", bipContext), position)
  def inlinePositionContext(
      position: PositionRange,
      compress: Boolean,
  ): String =
    InputOrigin.inlineContext(LiteralReadable("literal", bipContext), position)
...
case class PositionRange(
  ...
  def onlyPositionText: String =
    startEndColIdx match {
      ...
      case None => s":${startLineIdx + 1}-:${endLineIdx + 1}"
    }
  ...
  override protected def inlineContextHere(
    ...
    tail.spanFind[Source] match {
      case (_, Some((source, tail))) =>
        (source.inlinePositionContext(this, compress), tail)
  ...
  override protected def shortPositionHere(tail: Origin): (String, Origin) =
    tail.spanFind[Source] match {
      case (_, Some((source, tail))) =>
        (s"${source.genericShortPosition}$onlyPositionText", tail)
      case (_, None) => ("broken position", tail)
    }
}

/** A sequence of OriginContents. This sequence can be mutated (add, remove,
  * replace) for convenience.
  * @param originContents
  *   The known origin contents at the time of Origin creation. Can be empty for
  *   a new Origin.
  */
final case class Origin(originContents: Seq[OriginContent])
    extends Blame[VerificationFailure] with HasContext {
  def tail: Origin = Origin(originContents.tail)

  def find[T <: OriginContent](implicit tag: ClassTag[T]): Option[T] =
    originContents.collectFirst { case t: T => t }

  def findAll[T <: OriginContent](implicit tag: ClassTag[T]): Seq[T] =
    originContents.collect { case t: T => t }
  ...
  def spanFind[T <: OriginContent](
      implicit tag: ClassTag[T]
  ): (Origin, Option[(T, Origin)]) = {
  ...
  def where(
    ...
    Origin(
      Option(context).map(LabelContext).toSeq ++ Option(name)
        .map(name => PreferredName(Seq(name))).toSeq ++ Option(indirect)
        .map(name => IndirectName(name)).toSeq ++ Option(prefix).map(NamePrefix)
        .to(Seq) ++ originContents
    )

  /** Do not use to indicate a preferred name. This is to indicate the formal
    * source name in the input. Use where(name = ...) instead.
    */
  def sourceName(name: String): Origin = withContent(SourceName(name))
...
//    withContent(OriginCols(cols))
  ...
  def getPreferredName: Option[Name] =
    originContents.headOption
      .flatMap(_.name(tail).orElse(tail.getPreferredName))
  ...
  def inlineContext(compress: Boolean = true): Option[Seq[String]] =
    originContents.headOption.flatMap(
      _.inlineContext(tail, compress).orElse(tail.inlineContext(compress))
    )
  ...
  def shortPosition: Option[String] =
    originContents.headOption
      .flatMap(_.shortPosition(tail).orElse(tail.shortPosition))

  def shortPositionText: String = shortPosition.getOrElse("[unknown position]")
...
object InputOrigin {
  ...
  def contextLines(readable: Readable, position: PositionRange): String = {
    ...
    val cols = position.startEndColIdx.map { case (startColIdx, endColIdx) =>
  ...
  def inlineContext(
    ...
    def compressF: String => String =
      if (compress)
        compressInlineText
      else
        identity[String]
    readable.readLines()
      .slice(position.startLineIdx, position.endLineIdx + 1) match {
