...
case class TypeErrorExplanation(expr: Expr[_], message: String)
    extends CheckError
...
case object CheckContext {
  case class ScopeFrame[G](
    ...
    private lazy val scannedDeclSet =
      scanLazily.flatMap(ResolveReferences.scanScope(_, inGPUKernel = false))
        .toSet
