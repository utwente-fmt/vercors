...
case class ReferenceResolutionContext[G](
    ...
    javaBipStatePredicates: ListMap[Expr[G], JavaAnnotation[G]] =
      ListMap[Expr[G], JavaAnnotation[G]](),
    javaBipGuards: ListMap[Expr[G], JavaMethod[G]] =
      ListMap[Expr[G], JavaMethod[G]](),
  ...
  def asTypeResolutionContext: TypeResolutionContext[G] =
    TypeResolutionContext(
      stack,
      currentJavaNamespace,
      None,
      Nil,
      inGpuKernel,
      externallyLoadedElements,
    )
  ...
  def withCheckContext(ctx: CheckContext[G]): ReferenceResolutionContext[G] = {
    ...
    copy(checkContext = ctx, stack = stack.map(_.filter(!filter.contains(_))))
  }
  ...
  def declareJavaBipGuards(
      gs: Seq[(Expr[G], JavaMethod[G])]
  ): ReferenceResolutionContext[G] =
    copy(javaBipGuards = javaBipGuards ++ gs.toMap)

  def enableJavaBipGuards(): ReferenceResolutionContext[G] =
    copy(javaBipGuardsEnabled = true)

  def currentPkg: Option[JavaName[G]] = currentJavaNamespace.flatMap(_.pkg)
  def currentFqn: Option[JavaName[G]] =
    currentPkg.map(pkg =>
      JavaName(
        pkg.names ++ currentJavaClass.map(cls => Seq(cls.name)).getOrElse(Seq())
      )(DiagnosticOrigin)
    )

  def appendTypeEnv(
      typeEnv: Map[Variable[G], Type[G]]
  ): ReferenceResolutionContext[G] = copy(typeEnv = this.typeEnv ++ typeEnv)
}
