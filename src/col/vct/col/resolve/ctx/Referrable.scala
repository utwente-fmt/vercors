...
sealed trait SpecDerefTarget[G]
    extends CDerefTarget[G] with JavaDerefTarget[G] with PVLDerefTarget[G]
...
case class RefCPPFunctionDefinition[G](decl: CPPFunctionDefinition[G])
    ...
    with CPPInvocationTarget[G]
    with ResultTarget[G]
case class RefCPPNamespaceDefinition[G](decl: CPPNamespaceDefinition[G])
    extends Referrable[G]
...
case class RefVeyMontThread[G](decl: VeyMontThread[G])
...
case class ImplicitDefaultJavaConstructor[G]()
    extends Referrable[G] with JavaConstructorTarget[G]
case class ImplicitDefaultPVLConstructor[G]()
    extends Referrable[G] with PVLConstructorTarget[G]
