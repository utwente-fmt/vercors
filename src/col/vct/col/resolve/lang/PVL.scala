...
case object PVL {
    ...
    t match {
      case TClass(Ref(cls)) =>
        val resolvedCons = cls.declarations.collectFirst {
          case cons: PVLConstructor[G] if Util.compat(args, cons.args) =>
            RefPVLConstructor(cons)
        }
  ...
  def findInstanceMethod[G](
    ...
    obj.t match {
      ...
      case TClass(ref) =>
        ref.decl.declarations.flatMap(Referrable.from).collectFirst {
          case ref: RefInstanceFunction[G]
              if ref.name == method && Util.compat(args, typeArgs, ref.decl) =>
            ref
      ...
      case _ =>
        PVL.builtinInstanceMethod(obj, method, args)
          .orElse(Spec.builtinInstanceMethod(obj, method, blame))
    }
  ...
  def builtinInstanceMethod[G](
      ...
      method: String,
      args: Seq[Expr[G]],
  ): Option[PVLBuiltinInstanceMethod[G]] = {
    implicit val o: Origin = obj.o
    Some((obj.t, method, args) match {
      case (TFloat(_, _), "toFloat32", Seq()) =>
        PVLBuiltinInstanceMethod[G](obj => _ => CastFloat(obj, PVL.float32))
      case (TFloat(_, _), "toFloat64", Seq()) =>
        PVLBuiltinInstanceMethod[G](obj => _ => CastFloat(obj, PVL.float64))
      case _ => return None
    })
  }

  def float32[G](implicit o: Origin = DiagnosticOrigin): TFloat[G] =
    TFloats.ieee754_32bit
  def float64[G](implicit o: Origin = DiagnosticOrigin): TFloat[G] =
