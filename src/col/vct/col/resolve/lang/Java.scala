...
case object Java extends LazyLogging {
  ...
  case class JavaSystemOrigin(preferredName: String) extends Origin {
    ...
    override def inlineContext: String =
      "[Class loaded from JRE with reflection]"
  }
  ...
  def translateRuntimeType[G](
    ...
    t match {
      ...
      case arr if arr.isArray =>
        TArray(translateRuntimeType(arr.getComponentType))
  ...
  def translateRuntimeParameter[G](
    ...
    new JavaParam(Seq(), param.getName, translateRuntimeType(param.getType))(
      SourceNameOrigin(param.getName, o)
    )
  }

  def translateRuntimeClass[G](cls: Class[_])(implicit
      o: Origin,
      ctx: TypeResolutionContext[G],
    ...
    val cons = cls.getConstructors.map(cons => {
      new JavaConstructor(
        ...
        parameters = cons.getParameters.toIndexedSeq
          .map(translateRuntimeParameter[G]),
        typeParameters = Nil,
        signals = Nil,
        body = Block(Nil),
        contract =
    ...
    val methods = cls.getMethods.map(method => {
      new JavaMethod(
        ...
        dims = 0,
        name = method.getName,
        parameters = method.getParameters.toIndexedSeq
          .map(translateRuntimeParameter[G]),
        typeParameters = Nil,
        signals = Nil,
        body = None,
    ...
    if (cls.isAnnotation) {
      new JavaAnnotationInterface[G](
        ...
        modifiers = Nil,
        ext =
          cls.getInterfaces.toIndexedSeq
            .map(cls => lazyType(cls.getName.split('.').toIndexedSeq, ctx))(0),
      ...
      new JavaInterface[G](
        ...
        modifiers = Nil,
        typeParams = Nil,
        ext = cls.getInterfaces.toIndexedSeq
          .map(cls => lazyType(cls.getName.split('.').toIndexedSeq, ctx)),
        decls = fields.toIndexedSeq ++ cons.toIndexedSeq ++ methods.toIndexedSeq,
      )(SourceNameOrigin(cls.getName.split('.').last, o))
    } else {
      new JavaClass[G](
        ...
        ext = Option(cls.getSuperclass)
          .map(cls => lazyType(cls.getName.split('.').toIndexedSeq, ctx))
          .getOrElse(JAVA_LANG_OBJECT),
        imp = cls.getInterfaces.toIndexedSeq
          .map(cls => lazyType(cls.getName.split('.').toIndexedSeq, ctx)),
        decls = fields.toIndexedSeq ++ cons.toIndexedSeq ++ methods.toIndexedSeq,
      )(SourceNameOrigin(cls.getName.split('.').last, o))
    }
                ...
                for {
                  ...
                  baseFile <-
                    ns.pkg.getOrElse(JavaName(Nil)).names
                      .foldRight[Option[File]](Option(file.getParentFile)) {
                        case (name, Some(file)) if file.getName == name =>
  ...
  def findConstructor[G](
    ...
    t match {
      case JavaTClass(Ref(cls), _) =>
        ...
        args match {
          case Nil =>
            definedConstructor.orElse(Some(ImplicitDefaultJavaConstructor()))
...
case object JavaAnnotationData {
  ...
  final case class BipPort[G](name: String, portType: BipPortType[G])(implicit
      val o: Origin
  ) extends JavaAnnotationData[G]
  ...
  final case class BipStatePredicate[G](name: String, expr: Expr[G])(implicit
      val o: Origin
  ) extends JavaAnnotationData[G]
