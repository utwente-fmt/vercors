...
case object Util {
  def compatJavaParams[G](
      args: Seq[Expr[G]],
      params: Seq[JavaParam[G]],
  ): Boolean = compatTypes(args, params.map(_.t))
  ...
  ): Boolean = compatTypes(typeEnv, args, params.map(_.t))
  ...
  def compatTypes[G](
    ...
    args.size == types.size && types.zip(args).forall { case (t, e) =>
      t.particularize(typeEnv).superTypeOf(e.t)
    }

  def compat[G](
      args: Seq[Expr[G]],
      typeArgs: Seq[Type[G]],
      genericInvokable: ContractApplicable[G],
  ): Boolean =
  ...
  def compat[G](
    ...
    compat(
      ...
      args,
      typeArgs,
      genericInvokable.args,
      genericInvokable.typeArgs,
    )
