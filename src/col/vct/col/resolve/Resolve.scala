  ...
  def enterContext[G](
    ...
    node match {
      ...
      case Scope(locals, body) =>
        ctx.copy(stack =
          ((locals ++ scanScope(body, /* inGPUkernel = */ false))
            .flatMap(Referrable.from)) +: ctx.stack
        )
  ...
  def resolveOne[G](node: Node[G], ctx: TypeResolutionContext[G]): Unit =
    node match {
      ...
      case cls: Class[G] =>
        ...
        cls.supports.foreach(
          _.tryResolve(name =>
            Spec.findClass(name, ctx)
              .getOrElse(throw NoSuchNameError("class", name, cls))
          )
        )
...
case object ResolveReferences extends LazyLogging {
  ...
  def resolve[G](
    ...
    val inGPU =
      inGPUKernel ||
        (node match {
          case f: CFunctionDefinition[G] =>
            f.specs.collectFirst { case _: CGpgpuKernelSpecifier[G] => () }
              .isDefined
          case _ => false
        })
      ...
      node match {
        case l @ Let(binding, value, main) =>
          val innerCtx = enterContext(node, ctx, inGPU)
            .copy(checkContext = l.enterCheckContext(ctx.checkContext))
  ...
  def scanScope[G](node: Node[G], inGPUKernel: Boolean): Seq[Declaration[G]] =
    node match {
      case _: Scope[G] => Nil
  ...
  def enterContext[G](
    ...
    (node match {
      ...
      case Scope(locals, body) =>
        ctx.declare(locals ++ scanScope(body, inGPUKernel))
  ...
  def resolveFlatly[G](
    ...
    node match {
      ...
      case inv @ PVLNew(t, args, givenMap, yields) =>
        inv.ref = Some(
          PVL.findConstructor(t, args).getOrElse(throw NoSuchConstructor(inv))
        )
        Spec.resolveGiven(givenMap, inv.ref.get, inv)
        Spec.resolveYields(ctx, yields, inv.ref.get, inv)
