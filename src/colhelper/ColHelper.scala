...
case class ColHelper() {
  ...
  def generate(
    ...
    info.checkSanity()
    println(
      s"[info] [ColHelper] Generating helpers for ${info.defs.size} node types"
    )

    // Construct the output path using the col ast package, as well as the name for the package declaration
    val packageOutput: Path = ("java" +: PACKAGE)
      .foldLeft(output.toPath)(_.resolve(_))
    packageOutput.toFile.mkdirs()
    val packageName =
      PACKAGE.tail.foldLeft[Term.Ref](Term.Name(PACKAGE.head))((t, n) =>
        Term.Select(t, Term.Name(n))
      )
    ...
    Seq[ColHelperMaker](
      ...
      println(
        s"[info] [ColHelper] Generating helpers in ${maker.getClass.getSimpleName}"
      )
      val helpers = maker.make()
      println(
        s"[info] [ColHelper] Writing out helpers from ${maker.getClass.getSimpleName}"
      )
      helpers.map { case (fileName, stats) =>
