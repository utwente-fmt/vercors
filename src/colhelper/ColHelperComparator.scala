...
case class ColHelperComparator(info: ColDescription) extends ColHelperMaker {
  def valueEqual(t: Type, left: Term, right: Term): Term =
    t match {
      case Type.Apply(Type.Name(name), List(Type.Name("G")))
          if info.supports("Node")(name) =>
        q"true"

      case Type.Apply(Type.Name("Ref"), _) => q"true"

      case Type.Name("Int") | Type.Name("BigInt") | Type.Name("BigDecimal") |
          Type.Name("String") | Type.Name("Boolean") | Type
            .Apply(Type.Name("Referrable"), List(Type.Name("G"))) | Type
            .Name("ExpectedError") | Type.Name("BitString") =>
        q"$left == $right"
      ...
      case Type.Apply(Type.Name("Set"), _) => q"$left == $right"
      ...
      case Type.Tuple(args) =>
        args.zipWithIndex.map { case (inner, idx) =>
          val access = Term.Name(s"_${idx + 1}")
          valueEqual(
            inner,
            Term.Select(left, access),
            Term.Select(right, access),
          )
        }.filter(_ != q"true").reduceOption((l, r) => q"$l && $r")
          .getOrElse(q"true")
    }

  def refEqual(t: Type, left: Term, right: Term): Term =
    t match {
      case Type.Apply(Type.Name("Ref"), _) =>
        q"LazyList(Comparator.MatchingReference($left.decl, $right.decl))"
  ...
  def nodeEqual(t: Type, left: Term, right: Term): Term =
    t match {
      case Type.Apply(Type.Name(name), List(Type.Name("G")))
          if info.supports("Node")(name) =>
        q"Comparator.compare($left, $right)"
      ...
      case Type.Tuple(args) =>
        ...
        }.filter(_ != q"LazyList.empty").reduceOption((l, r) => q"$l #::: $r")
          .getOrElse(q"LazyList.empty")
    }
  ...
  def recurse(term: Term): Term =
    term match {
      ...
      case q"(if($cond) $whenTrue else $whenFalse).to(LazyList)" =>
        simplify(
          q"if($cond) $whenTrue.to(LazyList) else $whenFalse.to(LazyList)"
        )
  ...
  def simplifyFlatly(term: Term): Term =
    simplifyFlatly(term match {
      ...
      case q"$zipped.flatMap({ case ($l, $r) => LazyList.empty })" =>
        q"LazyList.empty"
      case q"$zipped.forall({ case ($l, $r) => true })" => q"true"

      case q"if($cond) $whenTrue else $whenFalse"
          if whenTrue.show[Structure] == whenFalse.show[Structure] =>
        whenTrue

      case q"$l1.isEmpty && $r1.isEmpty || $l2.nonEmpty && $r2.nonEmpty"
          if l1.show[Structure] == l2.show[Structure] &&
            r1.show[Structure] == r2.show[Structure] =>
        q"$l1.isEmpty == $r1.isEmpty"

      case other => return other
    })

  def simplify(term: Term): Term = simplifyFlatly(recurse(term))
  ...
  def makeCase(defn: ClassDef): (String, List[Stat]) = {
    ...
    val equal = defn.params.map(paramEquals)
      .reduceOption[Term]((l, r) => q"$l && $r").getOrElse(q"true")
    val refCmp = defn.params.map(paramRefCmp)
      .reduceOption[Term]((l, r) => q"$l #::: $r").getOrElse(q"LazyList.empty")
    val subnodesCmp = defn.params.map(paramNodeCmp)
      .reduceOption[Term]((l, r) => q"$l #::: $r").getOrElse(q"LazyList.empty")
    ...
    val apply =
      q"""def apply[L, R](anyLeft: Node[L], anyRight: Node[R]): LazyList[Comparator.Difference[L, R]] = {
    ...
    objectName -> List(Defn.Object(
      Nil,
      Term.Name(objectName),
      Template(
        Nil,
        List(Init(t"Comparator.Compare", Name.Anonymous(), Nil)),
  ...
  def makeComparator(): (String, List[Stat]) =
    "Comparator" -> List(q"""
    object Comparator {
      ...
      val compareLookupTable: Map[java.lang.Class[_], Compare] =
        Map(..${info.defs.map(defn => q"classOf[${defn.typ}[_]] -> ${Term.Name("Compare" + defn.baseName)}").toList})
