...
case class ColHelperJavaRewriter(info: ColDescription) extends ColHelperMaker {
  def makeMapEntry(family: String)(cls: ClassDef): Term =
    q"""classOf[${cls.typ}[_]] -> new RWFunc[${Type.Name(family)}] {
      def apply[Pre, Post](node: ${Type
        .Name(family)}[Pre], rw: JavaRewriter[Pre, Post]): ${Type.Name(family)}[Post] =
  ...
  def javaRewrite(ret: Type.Name)(cls: ClassDef): Stat =
    q"""
  ...
  def javaRewriteDecl(cls: ClassDef): Stat =
    q"""
