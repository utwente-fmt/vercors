...
/** object Namespace { case class Thing(param1: Int)(val blame: Blame)(implicit
  * val o: Origin) extends Node }
  *
  * becomes: ClassDef(Seq("Namespace", "Thing"), List(param1: Int), Some(Blame),
  * List(case))
  */
case class ClassDef(
    names: Seq[String],
    params: List[Term.Param],
    blameType: Option[Type],
    mods: List[Mod],
) {
  for (param <- params) {
  ...
  def term: Term =
    if (names.size == 1)
      ...
      Term.Select(termQual, Term.Name(baseName))

  def typ: Type =
    if (names.size == 1)
      Type.Name(baseName)
    else
      Type.Select(termQual, Type.Name(baseName))

  def rewriteHelperName: Type.Name = Type.Name("Rewrite" + baseName)

  def rewriteBuilderName: Type.Name = Type.Name(baseName + "Builder")
...
class ColDescription {
    ...
    for (defn <- defs) {
      if (supports("Declaration")(defn.baseName)) {
        DECLARATION_KINDS.filter(supports(_)(defn.baseName)) match {
          ...
          case x :: y :: _ =>
            ColHelperUtil.fail(
              s"Definition ${defn.baseName} supports multiple declaration kinds, such as $x and $y. Only one declaration kind is allowed."
            )
        }
        ...
        families.toSeq.filter(supports(_)(defn.baseName)) match {
          case x :: y :: _ =>
            ColHelperUtil.fail(
              s"Definition ${defn.baseName} supports multiple node families, such as $x and $y. Only one node family is allowed."
            )
          case _ =>
        }
      } else {
        ColHelperUtil.fail(
          s"Definition ${defn.baseName} does not support Declaration or NodeFamily.\n" +
  ...
  /** Provides the default way to rewrite a term with a fixed type
    */
  def rewriteDefault(term: Term, typ: Type): Term =
    typ match {
      case Type.Apply(
            Type.Name("Seq"),
            List(Type.Apply(Type.Name(declKind), List(Type.Name("G")))),
          ) if DECLARATION_KINDS.contains(declKind) =>
        q"rewriter.${ColDefs.scopes(declKind)}.dispatch($term)"
      case Type.Apply(Type.Name(collectionType), List(arg))
          if Set("Seq", "Set", "Option").contains(collectionType) =>
      ...
      case Type.Tuple(other) =>
        ColHelperUtil.fail(
          s"Oops, this tuple is too long for me! size=${other.size}",
          node = Some(typ),
        )

      case Type.Apply(Type.Name(declKind), List(Type.Name("G")))
          if DECLARATION_KINDS.contains(declKind) =>
        q"rewriter.${ColDefs.scopes(declKind)}.dispatch($term)"
      case Type.Apply(Type.Name(typ), List(Type.Name("G")))
          if families.contains(typ) =>
        q"rewriter.dispatch($term)"

      case Type.Apply(Type.Name(typeName), List(Type.Name("G")))
          if supports("Node")(typeName) =>
        ColHelperUtil.fail(
      ...
      case Type.Apply(
            Type.Name("Ref"),
            List(_, Type.Apply(decl @ Type.Name(tDecl), _)),
          ) =>
        if (ColDefs.DECLARATION_KINDS.exists(kind => supports(kind)(tDecl)))
          q"rewriter.porcelainRefSucc[$decl[Post]]($term).getOrElse(rewriter.succ[${Type
              .Name(tDecl)}[Post]]($term.decl))"
        else
          q"rewriter.porcelainRefSucc[$decl[Post]]($term).getOrElse(rewriter.anySucc[${Type
              .Name(tDecl)}[Post]]($term.decl))"
      case Type.Name("Int") | Type.Name("String") | Type.Name("Boolean") | Type
            .Name("BigInt") | Type.Name("BigDecimal") | Type.Name("BitString") |
          Type.Apply(Type.Name("Referrable"), List(Type.Name("G"))) | Type
            .Name("ExpectedError") =>
        term
  ...
  /** Collect the shape of a node. Nodes are shaped either like: (case)
    * class(param: T, ...)(implicit o: Origin) or: (case) class(param: T,
    * ...)(val blame: BT)(implicit o: Origin) We need to remember the modifier
    * ("case") to know how to construct the node. Some nodes are within an
    * object, so we remember the chain in the path, e.g.: object Constants {
    * class Test()(implicit val o: Origin) } We would need the path
    * Seq("Constants").
    */
  def collectNode(path: Seq[String])(stat: Stat): Unit =
    stat match {
      case Defn.Class(
            mods,
            name,
            List(Type.Param(_, Type.Name("G"), _, _, _, _)),
            Ctor.Primary(_, _, parameterLists),
            _,
          )
          if mods.collectFirst { case Mod.Abstract() => () }.isEmpty &&
            Set(2, 3).contains(parameterLists.size) =>
        val originList =
        ...
        originList match {
          case List(
                Term.Param(
                  List(scala.meta.Mod.Implicit(), scala.meta.Mod.ValParam()),
                  Name("o"),
                  Some(scala.meta.Type.Name("Origin")),
                  _,
                )
              ) =>
            if (parameterLists.size == 2) {
              ...
              parameterLists(1) match {
                ...
                case _ =>
                  ColHelperUtil.fail(
                    "The second (of three) parameter lists must only contain the blame parameter",
                    node = Some(stat),
                  )
              }
            } else {
              ColHelperUtil.fail(
                "Class must have two or three parameter lists",
                node = Some(stat),
              )
            }
          case _ =>
        }
      case otherCls: Defn.Class if otherCls.mods.collectFirst {
            case Mod.Abstract() => ()
          }.isEmpty =>
        ColHelperUtil.fail(
          "Could not parse the following class. Is the class in the right format?",
          node = Some(otherCls),
        )
  ...
  /** Collect all the base classes and traits of every definition. They are
    * remembered by base name only, and hence nodes may not have overlapping
    * names.
    */
  def collectBases(stat: Stat): Unit =
    stat match {
  ...
  /** Collect node families: the root of a rewriting class. Nodes in a family
    * always rewrite to a type in that family.
    */
  def collectFamily(stat: Stat): Unit =
    stat match {
      case Defn.Class(_, name, _, _, Template(_, inits, _, _)) =>
        if (
          inits.collectFirst {
            case Init(
                  Type.Apply(Type.Name("NodeFamily"), List(Type.Name("G"))),
                  _,
                  _,
                ) =>
              ()
