...
case class ColHelperSerialize(info: ColDescription, proto: ColProto)
    extends ColHelperMaker {
  def serType(name: String): Type = t"ser.${Type.Name(proto.Name(name).ucamel)}"

  def makeFamilyDispatch(family: String): List[Stat] =
    List(q"""
  ...
  def makeFamilyDispatchLut(family: String, decl: Boolean): List[Stat] =
    if (info.defs.exists(_.baseName == family))
      ...
      List(q"""
        val ${Pat.Var(Term.Name(family + "Lut"))}: Map[java.lang.Class[_], (Serialize, ${t"${Type
            .Name(family)}[_]"}) => ${serType(family)}] = Map(..${info.defs
          .filter(d => info.supports(family)(d.baseName)).map { defn =>
            val vArg =
              q"""ser.${Term.Name(proto.Name(family).ucamel)}.V.${Term.Name(proto.Name(defn.baseName).ucamel)}(
  ...
  def serializeTerm(term: Term, typ: proto.Typ): Term =
    proto.primitivize(typ) match {
      ...
      case proto.TBigInt =>
        q"ser.BigInt(com.google.protobuf.ByteString.copyFrom($term.toByteArray))"
      case proto.TBigDecimal =>
        q"ser.BigDecimal($term.scale, ser.BigInt(com.google.protobuf.ByteString.copyFrom($term.underlying().unscaledValue().toByteArray())))"
      case proto.TString => term
      case proto.TBitString =>
        q"ser.BitString(com.google.protobuf.ByteString.copyFrom($term.rawData), $term.skipAtLastByte)"
      ...
      case typ @ proto.TName(name) if proto.boxedTypeForward.contains(typ) =>
        q"ser.${Term.Name(name)}(${serializeTerm(term, proto.boxedTypeForward(typ))})"
      case typ @ proto.TName(name) if proto.boxedTypeTuple.contains(typ) =>
        q"""ser.${Term.Name(name)}(..${val ts = proto.boxedTypeTuple(typ).ts
          ts.zipWithIndex.map { case (typ, i) =>
            serializeTerm(Term.Select(term, Term.Name(s"_${i + 1}")), typ)
          }.toList })"""
  ...
  def serializeParam(defn: ClassDef)(param: Term.Param): Term =
    serializeTerm(
      q"node.${Term.Name(param.name.value)}",
      proto.getType(param.decltpe.get),
    )

  def makeNodeSerialize(defn: ClassDef): List[Stat] =
    List(q"""
    def ${Term.Name("serialize" + defn.baseName)}(node: ${defn.typ}[_]): ${serType(defn.baseName)} =
      ser.${Term.Name(proto.Name(defn.baseName).ucamel)}(..${val nodeParams =
        defn.params.map(serializeParam(defn))
      val idParams =
  ...
  def make(): List[(String, List[Stat])] = List(("Serialize", makeSerialize()))
}
