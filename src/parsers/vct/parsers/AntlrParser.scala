...
abstract class AntlrParser extends Parser {
  ...
  def getExpectedErrors(
    ...
    for (
      token <- lexer.getTokens.asScala
        .filter(_.getChannel == expectedErrorChannel)
    ) {
      ...
      if (token.getType == expectedErrorEnd) {
        ...
        val err = ExpectedError(
          code,
          OriginProvider(baseOrigin, start, token),
  ...
  protected def getErrorsFor[T](
      ...
      lexer: runtime.Lexer,
  )(f: => T): Either[Seq[ParseError], T] = Right(f)
  ...
  def parseAntlrStreamOrThrow[Ctx, G, Out](
      stream: runtime.CharStream,
      baseOrigin: Origin = Origin(Nil),
  )(
      parseCtx: Parser => Ctx,
      convert: (Converter[G], Ctx) => Out,
  ): (Out, Seq[ExpectedError]) = {
  ...
  def parseReaderOrThrow[Ctx, G, Out](
    ...
    parseAntlrStreamOrThrow(CharStreams.fromReader(reader), baseOrigin)(
      parseCtx,
      convert,
    )

  def parseOrThrow[Ctx, G, Out](
      readable: Readable,
      baseOrigin: Origin = Origin(Nil),
  )(
      parseCtx: Parser => Ctx,
    ...
    try {
      readable.read { reader =>
        parseReaderOrThrow(
          reader,
          baseOrigin.withContent(ReadableOrigin(readable)),
        )(parseCtx, convert)
      }
    } catch {
  ...
  def parseAntlrStream[G](
      stream: runtime.CharStream,
      baseOrigin: Origin = Origin(Nil),
  ): ParseResult[G] = {
    val (result, expectedErrors) =
      parseAntlrStreamOrThrow(stream, baseOrigin)(
        parser => parseMain(parser),
        mainToResult[G],
      )
