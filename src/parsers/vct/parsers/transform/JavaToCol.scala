...
case class JavaToCol[G](
  ...
  def convert(implicit
    ...
    decl match {
      ...
      case TypeDeclaration1(
        ...
        Seq(new Enum(convert(constants))(
  ...
  def convert(implicit constant: EnumConstantContext): EnumConstant[G] =
    constant match {
      case EnumConstant0(Nil, name, None, None) =>
        new EnumConstant()(SourceNameOrigin(convert(name), origin(constant)))
      case constant =>
        fail(
          constant,
  ...
  def convert(implicit
      decl: AnnotationTypeElementDeclarationContext
  ...
  def convert(implicit
      modifier: ClassOrInterfaceModifierContext
  ): JavaModifier[G] =
    modifier match {
  ...
  def convert(implicit
      annotationArgs: AnnotationArgsContext
  ): Seq[(String, Expr[G])] =
  ...
  def convert(implicit
      pair: ElementValuePairContext
  ): Option[(String, Expr[G])] =
    pair match {
      case ElementValuePair0(name, _, ElementValue0(expr)) =>
        logger.warn(
          s"Annotation array initializer at ${originProvider(pair).shortPosition} is discarded"
        )
        None
        ...
        bound match {
          case None =>
            new Variable(TType(Java.JAVA_LANG_OBJECT))(
              SourceNameOrigin(convert(id), origin(arg))
            )
  ...
  def convert(implicit
      decl: InterfaceBodyDeclarationContext
  ): Seq[ClassDeclaration[G]] =
    decl match {
  ...
  def convert(implicit
      decl: InterfaceMemberDeclarationContext,
      mods: Seq[JavaModifier[G]],
      c: ContractCollector[G],
  ): Seq[ClassDeclaration[G]] =
    decl match {
  ...
  def convert(implicit
      decls: VariableDeclaratorsContext
  ...
  def convert(implicit
      decl: VariableDeclaratorContext
  ...
  def convert(implicit
      decls: ConstantDeclaratorListContext
  ): Seq[JavaVariableDeclaration[G]] =
    decls match {
  ...
  def convert(implicit
      decl: ConstantDeclaratorContext
  ): JavaVariableDeclaration[G] =
    decl match {
  ...
  def convert(implicit
      params: InitFormalParameterListContext
  ): Seq[JavaParam[G]] =
    params match {
    ...
    param match {
      case FormalParameter0(modifiers, tNode, nameDims) =>
        ...
        new JavaParam(modifiers.map(convert(_)), name, t)(
          SourceNameOrigin(name, origin(param))
        )
    }
  ...
  def convert(implicit
      decl: LocalVariableDeclarationStatementContext
  ): Statement[G] =
    decl match {
  ...
  def convert(implicit label: LoopLabelContext): LabelDecl[G] =
    label match {
      case LoopLabel0(name, _) =>
        new LabelDecl()(SourceNameOrigin(convert(name), origin(label)))
    }

  def convert(implicit
      switchBlock: SwitchBlockStatementGroupContext
  ): Seq[Statement[G]] =
    switchBlock match {
  ...
  def convert(implicit grab: CatchClauseContext): CatchClause[G] =
    grab match {
      case CatchClause0(_, _, _mods, ts, id, _, body) =>
        CatchClause(
          new Variable(convert(ts))(
            SourceNameOrigin(convert(id), origin(grab))
          ),
          convert(body),
        )
    }
  ...
  def convertBipGlueElement(implicit
      s: ParserRuleContext
  ...
  def convert(implicit
      invocation: ExplicitGenericInvocationSuffixContext,
      obj: Option[Expr[G]],
      typeArgs: Seq[Type[G]],
  ): Expr[G] =
    invocation match {
  ...
  def convert(implicit
      creator: ClassCreatorRestContext,
      ts: Seq[Type[G]],
      name: Type[G],
      givenArgs: Seq[(Ref[G, Variable[G]], Expr[G])],
      yields: Seq[(Expr[G], Ref[G, Variable[G]])],
  ...
  def convert(implicit
      creator: ArrayCreatorRestContext,
      name: Type[G],
  ): Expr[G] =
    creator match {
