...
case class LLVMContractToCol[G](
  ...
  def convert(
      implicit contract: ValEmbedContractContext
  ...
  def convert(implicit id: LangIdContext): String =
    id match { case LangId0(text) => text }
  ...
  def convert(implicit expr: LangExprContext): Expr[G] =
  ...
  def convert(implicit inst: InstructionContext): Expr[G] =
    inst match {
      ...
      case bitOp =>
        left.t match {
          ...
          case other =>
            throw ParseError(
              left.o,
              s"Expected an integer or boolean expression here, but got `$other`",
            )
        }
    }
  }
  ...
  def convert(implicit cmpOp: CompareInstructionContext): Expr[G] =
    cmpOp match {
      case CompareInstruction0(_, _, pred, _, lhs, _, rhs, _) =>
        convert(pred, lhs, rhs)
    }

  def convert(
      implicit pred: CompPredContext,
      lhs: ExpressionContext,
      rhs: ExpressionContext,
  ): Expr[G] = {
  ...
  def convert(implicit const: ConstantContext): Expr[G] = {
    const match {
      case BoolConstRule(const) =>
        BooleanValue(const match {
          case ConstTrue(_) => true
          case ConstFalse(_) => false
        })(origin(const))
      case IntConstRule(IntegerConstant0(constInt)) =>
        IntegerValue(BigInt(constInt))(origin(const))
    }
  }

  def convert(implicit id: IdentifierContext): Expr[G] =
    id match { case Identifier0(text) => local(id, text) }
  ...
  def convert(implicit arg: ValArgContext): Variable[G] =
    arg match { case ValArg0(t, id) => createVariable(arg, id, t) }
  ...
  def convert(implicit exps: ValExpressionListContext): Seq[Expr[G]] =
    exps match {
