...
abstract class OriginProvider extends PositionContextProvider[Origin] {
  ...
  def setTokenStream(tokenStream: CommonTokenStream): Unit =
    _tokenStream = Some(tokenStream)
...
case class RedirectOriginProvider(o: Origin, textualOrigin: String)
    extends OriginProvider {
  override def apply(
      startLineIdx: Int,
      endLineIdx: Int,
      cols: Option[(Int, Int)],
...
case class InterpretedFileOriginProvider(
    original: OriginProvider,
    interpreted: Readable,
) extends OriginProvider {
  private def getLineOffset(lineIdx: Int): Option[Int] = {
    val firstTokenAtOrPastLine = (0 until tokenStream.size())
      .find(i => tokenStream.get(i).getLine - 1 >= lineIdx)
    ...
    (getLineOffset(startLineIdx), getLineOffset(endLineIdx)) match {
      case (Some(startOffset), Some(endOffset)) =>
        InterpretedOrigin(
          ...
          endLineIdx,
          cols,
          original(startLineIdx + startOffset, endLineIdx + endOffset, None),
        )
      case _ => ReadableOrigin(interpreted, startLineIdx, endLineIdx, cols)
    }
  }
