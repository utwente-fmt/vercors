...
case class PVLToCol[G](
    ...
    enum match {
      case EnumDecl0(_, name, _, constants, _, _) =>
        new vct.col.ast.Enum[G](constants.map(convertConstants(_)).getOrElse(
          Nil
        ))(SourceNameOrigin(convert(name), origin(enum)))
    }

  def convertConstants(implicit
      identifierList: IdentifierListContext
  ): Seq[EnumConstant[G]] =
    identifierList match {
      case IdentifierList0(id) =>
        Seq(new vct.col.ast.EnumConstant[G]()(SourceNameOrigin(
          convert(id),
          origin(identifierList),
        )))
      case IdentifierList1(id, _, tail) =>
        new vct.col.ast.EnumConstant[G]()(SourceNameOrigin(
          convert(id),
          origin(identifierList),
        )) +: convertConstants(tail)
    }

  def convert(implicit decl: SeqProgDeclContext): Declaration[G] =
    decl match {
      case SeqProgMethod(methods) => convert(methods)
      case SeqProgRunMethod(runMethod) => convert(runMethod).head
      case SeqProgThread(_, threadId, _, threadType, _, args, _, _) =>
        new VeyMontThread(
          convert(threadType),
          args.map(convert(_)).getOrElse(Nil),
        )(SourceNameOrigin(convert(threadId), origin(decl)))
    }

  def convertVeyMontProg(implicit
      cls: DeclVeyMontSeqProgContext
  ): VeyMontSeqProg[G] =
    cls match {
      case DeclVeyMontSeqProg0(contract, _, name, _, args, _, _, decls, _) =>
        ...
        val runMethod = declseq.collectFirst { case x: RunMethod[G] => x }
          .getOrElse(
            throw new RuntimeException(
              "A seq_prog needs to have a run method, but none was found!"
            )
          )
        val methods = declseq.collect { case m: InstanceMethod[G] => m }
        val threads = declseq.collect { case v: VeyMontThread[G] => v }
        withContract(
          contract,
          contract => {
            new VeyMontSeqProg(
            ...
            withContract(
              ...
              contract => {
                new Procedure(
                  ...
                  SourceNameOrigin(convert(name), origin(method))
                )
              },
            ),
        )
    ...
    field match {
      case Field0(finalFlag, t, ids, _) =>
        convert(ids).map(name =>
          new InstanceField[G](convert(t), finalFlag.map(convert(_)).toSet)(
            SourceNameOrigin(name, origin(field))
          )
        )
    }
  ...
  def convert(implicit args: ArgsContext): Seq[Variable[G]] =
    args match {
      case Args0(t, name) =>
        Seq(new Variable(convert(t))(
          SourceNameOrigin(convert(name), origin(name))
        ))
  ...
  def convert(implicit expr: MultExprContext): Expr[G] =
    expr match {
      ...
      case MultExpr2(left, _, right) =>
        Mod(convert(left), convert(right))(blame(expr))
  ...
  def convert(implicit expr: NewExprContext): Expr[G] =
    expr match {
      ...
      case NewExpr1(_, t, dims) =>
        NewArray(convert(t), convert(dims), moreDims = 0)(blame(expr))
  ...
  def convert(implicit expr: UnitContext): Expr[G] =
    expr match {
      ...
      case Unit5(s"${n}f") /* take off final "f" */ =>
  ...
  def convert(implicit stat: StatementContext): Statement[G] =
    stat match {
      ...
      case PvlIf(_, _, cond, _, body, Some(ElseBlock0(_, otherwise))) =>
        Branch(Seq((convert(cond), convert(body)), (tt, convert(otherwise))))
          ...
          contract => {
            val decl =
              name match {
                ...
                case Some(name) =>
                  new ParBlockDecl[G]()(
                    SourceNameOrigin(convert(name), origin(region))
                  )
              }
            ParBlock(
              decl,
  ...
  def convert(implicit decls: DeclListContext, t: Type[G]): Seq[Statement[G]] =
    decls match {
      case DeclList0(name, None) =>
        Seq(LocalDecl(
          new Variable(t)(SourceNameOrigin(convert(name), origin(name)))
        ))
      ...
      case DeclList1(name, Some(DeclInit0(_, init)), _, more) =>
        val v =
          new Variable[G](t)(SourceNameOrigin(convert(name), origin(name)))
        Seq(
          LocalDecl[G](v),
  ...
  def convert(implicit
    ...
    decl match {
      ...
      case ValProverType(_, name, ints, _) =>
        Seq(new ProverType(convert(ints))(
          SourceNameOrigin(convert(name), origin(decl))
        ))
  ...
  def convert(implicit
      int: ValProverInterpretationsContext
  ...
  def convert(implicit
      int: ValProverInterpretationContext
