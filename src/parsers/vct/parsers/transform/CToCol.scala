...
case class CToCol[G](
  ...
  def convert(implicit
      externalDecl: ExternalDeclarationContext
  ...
  def convert(implicit
      declSpec: DeclarationSpecifierContext
  ...
  def convert(implicit
      storageClass: StorageClassSpecifierContext
  ...
  def convert(implicit
      kernel: GpgpuKernelSpecifierContext
  ...
  def convert(implicit stat: LabeledStatementContext): Statement[G] =
    stat match {
      case LabeledStatement0(label, _, inner) =>
        Label(
          new LabelDecl()(
            SourceNameOrigin(convert(label), originProvider(stat))
          ),
          convert(inner),
        )
  ...
  def convert(implicit
      specifiers: SpecifierQualifierListContext
  ): Seq[CDeclarationSpecifier[G]] =
    specifiers match {
  ...
  def convert(implicit expr: PostfixExpressionContext): Expr[G] =
    expr match {
      ...
      case PostfixExpression3(struct, _, field) =>
        CStructAccess(convert(struct), convert(field))(blame(expr))
      case PostfixExpression4(struct, _, field) =>
        CStructDeref(convert(struct), convert(field))
