...
object ParseAllExamples {
  def track[T <: Parser](read: hre.io.Readable, recognizer: T): T = {
    ...
    recognizer.addErrorListener(CollectingErrorListener(
      Origin(Seq(ReadableOrigin(read))),
      DebugOptions.ALL,
    ))
    recognizer
  }
    ...
    f.read { reader =>
      ext match {
        case "pvl" =>
          track(
            f,
            new PVLParser(new CommonTokenStream(new LangPVLLexer(
              CharStreams.fromReader(reader)
            ))),
          ).program()
        case "java" =>
          track(
            f,
            new JavaParser(new CommonTokenStream(new LangJavaLexer(
              CharStreams.fromReader(reader)
            ))),
          ).compilationUnit()
        case "c" | "cu" | "cl" | "h" =>
          ...
          RWFile(tmp).read { reader =>
            track(
              RWFile(tmp),
              new CParser(new CommonTokenStream(new LangCLexer(
                CharStreams.fromReader(reader)
              ))),
            ).compilationUnit()
          }
        case "cpp" =>
          ...
          RWFile(tmp).read { reader =>
            track(
              RWFile(tmp),
              new CPPParser(new CommonTokenStream(new LangCPPLexer(
                CharStreams.fromReader(reader)
              ))),
            ).translationUnit()
          }
  ...
  val ALLOWED_EXTS = Set("pvl", "java", "c", "cu", "cl", "h", "cpp")
  ...
  /** Parse all files in a given directory, or just one file. Argument 1: file
    * or directory to parse files from (optional, default: examples) Argument 2:
    * set of extensions to filter for (optional, default:
    * pvl,java,c,cu,cl,h,cpp)
    */
  def main(args: Array[String]): Unit = {
    ...
    val files = paths.collect {
      case path if getExt(path).nonEmpty && exts.contains(getExt(path).get) =>
        getExt(path).get -> path
    }
