...
case class ColJavaParser(debugOptions: DebugOptions, blameProvider: BlameProvider, specCommentsNeeded: Boolean = true) extends AntlrParser {
  ...
  override def newLexer(charStream: CharStream): Lexer = new LangJavaLexer(charStream)
  ...
  override def newConverter[G](baseOrigin: Origin, expectedErrors: Seq[(Token, Token, ExpectedError)], tokenStream: TokenStream): Converter[G] =
    JavaToCol[G](baseOrigin, blameProvider, expectedErrors)

  override def mainToResult[G](converter: Converter[G], main: JavaParser.CompilationUnitContext): Seq[GlobalDeclaration[_]] =
    converter.convert(main)

  def parseExpr[G](readable: Readable): (Expr[G], Seq[ExpectedError]) =
    parseOrThrow[JavaParser.ExprContext, G, Expr[G]](readable)(p => p.expr(), (cv: Converter[G], e) => cv.convert(e).asInstanceOf[Expr[G]])
}
