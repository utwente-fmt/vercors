...
case class ColLLVMParser(debugOptions: DebugOptions, blameProvider: BlameProvider, vcllvm: Path) extends Parser with LazyLogging {
  ...
  override def parseReader[G](reader: Reader, baseOrigin: Origin): ParseResult[G] = throw new UnsupportedOperationException()
}
