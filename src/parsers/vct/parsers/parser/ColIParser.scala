...
case class ColIParser(debugOptions: DebugOptions, blameProvider: BlameProvider, cOrigin: Option[Origin]) extends AntlrParser {
  ...
  override def newLexer(charStream: CharStream): Lexer = new LangCLexer(charStream)
  ...
  override def newParser(tokenStream: TokenStream): Parser = new CParser(tokenStream)
  ...
  override def newConverter[G](baseOrigin: Origin, expectedErrors: Seq[(Token, Token, ExpectedError)], tokenStream: TokenStream): Converter[G] =
    CToCol(baseOrigin, blameProvider, expectedErrors, cOrigin.map(o => (tokenStream, o)))
