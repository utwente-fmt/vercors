...
case class ColLLVMContractParser(
  ...
  override def newLexer(charStream: CharStream): Lexer =
    new LangLLVMSpecLexer(charStream)
  override def expectedErrorChannel: Int =
    LangLLVMSpecLexer.EXPECTED_ERROR_CHANNEL
  ...
  override def newParser(tokenStream: TokenStream): Parser =
    new LLVMSpecParser(tokenStream)
  override def parseMain(parser: Parser): MainContext =
    parser.valGlobalDeclaration()
  ...
  override def mainToResult[G](
      ...
      main: MainContext,
  ): Seq[GlobalDeclaration[_]] = Seq(converter.convert(main))

  def parseFunctionContract[G](
      reader: Reader,
      baseOrigin: Origin = Origin(Nil),
  ): (ApplicableContract[G], Seq[ExpectedError]) =
    parseReaderOrThrow[
      LLVMSpecParser.ValEmbedContractContext,
      G,
      ApplicableContract[G],
    ](reader, baseOrigin)(
      _.valEmbedContract(),
      _.convert(_).asInstanceOf[ApplicableContract[G]],
    )
}
