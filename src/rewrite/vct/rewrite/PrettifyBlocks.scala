...
case class PrettifyBlocks[Pre <: Generation]() extends Rewriter[Pre] {
  ...
  def dispatchFlatly(stat: Statement[Pre]): Seq[Statement[Post]] =
    stat match {
    ...
    stat match {
      ...
      case Scope(locals, body) =>
        if (variables.nonEmpty) {
          locals.foreach(dispatch)
          dispatch(body)
        } else { collectVariables(body, extra = locals) }
      ...
      case act: ModelDo[Pre] => act.rewrite(impl = collectVariables(act.impl))
  ...
  override def dispatch(parRegion: ParRegion[Pre]): ParRegion[Rewritten[Pre]] =
    parRegion match {
