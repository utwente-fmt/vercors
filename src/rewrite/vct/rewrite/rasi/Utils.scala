...
case object Utils {
   ...
   * @return <code>max{|a|, |b|}</code>
   */
  def abs_max(a: Int, b: Int): Int = Seq(-a, a, -b, b).max

  /**
   * Maximum product between either <code>a1</code> or <code>a2</code> and either <code>b1</code> or <code>b2</code>.
   *
   * @param a1 Alternative group 1
   * @param a2 Alternative group 1
   * @param b1 Alternative group 2
   * @param b2 Alternative group 2
   * @return <code>max{a1*b1, a2*b1, a2*b1, a2*b2}</code>
   */
  def prod_max(a1: Int, a2: Int, b1: Int, b2: Int): Int = Seq(a1 * b1, a1 * b2, a2 * b1, a2 * b2).max

  /**
   * Minimum product between either <code>a1</code> or <code>a2</code> and either <code>b1</code> or <code>b2</code>.
   *
   * @param a1 Alternative group 1
   * @param a2 Alternative group 1
   * @param b1 Alternative group 2
   * @param b2 Alternative group 2
   * @return <code>min{a1*b1, a2*b1, a2*b1, a2*b2}</code>
   */
   ...
   *
   * @param contract Loop contract
   * @return Loop invariant represented by <code>contract</code>
   */
  ...
  /**
   * Transforms an accounted predicate from a subroutine contract into a boolean/resource expression.
   *
   * @param contract Subroutine contract
   * @return Boolean expression representing <code>contract</code>
   */
  ...
  /**
   * Substitutes free variables in a boolean expression for other given expressions according to the given map.
   *
   * @param cond Boolean condition, potentially containing free variables
   * @param args A map from free variables in <code>cond</code> to expressions to be substituted
   * @return The given condition with free variables replaced
   */
   ...
   * @return <code>true</code> if <code>node</code> or one of its subnodes refers to the global invariant,
  ...
  /**
   * Transforms a valuation on resolvable variables to a valuation on concrete variables.
   *
   * @param m Valuation on resolvable variables
   * @return Same valuation with all non-trackable variables removed
   */
   ...
   * @param v1 Valuation
   * @param v2 Valuation
   * @return The concatenation of <code>v1</code> and <code>v2</code>, with elements contained in both valuations
   *         mapped to the intersection of both
   */
   ...
   *
   * @param states States of the graph, each representing an abstract state
   * @param edges Edges of the graph
   * @param out Path to the output file to write to
   */
  def print[G](states: Seq[AbstractState[G]], edges: Seq[(AbstractState[G], AbstractState[G])], out: Path): Unit = {
    ...
    RWFile(out).write(w => print_state_space(node_names, edges, w, states.head.to_expression.toInlineString.length > 100))
  }
