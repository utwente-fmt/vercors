...
class ConstraintSolver[G](state: AbstractState[G], vars: Set[_ <: ResolvableVariable[G]], is_contract: Boolean) {
  ...
  private def handle_collection_update(comp: Comparison[G], pure_left: Boolean, negate: Boolean): Set[ConstraintMap[G]] = {
    ...
    val indices: Set[(IndexedVariable[G], Int)] = get_contained_indices(variable)
    val size_vars: Set[SizeVariable[G]] = get_size_vars(variable)

    val index_update_map: Set[(ResolvableVariable[G], UncertainValue)] = indices.map(t => (t._1, value.get(t._2)))
    val size_update_map: Set[(ResolvableVariable[G], UncertainValue)] = size_vars.map(v => (v, value.len))
  ...
  private def get_contained_indices(variable: Expr[G]): Set[(IndexedVariable[G], Int)] = variable match {
    ...
    case Drop(xs, count) => state.resolve_integer_expression(count).try_to_resolve() match {
      case None => ???
      case Some(i) => get_contained_indices(xs).filter(t => t._2 >= i).map(t => (t._1, t._2 - i))
    }
    case Slice(xs, from, to) => state.resolve_integer_expression(from).try_to_resolve() match {
      ...
      case Some(l) => state.resolve_integer_expression(to).try_to_resolve() match {
        case None => ???
        case Some(u) => get_contained_indices(xs).filter(t => l <= t._2 && t._2 < u).map(t => (t._1, t._2 - l))
      }
    }
    case Concat(left, right) => state.resolve_collection_expression(left).len.try_to_resolve() match {
      case None => ???
      case Some(i) => get_contained_indices(left) ++ get_contained_indices(right).map(t => (t._1, t._2 + i))
    }
  ...
  private def get_size_vars(variable: Expr[G]): Set[SizeVariable[G]] =
    vars.filter(v => v.is_contained_by(variable, state)).collect{ case v: SizeVariable[G] => v }
