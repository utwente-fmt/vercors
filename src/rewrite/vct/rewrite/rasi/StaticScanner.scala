...
class StaticScanner[G](initial_node: CFGNode[G], state: AbstractState[G]) {
  private val explored_nodes: mutable.Set[CFGEntry[G]] = mutable.Set.empty[CFGEntry[G]]
  ...
  private def scan_for_var_changes_from_node(node: CFGNode[G]): Boolean = {
    if (explored_nodes.contains(node)) return false
    explored_nodes.add(node)
    node_changes_vars(node) || node.successors.map(e => e.target)
                                   .collect{ case n: CFGNode[G] => n }
                                   .exists(n => scan_for_var_changes_from_node(n))
  }

  private def node_changes_vars(node: CFGNode[G]): Boolean = node.ast_node match {
    case Assign(target, value) => assignment_changes_vars(target, value)
    case Havoc(loc) => state.valuations.keySet.exists(v => v.is_contained_by(loc, state))
  ...
  private def assignment_changes_vars(target: Expr[G], value: Expr[G]): Boolean = {
    if (!state.valuations.keySet.exists(v => v.is_contained_by(target, state))) return false
    target.t match {
