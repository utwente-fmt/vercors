...
class VariableSelector[G](initial_state: AbstractState[G]) {
  ...
  /** Finds the untracked variables that distinguish the evaluation of the given
    * expression between the given valuations of tracked variables.
    *
    * @param valuations
    *   A set of different valuations of tracked variables, each of which can
    *   potentially evaluate to <code>true</code> on the given expression
    * @param expr
    *   Expression in which the valuations should be distinguished;
    *   <code>None</code> means <code>true</code>
    * @return
    *   A set of all variables that could, had they been known, have
  ...
  def distinguishing_variables(
    ...
    val constraints: Seq[Set[ConstraintMap[G]]] = states
  ...
  /** Finds the untracked variables that distinguish between the given
    * conditions in the initial state given by this object.
    *
    * @param conditions
    *   A set of conditions, each of which can potentially evaluate to
    *   <code>true</code> in the initial state
    * @return
    *   A set of all variables that could, had they been known, have
    *   distinguished between at least two of the given conditions
    */
  ...
  private def satisfying_valuations(
    ...
    expr match {
      ...
      case Some(e) =>
        if (!state.resolve_boolean_expression(e).can_be_false)
          Set(ConstraintMap.empty[G])
        else {
          val variables: Set[ResolvableVariable[G]] = free_variables(
            initial_state,
            e,
          )
          new ConstraintSolver(state, variables, is_contract = false)
            .resolve_assumption(e).filter(m => !m.is_impossible)
        }
    }
        ...
        expr match {
          ...
          case _ =>
            expr.subnodes.toSet[Node[G]].collect[Expr[G]] { case e: Expr[_] =>
              e
            }.flatMap(e => free_variables(state, e))
        }
      case _ => Set()
    }

  private def create_indexed_var_if_needed(
      state: AbstractState[G],
      expr: Expr[G],
      collection: Expr[G],
      subscript: Expr[G],
