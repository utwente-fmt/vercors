...
case class ConstraintMap[G](constraints: Map[ResolvableVariable[G], UncertainValue]) {
  /**
  ...
  def &&(other: ConstraintMap[G]): ConstraintMap[G] =
    ConstraintMap(constraints ++ other.constraints.map{ case (k, v) => k -> v.intersection(constraints.getOrElse(k, UncertainValue.uncertain_of(v.t[G]))) })
   ...
   */
  def ||(other: ConstraintMap[G]): ConstraintMap[G] =
    ConstraintMap.from_cons(constraints.keySet.intersect(other.constraints.keySet).map(k => k -> constraints(k).union(other.constraints(k))))
  ...
  /**
   * Checks whether the constraint map is empty. An empty map represents the constraint of all variables to the
   * uncertain value of their respective type. As such, a map containing only perfectly uncertain valuations is also
   * considered empty.
   *
   * @return <code>true</code> if the map does not constrain any variable, <code>false</code> otherwise
   */
  def is_empty: Boolean = resolve.forall(t => t._2 == UncertainValue.uncertain_of(t._1.t))
}
case object ConstraintMap {
  def from[G](variable: ResolvableVariable[G], value: UncertainValue): ConstraintMap[G] =
    ConstraintMap(Map.from(Seq(variable -> value)))

  def from_cons[G](cons: Set[(ResolvableVariable[G], UncertainValue)]): ConstraintMap[G] =
    ConstraintMap(Map.from(cons.map(t => t._1 -> t._2)))
