...
case class TrivialAddrOf[Pre <: Generation]() extends Rewriter[Pre] {
  ...
  //    y = &x;
  //  }
  def rewriteAssign(target: Expr[Pre], value: Expr[Pre], blame: Blame[AssignFailed], assignO: Origin): (Statement[Post], Expr[Post], Expr[Post]) = {
    implicit val o: Origin = assignO
    val newTarget = dispatch(target)
    val newValue = dispatch(value)
    val newPointer = Eval(PreAssignExpression(newTarget, NewPointerArray(newValue.t, const[Post](1))(PanicBlame("Size is > 0")))(blame))
