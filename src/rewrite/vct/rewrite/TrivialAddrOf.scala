...
case class TrivialAddrOf[Pre <: Generation]() extends Rewriter[Pre] {
  override def dispatch(e: Expr[Pre]): Expr[Post] = e match {
    ...
    case assign@PreAssignExpression(target, AddrOf(value)) if value.t.isInstanceOf[TClass[Pre]] =>
      ...
      val (newPointer, newTarget, newValue) = rewriteAssign(target, value, assign.o)
      val newAssign = PreAssignExpression(PointerSubscript(newTarget, const[Post](0))(PanicBlame("Should not happen")), newValue)(assign.blame)
      With(newPointer, newAssign)
    case other => rewriteDefault(other)
  }

  override def dispatch(s: Statement[Pre]): Statement[Post] = s match {
    case assign@Assign(target, AddrOf(value)) if value.t.isInstanceOf[TClass[Pre]] =>
      implicit val o: Origin = assign.o
      val (newPointer, newTarget, newValue) = rewriteAssign(target, value, assign.o)
      val newAssign = Assign(PointerSubscript(newTarget, const[Post](0))(PanicBlame("Should not happen")), newValue)(assign.blame)
      Block(Seq(newPointer, newAssign))
    case other => rewriteDefault(other)
  }

  def rewriteAssign(target: Expr[Pre], value: Expr[Pre], assignO: Origin): (Statement[Post], Expr[Post], Expr[Post]) = {
    implicit val o: Origin = assignO
    val newTarget = dispatch(target)
    val newValue = dispatch(value)
    val newPointer = Eval(PreAssignExpression(newTarget, NewPointerArray(newValue.t, const[Post](1))(PanicBlame("Should not happen")))(PanicBlame("Should not happen")))
