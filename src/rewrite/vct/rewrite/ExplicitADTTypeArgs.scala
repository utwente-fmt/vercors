...
import vct.col.rewrite.ExplicitADTTypeArgs.UnspecifiedADTTypeArgs
...
case object ExplicitADTTypeArgs extends RewriterBuilder {
  ...
  override def desc: String =
    "Specify explicit type arguments for function invocations in axiomatic datatypes."

  case class UnspecifiedADTTypeArgs(inv: ADTFunctionInvocation[_])
      extends UserError {
    override def code: String = "typeArgs"
    override def text: String =
      inv.o.messageInContext(
        "Missing type parameters for ADT function invocation."
      )
  }
}

case class ExplicitADTTypeArgs[Pre <: Generation]() extends Rewriter[Pre] {
  val owner: mutable.Map[ADTFunction[Pre], AxiomaticDataType[Pre]] = mutable
    .Map()
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    decl match {
      case adt: AxiomaticDataType[Pre] =>
        currentAdt.having(adt) { rewriteDefault(adt) }
      case other => rewriteDefault(other)
    }

  override def dispatch(e: Expr[Pre]): Expr[Post] =
    e match {
      case specified @ ADTFunctionInvocation(Some(_), _, _) =>
        rewriteDefault(specified)
      case unspecified @ ADTFunctionInvocation(None, Ref(func), args) =>
        if (currentAdt.nonEmpty && owner(func) == currentAdt.top) {
          ADTFunctionInvocation[Post](
            typeArgs = Some((
              succ(owner(func)),
              owner(func).typeArgs.map(v => TVar(succ[Variable[Post]](v))),
            )),
            ref = succ(func),
