...
import vct.col.rewrite.CheckContractSatisfiability.{
  AssertPassedNontrivialUnsatisfiable,
  NotWellFormedIgnoreCheckSat,
}
...
case object CheckContractSatisfiability extends RewriterBuilderArg[Boolean] {
  ...
  case class AssertPassedNontrivialUnsatisfiable(
  ...
  case class NotWellFormedIgnoreCheckSat(err: ExpectedError)
      extends Blame[VerificationFailure] {
    override def blame(error: VerificationFailure): Unit = err.trip(error)
  }
}

case class CheckContractSatisfiability[Pre <: Generation](
  ...
  def splitAccountedPredicate(pred: AccountedPredicate[Pre]): Seq[Expr[Pre]] =
    pred match {
      case UnitAccountedPredicate(pred) => unfoldStar(pred)
      case SplitAccountedPredicate(left, right) =>
        splitAccountedPredicate(left) ++ splitAccountedPredicate(right)
    }

  def checkSatisfiability(
      contract: ApplicableContract[Pre],
      n: Option[Declaration[Pre]],
  ): Unit = {
    implicit val origin: Origin = n.map(_.o).getOrElse(contract.o)
      .where(prefix = "checksat")
    foldStar(splitAccountedPredicate(contract.requires)) match {
      ...
      case pred =>
        val err = ExpectedError(
          "assertFailed:false",
          origin,
          AssertPassedNontrivialUnsatisfiable(contract),
        )
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    currentDeclaration.having(decl) { super.dispatch(decl) }

  override def dispatch(
      contract: ApplicableContract[Pre]
  ): ApplicableContract[Post] = {
    if (doCheck)
      checkSatisfiability(contract, currentDeclaration.topOption)
