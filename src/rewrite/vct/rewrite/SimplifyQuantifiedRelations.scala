...
case object SimplifyQuantifiedRelations extends RewriterBuilder {
  ...
  override def desc: String =
    "Simplify comparisons of integers that are inside a quantifier."
}

case class SimplifyQuantifiedRelations[Pre <: Generation]()
    extends Rewriter[Pre] {
  case object SimplifyQuantifiedRelationsOrigin extends Origin {
    override def preferredName: String = "unknown"
    override def shortPosition: String = "generated"
    override def context: String =
      "[At generated expression for the simplification of quantified integer relations]"
  ...
  case class ExtremeValue(
    ...
    def extremeValue(exprs: Seq[Expr[Pre]], maximizing: Boolean): Expr[Pre] =
      exprs match {
        case expr +: Nil => expr
    ...
    def extremeValue(
      ...
      val max =
        (e: Expr[Pre]) => extremeValue(e, maximizing).getOrElse(return None)
      val min =
        (e: Expr[Pre]) => extremeValue(e, !maximizing).getOrElse(return None)
        ...
        case Local(Ref(v)) =>
          if (bindings.contains(v)) {
            ...
            if (bounds.isEmpty)
              ...
              extremeValue(bounds.toSeq, !maximizing) - one
            else
              extremeValue(bounds.toSeq, !maximizing)
          } else { Local(v.ref) }
    ...
    def maximize(expr: Expr[Pre]): Option[Expr[Pre]] =
      extremeValue(expr, maximizing = true)
    def minimize(expr: Expr[Pre]): Option[Expr[Pre]] =
      extremeValue(expr, maximizing = false)
  }
  ...
  def trySimplify(
    ...
    val (globalConditions, bounds) = allConditions
      .partition(indepOf(bindings, _))

    // The hope is now that *every* condition in bounds is in the form `i (<|<=|==|>=|>) (bound!i)`
    val inclusiveLowerBound =
      bindings.map(_ -> mutable.ArrayBuffer[Expr[Pre]]()).toMap
    ...
    val newBody =
      Comparison.of(body) match {
        case Some((left, baseComp, right))
            if Comparison.CONTIGUOUS.contains(baseComp) =>
          val (const, comp, value) =
            if (indepOf(bindings, left)) { (left, baseComp, right) }
            else if (indepOf(bindings, right)) { (right, baseComp.flip, left) }
            else { return None }

          val maker = ExtremeValue(
            bindings,
            inclusiveLowerBound,
            exclusiveUpperBound,
          )
          ...
          val gt =
            if (!comp.less)
              Seq(
                comp.make(const, maker.maximize(value).getOrElse(return None))
              )
            else
              Nil
          val lt =
            if (!comp.greater)
              Seq(
                comp.make(const, maker.minimize(value).getOrElse(return None))
              )
            else
              Nil
    ...
    Some(Implies(
      AstBuildHelpers.foldAnd(
        globalConditions ++ bindings.flatMap(binding =>
          inclusiveLowerBound(binding).flatMap(lower =>
            exclusiveUpperBound(binding).map(upper => lower < upper)
          )
        )
      ),
      newBody,
