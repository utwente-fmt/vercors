...
case object UntupledQuantifiers extends RewriterBuilder {
  ...
  override def desc: String =
    "Expand out bindings in quantifiers, such as tuples"
}
...
case class UntupledQuantifiers[Pre <: Generation]() extends Rewriter[Pre] {
  ...
  def expand(
      ...
      initBindings: Seq[Variable[Post]] = Nil,
  )(implicit o: Origin): Expr[Post] = {

    if (bindings.isEmpty)
      return builder(initBindings)

    bindings.head.t match {
      case TBool() =>
        joiner(
          bindingValue.having(Map(bindings.head -> ff)) {
            expand(bindings.tail, builder, joiner, initBindings)
          },
          bindingValue.having(Map(bindings.head -> tt)) {
            expand(bindings.tail, builder, joiner, initBindings)
          },
        )
      case TTuple(preTs) =>
        val ts = preTs.map(dispatch)
        val vs = ts.zipWithIndex.map { case (t, i) =>
          new Variable[Post](t)(bindings.head.o.originContents.collectFirst {
            case TupledQuantifiers.CollectedBindingsOrigin(vs) => vs(i).o
            case _ => o
          }.get)
        }
        val value = LiteralTuple[Post](ts, vs.map(_.get))
        bindingValue.having(Map(bindings.head -> value)) {
          expand(bindings.tail, builder, joiner, initBindings ++ vs)
        }

      case other =>
        val v = new Variable[Post](dispatch(other))
        bindingValue.having(Map(bindings.head -> v.get)) {
          expand(bindings.tail, builder, joiner, initBindings :+ v)
        }
    }
  }
    ...
    e match {
      case Forall(bindings, triggers, body) =>
        expand(
          bindings,
          bindings =>
            Forall(bindings, triggers.map(_.map(dispatch)), dispatch(body)),
          And(_, _),
        )
      case Exists(bindings, triggers, body) =>
        expand(
          bindings,
          bindings =>
            Exists(bindings, triggers.map(_.map(dispatch)), dispatch(body)),
          Or(_, _),
        )
      case s @ Starall(bindings, triggers, body) =>
        expand(
          bindings,
          bindings =>
            Starall(bindings, triggers.map(_.map(dispatch)), dispatch(body))(
              s.blame
            ),
          Star(_, _),
        )
