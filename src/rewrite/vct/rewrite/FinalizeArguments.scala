...
case object FinalizeArguments extends RewriterBuilder {
  ...
  override def desc: String =
    "Make it so the formal parameters of methods are not assigned to."

  private def FormalParameterOrigin(inner: Origin): Origin =
    inner.where(prefix = "formal")
}
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    decl match {
      case method: AbstractMethod[Pre] =>
        ...
        val argLocals =
          method.args.intersect(assignedLocals)
            .map(v => v -> new Variable(dispatch(v.t))(v.o)).toMap
              ...
              variables.collect {
                method.args.map { v =>
                  if (assignedLocals.contains(v)) {
                    variables.succeed(
                      v,
                      new Variable(dispatch(v.t))(FormalParameterOrigin(v.o)),
                    )
                  } else { rewriteDefault(v) }
                }
              }._1,
            body = method.body.map(body =>
              Scope[Post](
                argLocals.values.toSeq,
                Block(Seq(
                  Block(argLocals.map { case (formal, local) =>
                    assignLocal[Post](local.get, Local(succ(formal)))
                  }.toSeq),
