...
object Utils {
  ...
  def find_all_subexpressions[G](expr: Expr[G]): Seq[Statement[G]] =
    expr match {
      // Expressions causing side effects
      case pae @ PreAssignExpression(target, value) =>
        Seq(Assign(target, value)(pae.blame)(pae.o))
      case pae @ PostAssignExpression(target, value) =>
        Seq(Assign(target, value)(pae.blame)(pae.o))
      ...
      case mi @ MethodInvocation(
        ...
        Seq(
          InvokeMethod(obj, ref, args, outArgs, typeArgs, givenMap, yields)(
            mi.blame
          )(mi.o)
        )
      case ci @ ConstructorInvocation(
            ref,
            classTypeArgs,
            args,
            outArgs,
        ...
        Seq(
          InvokeConstructor(
            ref,
            classTypeArgs,
            get_out_variable(ref.decl.cls, ci.o),
            args,
            outArgs,
            typeArgs,
            givenMap,
      ...
      case no @ NewObject(cls) =>
        Seq(Instantiate(cls, get_out_variable(cls, no.o))(no.o))
      ...
      case Select(condition, whenTrue, whenFalse) => {
        ...
        if (true_stmts.nonEmpty)
          branches = branches
            .appended((condition, Block(true_stmts)(whenTrue.o)))
        if (false_stmts.nonEmpty)
          branches = branches
            .appended((negate(condition), Block(false_stmts)(whenFalse.o)))
        if (branches.nonEmpty)
          statements ++ Seq(Branch(branches)(expr.o))
        else
          statements
      }
      case Implies(left, right) => {
        ...
        if (right_stmts.nonEmpty)
          left_stmts :+ Branch(Seq((left, Block(right_stmts)(right.o))))(expr.o)
        else
          left_stmts
      }
      case And(left, right) => {
      ...
      case Or(left, right) => {
        ...
        if (right_stmts.nonEmpty)
          left_stmts :+
            Branch(Seq((negate(left), Block(right_stmts)(right.o))))(expr.o)
        else
          left_stmts
      }
      // General recursion case
      case _ =>
        expr.subnodes.collect { case ex: Expr[G] => ex }
          .flatMap(e => find_all_subexpressions(e))
    }
  ...
  def find_all_cases[G](
      body: Statement[G],
      index: GlobalIndex[G],
  ): Seq[(SwitchCase[G], GlobalIndex[G])] =
    body match {
      case Switch(_, _) => Seq()
      ...
      case PVLLoop(init, _, update, _, bod) =>
        Seq((init, 0), (bod, 2), (update, 3))
          .flatMap(t => find_all_cases(t._1, index.enter_scope(body, t._2)))
      ...
      case IndetBranch(branches) =>
        branches.zipWithIndex
          .flatMap(t => find_all_cases(t._1, index.enter_scope(body, t._2)))
      case Loop(init, _, update, _, bod) =>
        Seq((init, 0), (bod, 2), (update, 3))
          .flatMap(t => find_all_cases(t._1, index.enter_scope(body, t._2)))
      case RangedFor(_, _, bod) => find_all_cases(bod, index.enter_scope(body))
      case TryCatchFinally(bod, after, catches) =>
        Seq((bod, 0), (after, 1))
          .concat(catches.zipWithIndex.map(t => (t._1.body, t._2 + 2)))
          .flatMap(t => find_all_cases(t._1, index.enter_scope(body, t._2)))
      case Synchronized(_, bod) => find_all_cases(bod, index.enter_scope(body))
      case ParInvariant(_, _, content) =>
        find_all_cases(content, index.enter_scope(body))
      ...
      case WandPackage(_, proof) =>
        find_all_cases(proof, index.enter_scope(body))
  ...
  def negate[G](expr: Expr[G]): Expr[G] =
    expr match {
  ...
  def and[G](expr1: Option[Expr[G]], expr2: Option[Expr[G]]): Option[Expr[G]] =
    expr1 match {
      case None => expr2
      case Some(e1) =>
        expr2 match {
          case None => expr1
          case Some(e2) =>
            Some(And(e1, e2)(e1.o)) // TODO: Is the origin important?
        }
    }
