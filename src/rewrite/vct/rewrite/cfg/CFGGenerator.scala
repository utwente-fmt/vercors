...
case class CFGGenerator[G]() extends LazyLogging {
  private val found_labels: mutable.Map[LabelDecl[G], CFGNode[G]] = mutable
    .HashMap[LabelDecl[G], CFGNode[G]]()
  private val searched_labels
      : mutable.Map[LabelDecl[G], mutable.Set[CFGNode[G]]] = mutable
    .HashMap[LabelDecl[G], mutable.Set[CFGNode[G]]]()
  ...
  def generate(entry: Procedure[G]): CFGNode[G] = {
    ...
    val res = convert(
      entry.body.get,
      GlobalIndex[G](mutable.Seq(InitialIndex(entry))),
    )
  ...
  private def convert(
    ...
    if (converted_nodes.contains(context))
      return converted_nodes(context)
    ...
    node match {
      ...
      case stmt: ExpressionContainerStatement[_] =>
        expression_container_to_cfg(stmt, context)
      ...
      case label: Label[_] =>
        ...
        searched_labels.getOrElse(label.decl, mutable.Set())
          .foreach(g => g.successors.addOne(CFGEdge(cfg_node, None)))
        cfg_node
      case goto: Goto[_] =>
        val cfg_node: CFGNode[G] = statement_to_cfg(node, context)
        ...
        if (cfg_node.successors.isEmpty) {
          if (searched_labels.contains(goto.lbl.decl))
            searched_labels(goto.lbl.decl).addOne(cfg_node)
          else
            searched_labels.addOne((goto.lbl.decl, mutable.Set(cfg_node)))
        }
        cfg_node
      ...
      case c: ControlContainerStatement[_]
          if !context.indices.head.isInstanceOf[RunMethodIndex[_]] =>
        ...
        if (!new_context.has_statement())
          statement_to_cfg(c, context)
        else
          convert(new_context.resolve().get, new_context)
      // Any non-special statement is simply converted to a CFG node
      case _ => statement_to_cfg(node, context)
    }
  }

  private def statement_to_cfg(
      stmt: Statement[G],
      context: GlobalIndex[G],
  ): CFGNode[G] = {
  ...
  private def assign_to_cfg(
      assign: Assign[G],
      context: GlobalIndex[G],
  ): CFGNode[G] =
    context.indices.head match {
      case AssignmentIndex(a, _) if a == assign =>
        statement_to_cfg(assign, context)
      case _ =>
        convert(
          Eval(assign.target)(assign.target.o),
          context.enter_scope(assign),
        )
    }

  private def expression_container_to_cfg(
      stmt: ExpressionContainerStatement[G],
      context: GlobalIndex[G],
  ): CFGNode[G] =
    context.indices.head match {
      case ExpressionContainerIndex(s, _) if s == stmt =>
        statement_to_cfg(stmt, context)
      case _ => convert(Eval(stmt.expr)(stmt.expr.o), context.enter_scope(stmt))
    }

  private def find_successors(
      node: Statement[G],
      context: GlobalIndex[G],
  ): mutable.Set[CFGEdge[G]] =
    node match {
      // Exceptional statements
      // Statements that can jump to another part of the program
      case Goto(lbl) =>
        found_labels.get(lbl.decl) match {
      ...
      case Fork(obj) =>
        val run_method: RunMethod[G] =
          obj.t.asClass.get.cls.decl.declarations.collect {
            case r: RunMethod[G] => r
          }.head
        // Get the successor(s) of the fork statement as well as the new thread, starting with the run method
        sequential_successor(context).addOne(CFGEdge(
          convert(
            run_method.body.get,
            GlobalIndex[G](mutable.Seq()).enter_scope(run_method),
          ),
          None,
        ))
      ...
      case Throw(obj) =>
        mutable.Set(CFGEdge(exception_successor(obj, context), None))
      case Break(label) =>
        label match {
      ...
      case IndetBranch(branches) =>
        mutable.LinkedHashSet.from(branches.zipWithIndex.map(b =>
      ...
      case _: ControlContainerStatement[_] =>
        evaluate_first(context.enter_scope(node))
  ...
  private def evaluate_first(index: GlobalIndex[G]): mutable.Set[CFGEdge[G]] = {
    if (index.has_statement())
      mutable.Set(CFGEdge(resolve_index(index), None))
    else
      sequential_successor(index)
  }

  private def sequential_successor(
      index: GlobalIndex[G],
      cond: Option[Expr[G]] = None,
  ): mutable.Set[CFGEdge[G]] = {
    if (index.indices.nonEmpty)
      index.make_step()
        .map(i => CFGEdge(resolve_index(i._1), Utils.and(i._2, cond)))
    else
      mutable.Set(CFGEdge(CFGTerminal(), cond))
  }

  private def handle_switch_statement(
      switch: Switch[G],
      context: GlobalIndex[G],
  ): mutable.Set[CFGEdge[G]] = {
    val switches: Seq[(SwitchCase[G], GlobalIndex[G])] = Utils
      .find_all_cases(switch.body, context.enter_scope(switch))
    ...
    for (s <- switches) {
      s._1 match {
        case c @ Case(pattern) =>
          ...
          if (default.isEmpty)
            conds = conds :+ (c, Eval(cond)(cond.o), s._2)
        case c @ DefaultCase() => default = Some((c, s._2))
      }
    }
    conds = conds.reverse

    var successor_to_previous: mutable.Set[CFGEdge[G]] =
      default match {
        case Some(t) =>
          mutable.Set(
            CFGEdge(convert(t._1, t._2), Some(Utils.negate(conds.head._2.expr)))
          )
        case None =>
          sequential_successor(context, Some(Utils.negate(conds.head._2.expr)))
      }

    for ((c, i) <- conds.zipWithIndex) {
      successor_to_previous
        .addOne(CFGEdge(convert(c._1, c._3), Some(c._2.expr)))
      ...
      if (i == conds.size - 1)
        successor_to_previous = mutable.Set(CFGEdge(node, None))
      else
        successor_to_previous = mutable
          .Set(CFGEdge(node, Some(Utils.negate(conds(i + 1)._2.expr))))
    }
    successor_to_previous
  }

  private def return_successors(
      index: GlobalIndex[G]
  ): mutable.Set[CFGEdge[G]] =
    index.return_from_call().map(t => CFGEdge(resolve_index(t._1), t._2))

  private def exception_successor(
      exception: Expr[G],
      index: GlobalIndex[G],
  ): CFGEntry[G] = resolve_index(index.handle_exception(exception))
  ...
  private def resolve_index(index: GlobalIndex[G]): CFGEntry[G] =
    index.resolve() match {
