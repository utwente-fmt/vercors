...
case object FilterSpecIgnore extends RewriterBuilder {
  ...
  override def desc: String =
    "Remove elements that are ignored with spec_ignore."
...
case class FilterSpecIgnore[Pre <: Generation]() extends Rewriter[Pre] {
  ...
  lazy val keepApplicables: Map[Applicable[Pre], Node[Pre]] =
    program.collect { case invocation: Invocation[Pre] =>
  ...
  override def dispatch(stat: Statement[Pre]): Statement[Post] =
    stat match {
      case block @ Block(statements) =>
        ...
        statements.foreach {
          case start: SpecIgnoreStart[_] => opens :+= start
          case end: SpecIgnoreEnd[_] =>
            opens match {
              case Nil => throw ExtraIgnoreEnd(end)
              case _ => opens = opens.init
            }
          case other => if (opens.isEmpty) { result += rewriteDefault(other) }
        }
        ...
        app.contract.requires match {
          case UnitAccountedPredicate(BooleanValue(false)) =>
            keepApplicables.get(app) match {
              case None => app.drop()
              case Some(_) => super.dispatch(app)
            }
