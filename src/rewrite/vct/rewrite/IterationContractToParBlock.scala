...
case object IterationContractToParBlock extends RewriterBuilder {
  ...
  override def desc: String =
    "Translate loops with an iteration-style contract to a parallel block."
  ...
  case class VariableReadOutsideParLoop(e: Local[_]) extends UserError {
    ...
    override def text: String =
      e.o.messageInContext(
        "This variable may not be read here, since it is used as a loop variable in a loop specified with an iteration contract."
      )
  }
  ...
  case class VariableWrittenInParLoop(assign: Assign[_]) extends UserError {
    ...
    override def text: String =
      assign.o.messageInContext(
        "This variable may not be written to, since it is used as a loop variable in a loop specified with an iteration contract."
      )
  }
}

case class IterationContractToParBlock[Pre <: Generation]()
  ...
  override def dispatch(stat: Statement[Pre]): Statement[Post] =
    stat match {
      case loop @ Loop(
            ...
            cond,
            update,
            contract @ IterationContract(requires, ensures, context_everywhere),
            body,
          ) =>
        val IterationContractData(v, low, high) = loop
          .getIterationContractData(IterationContractOrigin)
        ...
        currentIterationVariables.having(v) {
          sendDecls.scope {
            val (newVars, newBody) = variables.collect { dispatch(body) }
      ...
      case s @ Scope(locals, _) =>
        s.rewrite(locals =
          variables.dispatch(locals.filterNot(iterationLoopVariables.contains))
        )
  ...
  override def dispatch(e: Expr[Pre]): Expr[Rewritten[Pre]] =
    e match {
      case l @ Local(Ref(v))
          if iterationLoopVariables.contains(v) &&
            !currentIterationVariables.toSeq.contains(v) =>
