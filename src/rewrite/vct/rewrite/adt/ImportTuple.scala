...
case class ImportTuple[Pre <: Generation](importer: ImportADTImporter)
  ...
  def tupFst(e: Expr[Post], fstType: Type[Post], sndType: Type[Post])(implicit
      o: Origin
  ): Expr[Post] =
  ...
  def tupSnd(e: Expr[Post], fstType: Type[Post], sndType: Type[Post])(implicit
      o: Origin
  ): Expr[Post] =
    ADTFunctionInvocation[Post](
      Some((tupleAdt.ref, Seq(fstType, sndType))),
  ...
  override def postCoerce(t: Type[Pre]): Type[Post] =
    t match {
      case TTuple(Seq(t1, t2)) =>
        TAxiomatic(tupleAdt.ref, Seq(dispatch(t1), dispatch(t2)))
