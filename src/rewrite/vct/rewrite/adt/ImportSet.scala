...
case class ImportSet[Pre <: Generation](importer: ImportADTImporter)
  ...
  private lazy val setSingleton = find[ADTFunction[Post]](
    setAdt,
    "set_singleton",
  )
  private lazy val setUnionOne = find[ADTFunction[Post]](
    setAdt,
    "set_union_one",
  )
  private lazy val setUnion = find[ADTFunction[Post]](setAdt, "set_union")
  private lazy val setIntersection = find[ADTFunction[Post]](
    setAdt,
    "set_intersection",
  )
  private lazy val setDifference = find[ADTFunction[Post]](
    setAdt,
    "set_difference",
  )
  ...
  override def postCoerce(t: Type[Pre]): Type[Post] =
    t match {
      case TSet(inner) =>
        TAxiomatic[Post](setAdt.ref, Seq(dispatch(inner)))(t.o)
