...
case object ImportFrac extends ImportADTBuilder("frac") {
  case class RatZFracPreconditionFailed(
      inner: Blame[CoerceRatZFracFailed],
      expr: Expr[_],
  ...
  case class RatFracPreconditionFailed(
      inner: Blame[CoerceRatFracFailed],
      expr: Expr[_],
  ) extends Blame[PreconditionFailed] {
    override def blame(error: PreconditionFailed): Unit =
  ...
  case class ZFracFracPreconditionFailed(
      inner: Blame[CoerceZFracFracFailed],
      expr: Expr[_],
  ) extends Blame[PreconditionFailed] {
    override def blame(error: PreconditionFailed): Unit =
...
case class ImportFrac[Pre <: Generation](importer: ImportADTImporter)
  ...
  override def applyCoercion(e: => Expr[Post], coercion: Coercion[Pre])(
    ...
    coercion match {
      ...
      case CoerceFracZFrac() =>
        val rat = ADTFunctionInvocation[Post](
          Some((fracAdt.ref, Nil)),
          fracVal.ref,
          Seq(e),
        )
        FunctionInvocation[Post](zfracNew.ref, Seq(rat), Nil, Nil, Nil)(
          PanicBlame("a frac always fits in a zfrac.")
        )

      case CoerceRatZFrac() =>
        FunctionInvocation[Post](zfracNew.ref, Seq(e), Nil, Nil, Nil)(NoContext(
          RatZFracPreconditionFailed(globalBlame.top, e)
        ))
      case CoercionSequence(Seq(CoerceRatZFrac(), CoerceZFracFrac())) =>
        FunctionInvocation[Post](fracNew.ref, Seq(e), Nil, Nil, Nil)(NoContext(
          RatZFracPreconditionFailed(globalBlame.top, e)
        ))
      case CoerceZFracFrac() =>
        ...
        FunctionInvocation[Post](fracNew.ref, Seq(rat), Nil, Nil, Nil)(
          NoContext(ZFracFracPreconditionFailed(globalBlame.top, e))
        )

      case CoerceBoundIntFrac() =>
        FunctionInvocation[Post](fracNew.ref, Seq(WritePerm()), Nil, Nil, Nil)(
          PanicBlame("The constant 1 always fits in a frac.")
        )
      case CoerceBoundIntZFrac(_) =>
        FunctionInvocation[Post](
          zfracNew.ref,
          Seq(Select(e === const(0), NoPerm(), WritePerm())),
          Nil,
          Nil,
          Nil,
        )(PanicBlame("The constants 0 and 1 always fit in a zfrac."))
