...
import vct.rewrite.adt.ImportSetCompat.{SetEmptyFreshPreconditionFailed, SetEmptyPreconditionFailed}

case object ImportSetCompat extends ImportADTBuilder("map_compat") {
  case class SetEmptyPreconditionFailed(choose: Choose[_]) extends Blame[PreconditionFailed] {
    override def blame(error: PreconditionFailed): Unit =
      choose.blame.blame(SetEmpty(choose.xs))
  }

  case class SetEmptyFreshPreconditionFailed(choose: ChooseFresh[_]) extends Blame[PreconditionFailed] {
    override def blame(error: PreconditionFailed): Unit =
      choose.blame.blame(SetEmpty(choose.xs))
  }
}

case class ImportSetCompat[Pre <: Generation](importer: ImportADTImporter) extends ImportADT[Pre](importer) {
  private lazy val setCompatFile = parse("set_compat")

  private lazy val setChoose = find[Function[Post]](setCompatFile, "set_choose")
  private lazy val setChooseFresh = find[Procedure[Post]](setCompatFile, "set_choose_fresh")
