...
case class ImportNothing[Pre <: Generation](importer: ImportADTImporter)
    extends ImportADT[Pre](importer) {
  private lazy val nothingFile = parse("nothing")

  private lazy val nothingAdt = find[AxiomaticDataType[Post]](
    nothingFile,
    "nothing",
  )
    ...
    coercion match {
      case CoerceNothingSomething(target) =>
        FunctionInvocation[Post](
          ...
          Nil,
          Nil,
        )(PanicBlame("coercing from nothing requires nothing."))
