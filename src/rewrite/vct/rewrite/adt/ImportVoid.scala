...
case class ImportVoid[Pre <: Generation](importer: ImportADTImporter)
  ...
  override def postCoerce(e: Expr[Pre]): Expr[Post] =
    e match {
      case Void() => ADTFunctionInvocation[Post](None, voidUnit.ref, Nil)(e.o)
      case other => rewriteDefault(other)
    }
    ...
    decl match {
      case method: AbstractMethod[Pre] if method.returnType == TVoid[Pre]() =>
        allScopes.anyDeclare(
          allScopes.anySucceedOnly(method, method.rewrite(returnType = TVoid()))
        )
