...
case class ImportOption[Pre <: Generation](importer: ImportADTImporter)
  ...
  override def preCoerce(e: Expr[Pre]): Expr[Pre] =
    e match {
      ...
      // Case generated by EncodePointerArrays
      // optGet(optEmpty(a) ? None : get_pointer(optGet(a)))
      case og1 @ OptGet(
            Select(
              OptEmpty(a),
              OptNone() | OptNoneTyped(_),
              OptSome(
                inv @ ADTFunctionInvocation(args, f, Seq(og2 @ OptGet(b)))
              ),
            )
          ) if a == b =>
        preCoerce(
          ADTFunctionInvocation(args, f, Seq(OptGet(b)(og1.blame)(og2.o)))(
            inv.o
          )
        )
