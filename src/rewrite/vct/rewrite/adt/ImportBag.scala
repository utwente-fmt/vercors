...
case class ImportBag[Pre <: Generation](importer: ImportADTImporter)
  ...
  private lazy val bagSingleton = find[ADTFunction[Post]](
    bagAdt,
    "bag_singleton",
  )
  private lazy val bagUnionOne = find[ADTFunction[Post]](
    bagAdt,
    "bag_union_one",
  )
  private lazy val bagUnion = find[ADTFunction[Post]](bagAdt, "bag_union")
  private lazy val bagIntersection = find[ADTFunction[Post]](
    bagAdt,
    "bag_intersection",
  )
  private lazy val bagDifference = find[ADTFunction[Post]](
    bagAdt,
    "bag_difference",
  )
  ...
  private lazy val bagDisjointSkolem = find[ADTFunction[Post]](
    bagAdt,
    "bag_disjoint_skolem",
  )
  ...
  override def postCoerce(t: Type[Pre]): Type[Post] =
    t match {
      case TBag(inner) =>
        TAxiomatic[Post](bagAdt.ref, Seq(dispatch(inner)))(t.o)
