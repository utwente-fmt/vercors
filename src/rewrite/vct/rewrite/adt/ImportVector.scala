...
case object ImportVector extends ImportADTBuilder("vector") {
  case class VectorBoundsPreconditionFailed(
      inner: Blame[VectorBoundFailure],
      subscript: VectorSubscript[_],
  ...
  case class VectorOperatorToInvocation(div: DividingExpr[_])
      extends Blame[InvocationFailure] {
    override def blame(error: InvocationFailure): Unit =
      error match {
        case PreconditionFailed(dirs, _, _) =>
          div.blame.blame(VectorDivByZero(div, getNum(dirs)))
        case _ => PanicBlame("Should not occur")
      }
  }

  def getNum(dirs: Seq[AccountedDirection], accum: Int = 0): Int =
    dirs match {
...
case class ImportVector[Pre <: Generation](importer: ImportADTImporter)
    extends ImportADT[Pre](importer) {
  import ImportVector._
  private lazy val vectorFile = parse("vector")
  private lazy val vectorAdt = find[AxiomaticDataType[Post]](
    vectorFile,
    "vector",
  )
  ...
  val vectorOpsMethods
      : mutable.Map[(BigInt, String, Type[Post]), Function[Post]] = mutable
    .Map()
  ...
  def vlocMake(size: BigInt): Function[Post] = {
    ...
    globalDeclarations.declare(withResult((result: Result[Post]) => {
      ...
      val requires: Seq[Expr[Post]] = Seq(
        const[Post](0) <= i.get,
        i.get < const[Post](size),
      )
      val body = ADTFunctionInvocation[Post](
        Some((vectorAdt.ref, Seq(elementType))),
        vector_loc.ref,
        Seq(a.get, i.get),
      )
      ...
      function(
        ...
        requires = SplitAccountedPredicate(
          UnitAccountedPredicate(requires(0)),
          UnitAccountedPredicate(requires(1)),
        ),
  ...
  def makeVectorCreation(size: BigInt): Function[Post] = {
    ...
    globalDeclarations.declare(withResult((result: Result[Post]) => {
      ...
      val xs =
        Seq.tabulate(size.toInt)(x =>
          new Variable[Post](elementType)(o.where(name = f"x$x"))
        )
      val ensures: Seq[Expr[Post]] = xs.zipWithIndex.map({ case (x, i) =>
        Eq(
          makeSubScript(
            result,
            const[Post](i),
  ...
  def makeVectorOp(
      size: BigInt,
      elementType: Type[Post],
      isDivOp: Boolean,
      isCompareOp: Boolean,
      operator: (Expr[Post], Expr[Post]) => Expr[Post],
      operatorName: String,
  ): Function[Post] = {
    implicit val o: Origin = Origin(
      Seq(LabelContext(f"vector $operatorName method"))
    )
    ...
    globalDeclarations.declare(withResult((result: Result[Post]) => {
      ...
      val requires =
        if (isDivOp && size > 0)
          foldPredicate(Seq.tabulate(size.toInt)(i =>
            Neq(
              makeSubScript(
                y.get,
                const[Post](i),
      ...
      val ensures: Seq[Expr[Post]] =
        Seq.tabulate(size.toInt)(i =>
          Eq(
            makeSubScript(
              result,
              const[Post](i),
  ...
  override def postCoerce(t: Type[Pre]): Type[Post] =
    t match {
      case TVector(size, inner) =>
        TAxiomatic[Post](vectorAdt.ref, Seq(dispatch(inner)))(t.o)
  ...
  override def postCoerce(e: Expr[Pre]): Expr[Post] =
    e match {
      case LiteralVector(element, xs) =>
        val makeVector = makeVectorMethods
          .getOrElseUpdate(xs.size, makeVectorCreation(xs.size))
      ...
      case sub @ VectorSubscript(vec, index) =>
        ...
        makeSubScript(
          ...
          dispatch(elementT),
          size,
          NoContext(VectorBoundsPreconditionFailed(sub.blame, sub)),
          e.o,
        )
      case e: VectorBinExpr[Pre] =>
        val (size, elementTold) =
          e.t match {
            case TVector(size, inner) => (size, inner)
            case _ => ???
          }
        ...
        val divOpBlame =
          e match {
            case e: DividingVectorBinExpr[Pre] => Some(e); case _ => None
          }
        ...
        val vectorOpFunction =
          e match {
            case _: VectorEq[Pre] =>
              ...
              vectorOpsMethods.getOrElseUpdate(
                ...
                makeVectorOp(
                  ...
                  isCompareOp = true,
                  (l, r) =>
                    Select(Eq[Post](l, r)(o), const(1)(o), const(0)(o))(o),
                  "eq",
                ),
              )
            ...
            case div: VectorTruncDiv[Pre] =>
              ...
              vectorOpsMethods.getOrElseUpdate(
                (size, "truncDiv", elementT),
                makeVectorOp(
                  size,
                  elementT,
                  isDivOp = divOp,
                  isCompareOp = false,
                  (l, r) => TruncDiv[Post](l, r)(div.blame)(o),
                  "div",
                ),
              )
            case div: VectorFloorDiv[Pre] =>
              val o: Origin = Origin(Seq(LabelContext("vector div method")))
              vectorOpsMethods.getOrElseUpdate(
                (size, "floorDiv", elementT),
                makeVectorOp(
                  size,
                  elementT,
                  isDivOp = divOp,
                  isCompareOp = false,
                  (l, r) => FloorDiv[Post](l, r)(div.blame)(o),
                  "div",
                ),
              )
            ...
            case mod: VectorTruncMod[Pre] =>
              val o: Origin = Origin(Seq(LabelContext("vector mod method")))
              vectorOpsMethods.getOrElseUpdate(
                (size, "truncMod", elementT),
                makeVectorOp(
                  size,
                  elementT,
                  isDivOp = divOp,
                  isCompareOp = false,
                  (l, r) => TruncMod[Post](l, r)(mod.blame)(o),
                  "mod",
                ),
              )
          }
      ...
      case f: FunctionInvocation[Post] => e.rewriteDefault()
      case other => other.rewriteDefault()
    }
}
