...
case class DivFailed(div: DividingExpr[_]) extends Blame[InvocationFailure] {
  override def blame(error: InvocationFailure): Unit =
    error match {
      case PreconditionFailed(_, _, _) => div.blame.blame(ScalarDivByZero(div))
...
case object TruncDivMod extends RewriterBuilder {
  ...
  override def desc: String =
    "Encode truncated division and modulo using euclidean division and modulo."
}
...
case class TruncDivMod[Pre <: Generation]() extends Rewriter[Pre] {
  ...
  def truncMod(mod: TruncMod[Pre]): Expr[Post] = {
    val truncModFunc = truncModFunctions
      .getOrElseUpdate((), makeTruncModFunction())
    FunctionInvocation[Post](
      truncModFunc.ref,
      Seq(dispatch(mod.left), dispatch(mod.right)),
      Nil,
      Nil,
      Nil,
    )(DivFailed(mod))(mod.o)
  ...
  def truncDiv(div: TruncDiv[Pre]): Expr[Post] = {
    val truncDiv_func = truncDivFunctions
      .getOrElseUpdate((), makeTruncDivFunction())
    FunctionInvocation[Post](
      truncDiv_func.ref,
      Seq(dispatch(div.left), dispatch(div.right)),
      Nil,
      Nil,
      Nil,
    )(DivFailed(div))(div.o)
  ...
  def truncFunctionOrigin(operator: String) =
    Origin(Seq(LabelContext(s"generated at `$operator` operator")))
  ...
  def makeTruncModFunction(): Function[Post] = {
    ...
    val new_t = TInt[Post]()
    val a_var =
      new Variable[Post](new_t)(truncFunctionOrigin("%").where(name = "a"))
    val b_var =
      new Variable[Post](new_t)(truncFunctionOrigin("%").where(name = "b"))
    ...
    globalDeclarations.declare(
      function[Post](
        ...
        body = Some(let(
          ...
          a % b,
          (mod: Local[Post]) =>
            Select(a >= const(0) || mod === const(0), mod, mod - absb),
        )),
