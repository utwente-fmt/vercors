...
case class MonomorphizeClass[Pre <: Generation]()
    extends Rewriter[Pre] with LazyLogging {
  val currentSubstitutions: ScopedStack[Map[Variable[Pre], Type[Pre]]] =
    ScopedStack()

  type Key = (Class[Pre], Seq[Type[Pre]])
  case class InstantiationContext(
      cls: Class[Pre],
    ...
    def substitute =
      Substitute(Map.empty[Expr[Pre], Expr[Pre]], typeSubs = substitutions)
  ...
  def keepBodies: Boolean =
    ctx.topOption.map { ctx => ctx.keepBodies }.getOrElse(true)

  val knownInstantiations: mutable.LinkedHashSet[Key] = mutable.LinkedHashSet()
  // key: generically instantiated type in pre
  val genericSucc: SuccessionMap[(Key, Declaration[Pre]), Declaration[Post]] =
    SuccessionMap()
  ...
  def instantiate(
    ...
    logger.debug(
      s"Instantiating class ${cls.o.getPreferredNameOrElse().ucamel} $mode, args: $typeValues"
    )
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    decl match {
      case cls: Class[Pre] if cls.typeArgs.nonEmpty =>
        ...
        instantiate(
          cls,
          cls.typeArgs.map(v => v.t.asInstanceOf[TType[Pre]].t),
          true,
        )
      case method: InstanceMethod[Pre] if ctx.nonEmpty =>
        val newMethod: InstanceMethod[Post] = method.rewrite(body =
          if (keepBodies)
      ...
      case cons: Constructor[Pre] if ctx.nonEmpty =>
        val newCons = cons.rewrite(body =
          if (keepBodies)
            cons.body.map(dispatch)
          else
            None
        )
  ...
  override def dispatch(stmt: Statement[Pre]): Statement[Post] =
    stmt match {
      case inv: InvokeConstructor[Pre] if inv.classTypeArgs.nonEmpty =>
        ...
        val typeValues = ctx.topOption.map(_.evalTypes(inv.classTypeArgs))
          .getOrElse(inv.classTypeArgs)
        instantiate(cls, typeValues, false)
        inv.rewrite(ref =
          genericSucc
            .ref[Post, Constructor[Post]](((cls, typeValues), inv.ref.decl))
        )
      case inv: InvokeMethod[Pre] =>
        inv.obj.t match {
          case t: TClass[Pre] if t.typeArgs.nonEmpty =>
            val typeValues = ctx.topOption.map(_.evalTypes(t.typeArgs))
              .getOrElse(t.typeArgs)
            instantiate(t.cls.decl, typeValues, false)
            inv.rewrite(ref =
              genericSucc.ref[Post, InstanceMethod[Post]](
                ((t.cls.decl, typeValues), inv.ref.decl)
              )
            )
  ...
  override def dispatch(loc: Location[Pre]): Location[Post] =
    loc match {
      case loc @ FieldLocation(obj, Ref(field)) =>
        obj.t match {
          case t: TClass[Pre] if t.typeArgs.nonEmpty =>
            val typeArgs1 = ctx.topOption.map(_.evalTypes(t.typeArgs))
              .getOrElse(t.typeArgs)
            instantiate(t.cls.decl, typeArgs1, false)
            loc.rewrite(field =
              genericSucc.ref[Post, InstanceField[Post]](
                ((t.cls.decl, typeArgs1), field)
              )
            )
