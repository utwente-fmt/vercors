...
(
  ...
  def apply(rule: (Seq[Variable[Rule]], Expr[Rule], Expr[Rule], Origin), subject: Expr[Pre]): Option[Expr[Pre]] = {
    ...
    val debugFilter =
      debugFilterRule.map(_ == ruleOrigin.getPreferredName.get.preferredName).getOrElse(true) &&
    ...
    lazy val debugHeader: String = s"Expression `$subject` does not match rule ${ruleOrigin.getPreferredName.get.preferredName}, since"
    ...
    if(debugMatch && debugFilter) {
      if(debugMatchShort) {
        ...
        if (inst.nonEmpty) {
          ...
          inst.toSeq.sortBy { case (k, _) => k.o.getPreferredName.get.preferredName }.foreach {
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    debugNameStack.having(decl.o.getPreferredName.get.preferredName) {
      rewriteDefault(decl)
    }
