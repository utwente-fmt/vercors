  ...
  case class BuilderFor[Rule](
    ...
    override def apply[Pre <: Generation](): Rewriter[Pre] =
      ApplyTermRewriter(
        ...
        debugMatchShort,
        debugNoMatch,
        debugFilterInputKind,
        debugFilterRule,
      )
    override def key: String = "simplify"
    override def desc: String =
      "Apply axiomatic simplification lemmas to expressions."
  }
}
...
case class ApplyTermRewriter[Rule, Pre <: Generation](
  ...
  case class MalformedSimplificationRuleBinders(body: Expr[_])
      extends UserError {
  ...
  val rules: Seq[(Seq[Variable[Rule]], Expr[Rule], Expr[Rule], Origin)] =
    ruleNodes.map(node => (node.o, consumeForalls(node.axiom))).map {
      case (o, (free, body)) =>
        body match {
  ...
  val ruleMap: Map[java.lang.Class[_], Seq[
    (Seq[Variable[Rule]], Expr[Rule], Expr[Rule], Origin)
  ]] = rules.groupBy { case (_, pattern, _, _) => pattern.getClass }

  val debugNameStack: ScopedStack[String] = ScopedStack()

  val updateProgress: ScopedStack[String => Unit] = ScopedStack()

  def consumeForalls(node: Expr[Rule]): (Seq[Variable[Rule]], Expr[Rule]) =
    node match {
  ...
  case class ApplyParametricBindings(
    ...
    override def succProvider: SuccessorsProvider[Pre, Pre] =
      new SuccessorsProviderTrafo(allScopes.freeze) {
        ...
        override def succ[RefDecl <: Declaration[Pre]](
          ...
          bindings.getOrElse(decl, decl.asInstanceOf[RefDecl].ref)
            .asInstanceOf[Ref[Pre, RefDecl]]
      }
  ...
  case class ApplyRule(
      inst: Map[Variable[Rule], (Expr[Pre], Seq[Variable[Pre]])],
      typeInst: Map[Variable[Rule], Type[Pre]],
      defaultOrigin: Origin,
      rule: Expr[Rule],
  ) extends NonLatchingRewriter[Rule, Pre] {
    val binderOrigins: mutable.Map[Variable[Rule], Origin] = mutable.Map.empty

    def addBinderOrigin(
        newVar: Ref[Rule, Variable[Rule]],
        originalVar: Variable[Pre],
    ): Unit = {
      val newOrigin = originalVar.o
      if (
        binderOrigins.contains(newVar.decl) &&
        binderOrigins(newVar.decl) != newOrigin
      ) { throw MalformedSimplificationRuleBinders(rule) }
      else { binderOrigins(newVar.decl) = newOrigin }
    }
    ...
    override def dispatch(e: Expr[Rule]): Expr[Pre] =
      e match {
        case Local(Ref(v)) =>
          if (inst.contains(v))
            ApplyParametricBindings(Map.empty).dispatch(inst(v)._1)
          else
            Local[Pre](succ(v))(e.o)
        case FunctionOf(Ref(v), ruleVars) =>
          val (replacement, vars) = inst(v)
          ApplyParametricBindings(
            vars.zip(ruleVars.map(ruleVar => succ[Variable[Pre]](ruleVar.decl)))
              .toMap
    ...
    override def dispatch(d: Declaration[Rule]): Unit =
      d match {
        case v: Variable[Rule] =>
          variables.succeed(
            v,
            new Variable(dispatch(v.t))(
              binderOrigins.getOrElse(v, defaultOrigin)
            ),
          )
        case other => rewriteDefault(other)
      }

    override def dispatch(t: Type[Rule]): Type[Pre] =
      t match {
        case TVar(Ref(v)) =>
          typeInst(v) // PB: maybe this is wrong in contrived situations?
        case other => rewriteDefault(other)
      }
  }

  def apply(
      rule: (Seq[Variable[Rule]], Expr[Rule], Expr[Rule], Origin),
      subject: Expr[Pre],
  ): Option[Expr[Pre]] = {
    incApply()
    ...
    val debugFilter =
      debugFilterRule.map(_ == ruleOrigin.preferredName).getOrElse(true) &&
        debugFilterInputKind.map(_ == subject.getClass.getSimpleName)
          .getOrElse(true) &&
        (debugIn.isEmpty ||
          debugIn.exists(name => debugNameStack.exists(_ == name)))
    ...
    lazy val debugHeader: String =
      s"Expression `$subject` does not match rule ${ruleOrigin.preferredName}, since"
      ...
      typeInst.get(left) match {
        case Some(replacement) =>
          ...
          if (debugNoMatch && debugFilter && !matches)
            logger.debug(
              s"$debugHeader earlier `$left` matched `$replacement`, but now it must match `$right`"
            )
        ...
        case None =>
          ...
          val matches = leftUpperBound
            .superTypeOf(right.asInstanceOf[Type[Rule]])

          if (debugNoMatch && debugFilter && !matches) {
            logger.debug(
              s"$debugHeader the type-level variable `$left` matched `$right`, but " +
    ...
    def declareInst(
        left: Variable[Rule],
        right: Expr[Pre],
        leftBindings: Seq[Variable[Rule]],
    ): Boolean = {
      ...
      inst.get(left) match {
        case Some((replacement, _)) =>
          ...
          if (debugNoMatch && debugFilter && !matches)
            logger.debug(
              s"$debugHeader earlier `$debugLeft` matched `$replacement`, but now it must match `$right`"
            )

          matches
        case None =>
          val freeRight =
            FreeVariables.freeVariables(right).collect {
              case FreeVariables.ReadFreeVar(Local(Ref(v))) => v
            }.toSet

          val freeRightOfBindings = freeRight
            .intersect(bindingInst.values.toSet)
          ...
          if (extraBindingDeps.nonEmpty) {
            if (debugNoMatch && debugFilter)
              logger.debug(
                s"$debugHeader `$debugLeft` matches `$right`, but it is dependent on bindings within the pattern that are not declared as such: ${extraBindingDeps
                    .map(v => Local[Pre](v.ref).toString).mkString(", ")}."
              )
          ...
          if (debugNoMatch && debugFilter && !matches)
            logger.debug(
              s"$debugHeader `$debugLeft` (typed `${left
                  .t}`) matched `$right` (typed `${right.t}`), " +
    ...
    Compare.compare(pattern, subject) {
      ...
      case Comparator
            .MatchingDeclaration(left: Variable[Rule], right: Variable[Pre]) =>
        bindingInst(left) = right
      case Comparator.MatchingDeclaration(_, _) =>
        throw Unreachable(
          "Simplification rules do not declare anything other than variables from binders."
        )

      case Comparator
            .MatchingReference(left: Variable[Rule], right: Variable[Pre]) =>
        if (free.contains(left)) {
          ...
          /* !free.contains(left) */ {
            if (!bindingInst.get(left).contains(right)) { return None }
          }
      case Comparator.MatchingReference(_, _) =>
        throw Unreachable(
          "Simplification rules do not refer to anything other than variables."
        )

      case Comparator.StructuralDifference(Local(Ref(v)), right: Expr[Pre])
          if free.contains(v) =>
        if (!declareInst(v, right, Nil))
          return None

      case Comparator.StructuralDifference(
            FunctionOf(Ref(v), bindings),
            right: Expr[Pre],
          ) if free.contains(v) =>
        if (!declareInst(v, right, bindings.map(_.decl)))
          return None

      case Comparator.StructuralDifference(TVar(Ref(v)), right: Type[Pre])
          if free.contains(v) =>
        if (!declareTypeInst(v, right))
          return None
    ...
    val result = ApplyRule(inst.toMap, typeInst.toMap, subject.o, substitute)
      .dispatch(substitute)
          ...
          inst.toSeq.sortBy { case (k, _) => k.o.preferredName }.foreach {
            case (rule, (binding, over)) =>
              if (over.isEmpty)
                ...
                logger.debug(
                  s"  $rule = ($binding) parametric over ${over.mkString(", ")}"
                )
          }
        }
        if (typeInst.nonEmpty) {
          logger.debug("With type bindings:")
          typeInst.foreach { case (rule, binding) =>
            logger.debug(s"  $rule = $binding")
          }
        }
    ...
    override final def dispatch(e: Expr[Pre]): Expr[Pre] = {
      ...
      applyOnce(simplifiedBottomUp) match {
        case Some(simplifiedYetAgain) =>
          ...
          logger.debug(
            s"Complicated simplification: simplified node $simplifiedBottomUp was simplified yet again to $simplifiedYetAgain"
          )
  ...
  def incApply(): Unit = {
    ...
    if (countApply % 10000 == 0) {
      logger.debug(
        s"Applied $countApply rules ($countSuccess successfully) to expression $currentExpr"
      )
    }
  }

  def incSuccess(): Unit = { countSuccess += 1 }

  override def dispatch(e: Expr[Pre]): Expr[Post] =
    if (simplificationDone.nonEmpty)
      rewriteDefault(e)
    else
      simplificationDone.having(()) {
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    debugNameStack.having(decl.o.preferredName) { rewriteDefault(decl) }
