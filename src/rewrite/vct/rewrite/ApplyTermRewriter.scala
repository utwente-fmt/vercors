...
case class ApplyTermRewriter[Rule, Pre <: Generation](
  ...
  def apply(
    ...
    val debugFilter =
      debugFilterRule.forall(
        SourceName.stringToName(_) == ruleOrigin.getPreferredNameOrElse()
    ...
    lazy val debugHeader: String =
      s"Expression `$subject` does not match rule ${ruleOrigin.getPreferredNameOrElse().snake}, since"
    ...
    Compare.compare(pattern, subject) {
      ...
      case StructuralDifference(Local(Ref(v)), right: Expr[Pre])
          if free.contains(v) =>
        if (!declareInst(v, right, Nil))
          return None

      case StructuralDifference(FunctionOf(Ref(v), bindings), right: Expr[Pre])
          if free.contains(v) =>
        if (!declareInst(v, right, bindings.map(_.decl)))
          return None

      case StructuralDifference(TVar(Ref(v)), right: Type[Pre])
          if free.contains(v) =>
    ...
    if (debugMatch && debugFilter) {
      if (debugMatchShort) {
        ...
        if (inst.nonEmpty) {
          ...
          inst.toSeq.sortBy { case (k, _) =>
            k.o.getPreferredNameOrElse().camel
          }.foreach { case (rule, (binding, over)) =>
            if (over.isEmpty)
