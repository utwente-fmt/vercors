...
case object MonomorphizeContractApplicables extends RewriterBuilder {
  ...
  override def desc: String =
    "Monomorphize generic applicable declarations with their usages where Silver does not support generics."
}

case class MonomorphizeContractApplicables[Pre <: Generation]()
    extends Rewriter[Pre] {
  val currentSubstitutions: ScopedStack[Map[Variable[Pre], Type[Post]]] =
    ScopedStack()

  val monomorphizedRef: mutable.Map[
    (ContractApplicable[Pre], Seq[Type[Post]]),
    Ref[Post, ContractApplicable[Post]],
  ] = mutable.Map()
  val monomorphizedImpl: mutable.Map[
    (ContractApplicable[Pre], Seq[Type[Post]]),
  ...
  def getOrBuild(
      ...
      typeValues: Seq[Type[Post]],
  ): Ref[Post, ContractApplicable[Post]] =
    monomorphizedRef.get((app, typeValues)) match {
      case Some(ref) => ref
      case None =>
        monomorphizedRef((app, typeValues)) =
          new LazyRef(monomorphizedImpl((app, typeValues)))
        monomorphizedImpl((app, typeValues)) =
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    decl match {
      case app: ContractApplicable[Pre] if app.typeArgs.nonEmpty =>
        ...
        getOrBuild(
          app,
          app.typeArgs.map(v => dispatch(v.t.asInstanceOf[TType[Pre]].t)),
        )
      ...
      case inv: Invocation[Pre] if inv.ref.decl.typeArgs.nonEmpty =>
        ...
        inv match {
          case inv: ProcedureInvocation[Pre] =>
            inv.rewrite(
              ref = ref.asInstanceOf[Ref[Post, Procedure[Post]]],
              typeArgs = Nil,
            )
          case inv: MethodInvocation[Pre] =>
            inv.rewrite(
              ref = ref.asInstanceOf[Ref[Post, InstanceMethod[Post]]],
              typeArgs = Nil,
            )
          case inv: FunctionInvocation[Pre] =>
            inv.rewrite(
              ref = ref.asInstanceOf[Ref[Post, Function[Post]]],
              typeArgs = Nil,
            )
          case inv: InstanceFunctionInvocation[Pre] =>
            inv.rewrite(
              ref = ref.asInstanceOf[Ref[Post, InstanceFunction[Post]]],
              typeArgs = Nil,
            )
        }
      case other => rewriteDefault(other)
  ...
  override def dispatch(t: Type[Pre]): Type[Rewritten[Pre]] =
    t match {
      case TVar(Ref(v)) =>
        currentSubstitutions.topOption
          .getOrElse(Map.empty[Variable[Pre], Type[Post]])
          .getOrElse(v, TVar(succ(v)))
      case other => rewriteDefault(other)
    }
}
