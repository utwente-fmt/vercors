...
case object ComputeBipGlue extends RewriterBuilder {
  ...
  override def desc: String =
    "Encodes BIP glue into SAT to compute all possible synchronizations"
}

//noinspection JavaAccessorEmptyParenCall
case class ComputeBipGlue[Pre <: Generation]()
    extends Rewriter[Pre] with LazyLogging {
  var program: Program[Pre] = _

  def xor(bm: BooleanFormulaManager, bs: Seq[BooleanFormula]): BooleanFormula =
    bs match {
      case Seq(b) => b
  ...
  case class BipSmt(ctx: SolverContext) {
    ...
    def get(n: Node[Pre]): BooleanFormula = {
      n match {
        case _: BipTransition[Pre] | _: BipGlueDataWire[Pre] | _: BipPort[Pre] |
            _: BipData[Pre] =>
        case _ => ???
      }
  ...
  case class PortEnablesInputsOutputs(
      port: BipPort[Pre],
      datas: Seq[BipData[Pre]],
  ) extends Constraint {
    override def toBooleanFormula(ctx: BipSmt): BooleanFormula =
      ctx.bm.implication(ctx.get(port), ctx.bm.and(datas.map(ctx.get): _*))
  }

  case class OutputDataNeedsPort(
      data: BipOutgoingData[Pre],
      ports: Seq[BipPort[Pre]],
  ) extends Constraint {
    override def toBooleanFormula(ctx: BipSmt): BooleanFormula =
      ctx.bm.implication(ctx.get(data), ctx.bm.or(ports.map(ctx.get): _*))
  }

  case class InputDataNeedsPort(
      data: BipIncomingData[Pre],
      ports: Seq[BipPort[Pre]],
  ) extends Constraint {
    override def toBooleanFormula(ctx: BipSmt): BooleanFormula =
  ...
  case class InputRequiresOneActiveWire(
      data: BipIncomingData[Pre],
      incomingWires: Seq[BipGlueDataWire[Pre]],
  ) extends Constraint {
      ...
      bm.implication(
        ...
        xor(
          ...
          incomingWires.map { wire =>
            bm.and(ctx.get(wire.dataOut.decl), ctx.get(wire))
          },
        ),
      )
    }
  ...
  case class WireRequiresPorts(wire: BipGlueDataWire[Pre]) extends Constraint {
    override def toBooleanFormula(ctx: BipSmt): BooleanFormula =
      ctx.bm.implication(
        ctx.get(wire),
        ctx.bm.and(ctx.get(wire.dataOut.decl), ctx.get(wire.dataIn.decl)),
      )
  }
  ...
  case class Requires(requires: Seq[BipGlueRequires[Pre]]) extends Constraint {
    ...
    override def toBooleanFormula(ctx: BipSmt): BooleanFormula =
      others match {
        ...
        case _ =>
          ctx.bm.implication(ctx.get(port), ctx.bm.or(others.map(ctx.get): _*))
      }
  }

  case class Accepts(
      accepts: Seq[BipGlueAccepts[Pre]],
      allPorts: Seq[BipPort[Pre]],
  ) extends Constraint {
    // There should be > 0 clauses, and all clauses should concern the same port
    ...
    lazy val offPorts = allPorts.collect {
      case p if !others.contains(p) && p != port => p
    }
  ...
  case class ExcludeEmptySynchronization(allPorts: Seq[BipPort[Pre]])
      extends Constraint {
    override def toBooleanFormula(ctx: BipSmt): BooleanFormula = {
  ...
  def computeGlue(glue: BipGlue[Pre]): Seq[BipPortSynchronization[Pre]] = {
    ...
    val classes =
      program.collect {
        case cls: Class[Pre]
            if cls.declarations.exists(relevantDecls.contains) =>
          cls
      }.toIndexedSeq
    val ports = classes.flatMap { cls =>
      cls.declarations.collect { // TODO: Use common one for ports
    ...
    val portEnablesInputsOutputs: Seq[Constraint] = classes.flatMap { cls =>
      ...
      val transitions =
        cls.declarations.collect { case t: BipTransition[Pre] =>
          t.port.decl -> t
        }.toMap
      val outgoingDatas = cls.declarations.collect {
        case d: BipOutgoingData[Pre] => d
      }
      ports.map { port =>
    ...
    val inputDataNeedsPort: Seq[Constraint] = classes.flatMap { cls =>
      val transitions = cls.declarations.collect { case t: BipTransition[Pre] =>
        t
      }
      val inDatas = transitions.flatMap(_.data.map(_.decl))
      inDatas.map { data =>
        val ports = transitions.collect {
          case t if t.data.map(_.decl).contains(data) => t.port.decl
        }
    ...
    val requires = {
      ...
      requiresPorts.map { port =>
        Requires(glue.requires.filter(port == _.port.decl))
      }
    }
    val accepts = {
      val acceptsPorts = glue.accepts.map(_.port.decl).distinct
      acceptsPorts.map { port =>
        Accepts(glue.accepts.filter(port == _.port.decl), ports)
      }
    }

    val constraints =
      (portEnablesInputsOutputs ++ outputDataNeedsPort ++
        inputDataNeedsPort ++ inputRequiresOneActiveWire ++ requires ++
        accepts ++ wireRequiresPorts :+ ExcludeEmptySynchronization(ports))

    Using(SolverContextFactory.createSolverContext(Solvers.SMTINTERPOL)) {
      ctx =>
        Using(ctx.newProverEnvironment(
          ...
          val combinedFormula =
            formulas.fold(bipSmt.bm.makeTrue()) { (l, r) =>
              bipSmt.bm.and(l, r)
            }
          val allVars = bipSmt.fm.extractVariables(combinedFormula).keySet()
            .asScala.toSeq.sorted.map(bipSmt.bm.makeVariable)
          logger.debug("=== Formula: all BIP constraints ===")
          logger.debug(
            new PrettyPrinter(bipSmt.fm).formulaToString(combinedFormula)
          )
          logger.debug("=== End of formula: all BIP constraints ===")
          formulas.foreach(prover.addConstraint)

          val synchronizations
              : mutable.ArrayBuffer[BipPortSynchronization[Pre]] = mutable
            .ArrayBuffer()
          while (!prover.isUnsat) {
            val (synchronization, modelFormula) =
              Using(prover.getModel()) { model =>
                ...
                val modelFormula: BooleanFormula = bipSmt.bm
                  .and(allVars.map { v =>
                    bipSmt.bm
                      .equivalence(v, bipSmt.bm.makeBoolean(model.evaluate(v)))
                  }: _*)

                (
                  new BipPortSynchronization[Pre](enabledPorts, enabledWires)(
                    glue.blame
                  )(glue.o),
                  modelFormula,
                )
          ...
          logger
            .debug(s"Possible synchronizations found: ${synchronizations.size}")
          ...
          synchronizations.sortBy { synchronization =>
            (
              synchronization.ports.map(p => portInt(p.decl)),
              synchronization.wires.map(wireInt(_)),
            )
          }.toSeq
        }
    }.get.get
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    decl match {
      case glue: BipGlue[Pre] => computeGlue(glue).foreach(dispatch)
