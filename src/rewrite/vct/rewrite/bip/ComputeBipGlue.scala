...
case class ComputeBipGlue[Pre <: Generation]()
  ...
  case class BipSmt(ctx: SolverContext) {
    ...
    def unique(n: Node[Pre]): String =
      uniqueName(prefix(n) + n.o.getPreferredNameOrElse().camel)
  ...
  def computeGlue(glue: BipGlue[Pre]): Seq[BipPortSynchronization[Pre]] = {
    ...
    val ports = classes.flatMap { cls =>
      cls.decls.collect { // TODO: Use common one for ports
    ...
    val portEnablesInputsOutputs: Seq[Constraint] = classes.flatMap { cls =>
      ...
      val transitions =
        cls.decls.collect { case t: BipTransition[Pre] => t.port.decl -> t }
          .toMap
      val outgoingDatas = cls.decls.collect { case d: BipOutgoingData[Pre] =>
        d
      }
      ports.map { port =>
