...
case object InstantiateBipSynchronizations extends RewriterBuilder {
  ...
  override def desc: String =
    "Convert synchronizations of ports combinatorially to synchronizations of transitions"
}

case class InstantiateBipSynchronizations[Pre <: Generation]()
  ...
  override def dispatch(program: Program[Pre]): Program[Post] = {
    ...
    if (transitionSynchrons.nonEmpty) {
      logger.info(
        s"Converted ${portSynchrons.size} JavaBIP port synchronizations into ${transitionSynchrons.size} JavaBIP transition synchronizations"
      )
  ...
  def pickOneFromEach[T](elems: Seq[Seq[T]]): Seq[Seq[T]] =
    elems match {
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    decl match {
      case sync: BipPortSynchronization[Pre] =>
        ...
        val transitionSets = pickOneFromEach(sync.ports.map { case Ref(port) =>
          portToTransitions(port)
        })
        val newSyncs = transitionSets.map { transitions =>
          globalDeclarations.declare(
            BipTransitionSynchronization[Post](
              transitions.map(t => succ[BipTransition[Post]](t)),
              wires,
            )(sync.blame)(sync.o)
          )
        }
