...
import vct.col.rewrite.bip.EncodeBipPermissions.{
  GeneratedBipFieldPermissionOrigin,
  GeneratedBipPermissionsOrigin,
}
...
case object EncodeBipPermissions extends RewriterBuilder {
  ...
  override def desc: String =
    "encodes implicit permission management of BIP models"

  case class GeneratedBipPermissionsOrigin(component: BipComponent[_])
      extends Origin {
  ...
  case class GeneratedBipFieldPermissionOrigin(
      component: BipComponent[_],
      field: InstanceField[_],
  ) extends Origin {
    override def preferredName: String =
      s"bipPerm${field.o.preferredName.capitalize}"
  ...
  override def dispatch(node: Declaration[Pre]): Unit =
    node match {
      ...
      case component: BipComponent[Pre] =>
        ...
        val fields = currentClass.top.declarations.collect {
          case f: InstanceField[Pre] if !f.isFinal => f
        }
        ...
        classDeclarations.succeed(
          component,
          component.rewrite(invariant =
            Star(fieldPerms, rewriteDefault(component.invariant))
          ),
        )
