...
object EncodeForkJoin extends RewriterBuilder {
  ...
  case class RunnableMethodMissingError(expr: Expr[_]) extends UserError {
    ...
    override def text: String =
      expr.o.messageInContext("This class is missing a run method.")
  }
  ...
  case class ForkInstanceInvocation(fork: Fork[_])
    ...
    override def blame(error: InstanceInvocationFailure): Unit =
      error match {
        ...
        case PreconditionFailed(FailLeft +: _, _, _) =>
          fork.blame.blame(RunnableNotIdle(fork))
        case PreconditionFailed(FailRight +: _, failure, _) =>
          fork.blame.blame(RunnablePreconditionNotEstablished(fork, failure))
        case PreconditionFailed(_, _, _) => throw BlamePathError
        case ContextEverywhereFailedInPre(_, _) =>
          PanicBlame("fork method does not have c_e").blame(error)
      }
  }

  case class JoinInstanceInvocation(join: Join[_])
      extends Blame[InstanceInvocationFailure] {
    override def blame(error: InstanceInvocationFailure): Unit =
      error match {
        case InstanceNull(_) => join.blame.blame(JoinNull(join))
        case PreconditionFailed(_, _, _) =>
          join.blame.blame(RunnableNotRunning(join))
        case ContextEverywhereFailedInPre(_, _) =>
          PanicBlame("join method does not have c_e").blame(error)
      }
  }
}
...
case class EncodeForkJoin[Pre <: Generation]() extends Rewriter[Pre] {
  ...
  val idleToken: SuccessionMap[Class[Pre], InstancePredicate[Post]] =
    SuccessionMap()
  val runningToken: SuccessionMap[Class[Pre], InstancePredicate[Post]] =
    SuccessionMap()

  val forkMethod: SuccessionMap[Class[Pre], InstanceMethod[Post]] =
    SuccessionMap()
  val joinMethod: SuccessionMap[Class[Pre], InstanceMethod[Post]] =
    SuccessionMap()
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    decl match {
      ...
      case cons: Constructor[Pre] if currentClass.top.collectFirst {
      ...
      case m: RunMethod[Pre] =>
        ...
        idleToken(cls) = classDeclarations
          .declare(new InstancePredicate(Nil, None)(IdleToken(cls)))
        runningToken(cls) = classDeclarations
          .declare(new InstancePredicate(Nil, None)(RunningToken(cls)))
        forkMethod(cls) = classDeclarations.declare(
          new InstanceMethod(
            TVoid(),
            Nil,
            Nil,
            ...
            None,
            contract(
              blame = UnsafeDontCare.Satisfiability(
        ...
        joinMethod(cls) = classDeclarations.declare(
          new InstanceMethod(
            TVoid(),
            Nil,
            Nil,
  ...
  /** Performs a check on whether a class has a runnable method to perform an
    * expression that requires it. Throws an exception if there is no runnable
    * method.
    * @param expr
    *   \- the expression that requires the class to have a runnable method
    * @return
    *   The class of a given object
    * @throws \RunnableMethodMissingError
    *   if the class is detected not to have a Runnable method.
    */
  def getRunnableClass(expr: Expr[Pre]): Class[Pre] = {
    val classType = expr.t.asInstanceOf[TClass[Pre]]
    val check =
      classType.cls.decl.declarations.collectFirst { case _: RunMethod[Pre] =>
        () // return unit
      }.nonEmpty
    if (!check) { throw RunnableMethodMissingError(expr) }
    classType.cls.decl
  }
