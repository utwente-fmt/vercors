...
case object InlineApplicables extends RewriterBuilder {
  ...
  case class InlinedOrigin(definition: Origin, usages: Seq[Apply[_]])
      extends Origin {
    ...
    override def context: String =
      ...
      ) + Origin.HR + " In definition:\n" + Origin.HR + definition.context

    override def inlineContext: String =
      s"${definition.inlineContext} [inlined from] ${usages.head.o.inlineContext}"
  }
  ...
  case class Replacement[Pre](replacing: Expr[Pre], binding: Expr[Pre])(implicit
      o: Origin
  ) {
  ...
  case class Replacements[Pre](replacements: Seq[Replacement[Pre]]) {
    ...
    def expr(e: Expr[Pre])(implicit o: Origin): Expr[Pre] = {
      ...
      replacements.foldRight(replaced) { case (replacement, e) =>
        Let(replacement.withVariable, replacement.binding, e)
      }
    }

    def captureReturn(t: Type[Pre], body: Statement[Pre])(implicit
        o: Origin
    ): Expr[Pre] = {
...
case class InlineApplicables[Pre <: Generation]()
  ...
  override def dispatch(program: Program[Pre]): Program[Post] = {
    program.declarations.collect { case cls: Class[Pre] => cls }.foreach {
      cls => cls.declarations.foreach(classOwner(_) = cls)
    }
