...
case object EvaluationTargetDummy extends RewriterBuilder {
  ...
  override def desc: String =
    "Make a target to assign to for evaluations that contain no side effects, but must be well-formed regardless."

  private def EvaluationOrigin: Origin =
    Origin(Seq(PreferredName(Seq("evaluationDummy")), LabelContext("evaluate")))
}

case class EvaluationTargetDummy[Pre <: Generation]() extends Rewriter[Pre] {
  import EvaluationTargetDummy._

  /** Returns whether it is not possible for the expression to yield
    * verification failures. This is only for output prettification: it does not
    * need to be complete.
    */
  def infallible(e: Expr[Pre]): Boolean =
    e match {
      case Local(_) => true
      ...
      case Select(cond, whenTrue, whenFalse) =>
        infallible(cond) && infallible(whenTrue) && infallible(whenFalse)
      ...
      case LiteralMap(_, _, pairs) =>
        pairs.forall { case (k, v) => infallible(k) && infallible(v) }
      ...
      case NoPerm() | Null() | OptNone() | ReadPerm() | Void() | WritePerm() =>
        true
  ...
  override def dispatch(stat: Statement[Pre]): Statement[Post] =
    stat match {
      case Eval(e) if infallible(e) => Block(Nil)(stat.o)
      case Eval(other) =>
        val v = new Variable[Post](dispatch(other.t))(EvaluationOrigin)
        Scope(
          Seq(v),
          assignLocal(v.get(EvaluationOrigin), dispatch(other))(stat.o),
        )(stat.o)
