...
case object EncodeRangedFor extends RewriterBuilder {
  ...
  case class ForeachBoundOrigin(inner: Origin, name: String)
      extends PreferredNameOrigin
}
...
case class EncodeRangedFor[Pre <: Generation]() extends Rewriter[Pre] {
  ...
  override def dispatch(stat: Statement[Pre]): Statement[Post] =
    stat match {
      case rf @ RangedFor(
            iv @ IterVariable(iVar, fromExpr, toExpr),
            contract,
            body,
          ) =>
        ...
        val fromVar =
          new Variable[Post](TInt()(fromExpr.o))(
            ForeachBoundOrigin(fromExpr.o, "from")
          )
        val from =
          Local(fromVar.ref[Variable[Post]])(
            ForeachBoundOrigin(fromExpr.o, "from")
          )

        val toVar =
          new Variable[Post](TInt()(toExpr.o))(
            ForeachBoundOrigin(toExpr.o, "to")
          )
        val to =
          Local(toVar.ref[Variable[Post]])(ForeachBoundOrigin(toExpr.o, "to"))
  ...
  override def dispatch(contract: LoopContract[Pre]): LoopContract[Post] =
    (bounds.topOption, contract) match {
      ...
      case (Some((rf, iBounds)), ic: IterationContract[Pre]) =>
        ic.rewrite(context_everywhere =
          (iBounds &* dispatch(ic.context_everywhere))(rf.o)
        )
