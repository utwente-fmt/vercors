...
case object TupledQuantifiers extends RewriterBuilder {
  ...
  override def desc: String =
    "Collect the bindings of a quantifier into one binding: a tuple"

  case class CollectedBindingsOrigin(vs: Seq[Variable[_]], inner: Origin)
      extends Origin {
...
case class TupledQuantifiers[Pre <: Generation]() extends Rewriter[Pre] {
  val tupledVar: mutable.Map[Variable[Pre], (Variable[Post], Int)] = mutable
    .Map()

  def collect(
      bindings: Seq[Variable[Pre]]
  )(implicit o: Origin): Variable[Post] = {
    val collectedBinding =
      new Variable[Post](TTuple(bindings.map(_.t).map(dispatch)))(
        CollectedBindingsOrigin(bindings, o)
      )
