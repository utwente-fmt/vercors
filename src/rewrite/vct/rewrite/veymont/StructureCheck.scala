...
object StructureCheck extends RewriterBuilder {
  ...
  override def desc: String =
    "Check if program adheres to syntax of VeyMont input program"

  case class VeyMontStructCheckError(node: Node[_], msg: String)
      extends UserError { // SystemErrir fir unreachable errros
...
case class StructureCheck[Pre <: Generation]() extends Rewriter[Pre] {
    ...
    decl match {
      ...
      case m: InstanceMethod[Pre] =>
        if (inSeqProg.nonEmpty && m.args.nonEmpty)
          throw VeyMontStructCheckError(
            m,
            "Methods in seq_program cannot have any arguments!",
          )
        else if (inSeqProg.nonEmpty && m.returnType != TVoid[Pre]())
          throw VeyMontStructCheckError(
            m,
            "Methods in seq_program cannot have a non-void return type!",
          )
        else
          rewriteDefault(decl)
      case r: RunMethod[Pre] =>
        if (r.body.isEmpty)
          throw VeyMontStructCheckError(
            r,
            "Method run in seq_program needs to have a body!",
          )
        else
          rewriteDefault(decl)
  ...
  override def dispatch(prog: Program[Pre]): Program[Post] = {
    if (
      !prog.declarations.exists {
        case dcl: VeyMontSeqProg[Pre] =>
          if (dcl.threads.isEmpty)
            throw VeyMontStructCheckError(
              dcl,
              "A seq_program needs to have at least 1 thread, but none was found!",
            )
          else
            true
        case _ => false
      ...
      throw VeyMontStructCheckError(
        prog,
        "VeyMont requires a seq_program, but none was found!",
      )
    else
      rewriteDefault(prog)
  }
  ...
  private def checkMethodCall(
    ...
    expr match {
      case MethodInvocation(obj, _, args, _, _, _, _) =>
        obj match {
          case ThisSeqProg(_) =>
            if (args.isEmpty)
              rewriteDefault(st)
              ...
              throw VeyMontStructCheckError(
                st,
                "Calls to methods in seq_program cannot have any arguments!",
              )
          case DerefVeyMontThread(thread) =>
            val argderefs = args.flatMap(getDerefsFromExpr)
            val argthreads = argderefs.map(d =>
              getThreadDeref(
                d,
                VeyMontStructCheckError(
                  st,
                  "A method call on a thread object may only refer to a thread in its arguments!",
                ),
              )
            )
            if (argthreads.forall(_ == thread.decl))
              ...
              throw VeyMontStructCheckError(
                st,
                "A method call on a thread object may only refer to same thread in its arguments!",
              )
          case _ =>
            throw VeyMontStructCheckError(
              st,
              "This kind of method call is not allowed in seq_program",
            )
        }
