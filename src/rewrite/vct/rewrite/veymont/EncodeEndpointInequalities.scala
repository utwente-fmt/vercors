...
import vct.col.ast.{ApplicableContract, Choreography, Declaration, Endpoint, EndpointName, EndpointNameExpr, Expr, IterationContract, LoopContract, LoopInvariant, Program, SplitAccountedPredicate, UnitAccountedPredicate}
...
case class EncodeEndpointInequalities[Pre <: Generation]() extends Rewriter[Pre] with LazyLogging with VeymontContext[Pre]  {
  ...
  def makeInequalities(chor: Choreography[Pre]): Expr[Post] = {
    ...
    foldStar(makeInequalitySets(chor.endpoints).flatMap { case (endpoint, others) =>
      others.map { other =>
        EndpointNameExpr(EndpointName[Post](succ(endpoint))) !== EndpointNameExpr(EndpointName[Post](succ(other)))
      }
    })
  }
