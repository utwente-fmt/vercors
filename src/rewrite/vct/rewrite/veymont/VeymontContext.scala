...
import vct.col.ast.{Choreography, Class, Communicate, Endpoint, Program, Variable}
...
trait VeymontContext[Pre <: Generation] {
  object mappings {
    ...
    lazy val choreographyToCommunicates: Map[Choreography[Pre], Seq[Communicate[Pre]]] = choreographies.map { c =>
  ...
  lazy val choreographies: Seq[Choreography[Pre]] = mappings.program.collect { case c: Choreography[Pre] => c }.toIndexedSeq
