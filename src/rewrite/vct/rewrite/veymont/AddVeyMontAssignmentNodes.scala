...
object AddVeyMontAssignmentNodes extends RewriterBuilder {
  ...
  case class AddVeyMontAssignmentError(node: Node[_], msg: String)
      extends UserError {
  ...
  /** Utility function to get VeyMontThread from a Deref, and throw error err in
    * case of other dereferences
    * @param deref
    *   to be matched
    * @param err
    *   to be thrown if match unsuccessful
    * @tparam Pre
    *   type of expression
    * @return
    *   list of threads that are dereferenced in the expression
    */
  def getThreadDeref[Pre](
      deref: Deref[Pre],
      err: UserError,
  ): VeyMontThread[Pre] =
    deref.obj match {
...
case class AddVeyMontAssignmentNodes[Pre <: Generation]()
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    decl match {
      case dcl: VeyMontSeqProg[Pre] =>
        inSeqProg.having(()) { rewriteDefault(dcl) }
  ...
  def getAssignmentReceiver(target: Expr[Pre]): VeyMontThread[Pre] =
    target match {
      case Deref(obj, _) =>
        obj match {
          case DerefVeyMontThread(ref) =>
            ref.decl // target is correct ref to thread
          case _ =>
            throw AddVeyMontAssignmentError(
              target,
              "The target of this assignment must refer to a thread",
            )
        }
      case _ =>
        throw AddVeyMontAssignmentError(
          target,
          "The target of this assignment must be a dereference to a thread, e.g. someThread.someField",
        )
    }

  /** @param value
    *   is the expression to be checked on occurring method calls.
    * @return
    *   Whether all method calls of the assignment are method calls on thread
    *   objects
    */
  private def checkAssignmentMethodCalls(value: Expr[Pre]): Boolean = {
    !value.exists {
      case m: ProcedureInvocation[Pre] =>
        throw AddVeyMontAssignmentError(
          m,
          "Cannot call non-thread method in assignment!",
        )
  ...
  def createVeyMontAssignNode(
      receiver: VeyMontThread[Pre],
      a: Assign[Pre],
  ): Statement[Post] = {
    ...
    if (derefs.isEmpty)
      ...
      val thread =
        derefs.head.obj match {
          ...
          case _ =>
            throw AddVeyMontAssignmentError(
              a.value,
              "The value of this assignment is expected to be a Deref of a thread!",
            )
        }
      if (thread.ref.decl == receiver)
        new VeyMontAssignExpression[Post](succ(receiver), rewriteDefault(a))(
          a.o
        )
      else {
        val sender = getAssignmentSender(derefs.head)
        new VeyMontCommExpression[Post](
          succ(receiver),
          succ(sender),
          dispatch(derefs.head.ref.decl.t),
          rewriteDefault(a),
        )(a.o)
      }
    } else
      throw AddVeyMontAssignmentError(
        a.value,
        "The value of this assignment is not allowed to refer to multiple threads!",
      )
  }
  ...
  def getAssignmentSender(deref: Deref[Pre]): VeyMontThread[Pre] =
    getThreadDeref(
      ...
      AddVeyMontAssignmentError(
        deref,
        "Object identifiers in the value of this assignment can only refer to a thread!",
      ),
    )
