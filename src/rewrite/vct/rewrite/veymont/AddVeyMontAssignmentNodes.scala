...
object AddVeyMontAssignmentNodes extends RewriterBuilder {
  ...
  /**
   * Utility function to get Endpoint from a Deref, and throw error err in case of other dereferences
...
case class AddVeyMontAssignmentNodes[Pre <: Generation]() extends Rewriter[Pre] {
  ...
  def createVeyMontAssignNode(receiver: Endpoint[Pre], a: Assign[Pre]): Statement[Post] = {
  ...
  def getAssignmentSender(deref : Deref[Pre]): Endpoint[Pre] =
