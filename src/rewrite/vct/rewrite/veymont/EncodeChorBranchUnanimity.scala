...
case class EncodeChorBranchUnanimity[Pre <: Generation]()
    ...
    statement match {
      case branch @ ChorBranch(guards, yes, no) =>
        ...
        Block(Seq(
          assertions,
          Branch[Post](
            Seq((foldAnd(guards.map(rewriteGuard)), dispatch(yes))) ++ no.map {
              no => Seq((tt[Post], dispatch(no)))
            }.getOrElse(Nil)
          ),
        ))
      ...
      case loop @ ChorLoop(guards, contract, body) =>
        ...
        val finalLoop: Loop[Post] = Loop(
          ...
          maintainAssertions,
          currentLoop.having(loop) { dispatch(contract) },
          dispatch(body),
        )
  ...
  def rewriteGuard(guard: ChorGuard[Pre]): Expr[Post] =
    dispatch(guard.condition)
    ...
    (currentLoop.topOption, contract) match {
      case (Some(loop), inv: LoopInvariant[Pre]) =>
        ...
        inv.rewrite(invariant =
          dispatch(inv.invariant) &* allEqual(loop.guards.map(rewriteGuard))
        )
      case (Some(loop), inv @ IterationContract(requires, ensures, _)) =>
        implicit val o = contract.o
        inv.rewrite(
          requires =
            dispatch(requires) &* allEqual(loop.guards.map(rewriteGuard)),
