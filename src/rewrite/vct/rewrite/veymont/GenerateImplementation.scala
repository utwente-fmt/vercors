...
object GenerateImplementation extends RewriterBuilder {
  ...
  case class ParalleliseEndpointsError(node: Node[_], msg: String)
      extends UserError {
  ...
  private def ChannelFieldOrigin(
      channelName: String,
      assign: Statement[_],
  ): Origin = assign.o.where(name = channelName)
...
case class GenerateImplementation[Pre <: Generation]()
  ...
  val threadBuildingBlocks: ScopedStack[ThreadBuildingBlocks[Pre]] =
    ScopedStack()
  val threadClassSucc: SuccessionMap[Endpoint[Pre], Class[Post]] =
    SuccessionMap()
  val threadMethodSucc
      : SuccessionMap[(Endpoint[Pre], ClassDeclaration[Pre]), InstanceMethod[
        Post
      ]] = SuccessionMap()
  ...
  lazy val choreographies: Seq[Choreography[Pre]] = program.declarations
    .collect { case chor: Choreography[Pre] => chor }
  ...
  def isEndpointClass(c: Class[Pre]): Boolean =
    endpointClassToEndpoint.contains(c)
  def choreographyOf(c: Class[Pre]): Choreography[Pre] =
    endpointToChoreography(endpointClassToEndpoint(c))
