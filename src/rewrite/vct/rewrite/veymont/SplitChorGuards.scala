...
case class SplitChorGuards[Pre <: Generation]() extends Rewriter[Pre] {
  ...
  override def dispatch(decl: Declaration[Pre]): Unit =
    decl match {
      case prog: Choreography[Pre] =>
        currentProg.having(prog) { rewriteDefault(prog) }
  ...
  )(implicit blame: Blame[SeqBranchFailure], o: Origin): ChorBranch[Post] =
    branches match {
      ...
      case (e, s) +: (otherYes +: branches) =>
        ChorBranch(
          inferSeqGuard(e),
          dispatch(s),
          Some(unfoldBranch(otherYes +: branches)),
        )(blame)
      case _ => ???
    }
  ...
  def inferSeqGuard(e: Expr[Pre]): Seq[ChorGuard[Post]] = {
    ...
    pointed.map {
      case (Some(endpoint), expr) =>
        EndpointGuard[Post](succ(endpoint), dispatch(expr))(expr.o)
