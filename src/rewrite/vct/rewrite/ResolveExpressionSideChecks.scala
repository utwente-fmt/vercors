...
case object ResolveExpressionSideChecks extends RewriterBuilder {
  ...
  override def desc: String =
    "Encode with/then annotations that are only the evaluation of an otherwise pure expression."

  case class EvalCheckFunction(preferredName: String = "unknown")
      extends Origin {
    override def context: String =
      "At: [Function generated to check the evaluation of an expression is ok]"
    override def inlineContext: String =
      "[Function generated to check the evaluation of an expression is ok]"
    override def shortPosition: String = "generated"
  }
}

case class ResolveExpressionSideChecks[Pre <: Generation]()
  ...
  lazy val withEval: Function[Post] = {
    ...
    val checkValue =
      new Variable[Post](TAny())(EvalCheckFunction("checkedValue"))
    ...
    e match {
      case With(Eval(check), value) =>
        FunctionInvocation[Post](
          withEval.ref,
          Seq(dispatch(check), dispatch(value)),
      ...
      case Then(value, Eval(check)) =>
        FunctionInvocation[Post](
          thenEval.ref,
          Seq(dispatch(value), dispatch(check)),
          Seq(dispatch(value.t)),
          Nil,
          Nil,
