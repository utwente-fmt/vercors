...
case object ResolveExpressionSideChecks extends RewriterBuilder {
  ...
  private def EvalCheckFunction(preferredName: String = "unknown"): Origin =
    Origin(Seq(PreferredName(Seq(preferredName)), LabelContext("side check")))
}
...
case class ResolveExpressionSideChecks[Pre <: Generation]()
  ...
  lazy val withEval: Function[Post] = {
    ...
    val checkValue =
      new Variable[Post](TAnyValue())(EvalCheckFunction("checkedValue"))
