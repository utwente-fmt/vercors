...
case class Extract[G]() {
  ...
  private val map = mutable
    .ListMap[FreeVariables.FreeVariable[G], Variable[G]]()
  ...
  private def updateExprs(node: Node[G]): ListMap[Expr[G], Expr[G]] =
    FreeVariables.freeVariables(node).collect {
      case free @ ReadFreeVar(v) =>
        read += v
        v -> Local(
          map.getOrElseUpdate(free, new Variable(extract(v.t))(v.ref.decl.o))
            .ref[Variable[G]]
        )(ExtractOrigin(""))
      case WriteFreeVar(v) =>
        write += v
      ...
      case free @ FreeThisObject(t) =>
        t -> Local(
          map.getOrElseUpdate(
            free,
            new Variable(extract(TClass(t.cls)))(ExtractOrigin("this")),
          ).ref[Variable[G]]
        )(ExtractOrigin(""))
  ...
  private def updateTypes(node: Node[G]): ListMap[TVar[G], Type[G]] =
    FreeVariables.freeVariables(node).collect { case free @ ReadTypeVar(v) =>
      v -> TVar(
        map.getOrElseUpdate(free, new Variable(v.ref.decl.t)(v.ref.decl.o))
          .ref[Variable[G]]
      )(ExtractOrigin(""))
    }.to(ListMap)
