...
case object Comparison {
  ...
  def of[G](comparison: Expr[G]): Option[(Expr[G], Comparison, Expr[G])] =
  ...
  def matching[G](
      comparand: Expr[G],
      comparison: Expr[G],
  ): Option[(Comparison, Expr[G])] =
    of(comparison) match {
      case Some((left, comp, right)) =>
        if (left == comparand)
          Some((comp, right))
        else if (right == comparand)
          Some((comp.flip, left))
        else
          None
      case None => None
...
case class Comparison(
  ...
  def flip: Comparison = Comparison(less = greater, eq = eq, greater = less)

  def make[G](left: Expr[G], right: Expr[G])(implicit o: Origin): Expr[G] =
    this match {
