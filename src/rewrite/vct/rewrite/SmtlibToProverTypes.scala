...
case object SmtlibToProverTypes extends RewriterBuilder {
  ...
  override def desc: String =
    "Encode smtlib types and functions into their stringified counterpart"
  ...
  case class NotRepresentable(t: Type[_]) extends UserError {
    ...
    override def text: String =
      t.o.messageInContext(
        "This type does not have a stable representation on the smt level yet."
      )
  }
}

case class SmtlibToProverTypes[Pre <: Generation]() extends Rewriter[Pre] {
  def smtTypeString(t: Type[Pre]): String =
    t match {
      ...
      case TMap(key, value) =>
        "$Map<" + smtTypeString(key) + "~_" + smtTypeString(value) + ">"
      case TBool() => "Bool"
      case TRef() => "$Ref"
      case TInt() => "Int"
      case TRational() => "$Perm"
      case TAxiomatic(adt, args) =>
        adt.decl.o.getPreferredNameOrElse().ucamel + "<" +
          args.map(smtTypeString).mkString("~_") + ">"
      case TProverType(ref) =>
        ref.decl.interpretation.collectFirst { case (SmtLib(), int) => int }.get
      case TSmtlibArray(index, value) =>
        s"(Array ${index.map(smtTypeString).mkString(" ")} ${smtTypeString(value)})"
      case TSmtlibBitVector(size) => s"(_ BitVec $size)"
      case TSmtlibRoundingMode() => "RoundingMode"
      case TSmtlibFloatingPoint(exponentBits, mantissaAndSignBits) =>
        s"(_ FloatingPoint $exponentBits $mantissaAndSignBits)"
  ...
  val declaredFunc
      : mutable.Map[(String, Seq[Type[Pre]]), ProverFunction[Post]] = mutable
    .Map()

  def getType(t: Type[Pre]): TProverType[Post] = {
    val asText = smtTypeString(t)
    TProverType(
      declaredType.getOrElseUpdate(
        asText, {
          implicit val o: Origin = t.o
          globalDeclarations
            .declare(new ProverType[Post](Seq(SmtLib[Post]() -> asText)))
        },
      ).ref
    )
  }
      ...
      case e: SmtlibExpr[Pre] =>
        ...
        e match {
          ...
          case SmtlibExtract(
            ...
            getExpr(
              e,
              s"(_ extract $inclusiveEndIndexFromRight $startIndexFromRight)",
              bv,
            )
          ...
          case SmtlibFp(sign, exponent, mantissa) =>
            getExpr(e, "fp", sign, exponent, mantissa)
          ...
          case SmtlibFpFma(left, right, addend) =>
            getExpr(e, "fp.fma", left, right, addend)
          ...
          case SmtlibFpRoundToIntegral(arg) =>
            getExpr(e, "fp.roundToIntegral", arg)
          ...
          case SmtlibFpFromUInt(bv, exponentBits, mantissaAndSignBits) =>
            getExpr(
              e,
              s"(_ to_fp_unsigned $exponentBits $mantissaAndSignBits)",
              bv,
            )
          case SmtlibFpToReal(arg) => getExpr(e, "fp.to_real", arg)
          case SmtlibFpToSInt(arg, bits) =>
            getExpr(e, s"(_ fp.to_sbv $bits)", arg)
          ...
          case SmtlibLiteralString(data) =>
            ...
            val content = data.codePoints().toScala(Seq).map {
              case c if 0x20 <= c && c <= 0x7e && c != 0x5c =>
          ...
          case SmtlibStrPrefixOf(left, right) =>
            getExpr(e, "str.prefixof", left, right)
          case SmtlibStrSuffixOf(left, right) =>
            getExpr(e, "str.suffixof", left, right)
          case SmtlibStrContains(left, right) =>
            getExpr(e, "str.contains", left, right)
          case SmtlibStrIndexOf(haystack, needle, fromIndex) =>
            getExpr(e, "str.indexof", haystack, needle, fromIndex)
          ...
          case SmtlibStrReplaceRe(haystack, re, replacement) =>
            getExpr(e, "str.replace_re", haystack, re, replacement)
          case SmtlibStrReplaceReAll(haystack, re, replacement) =>
            getExpr(e, "str.replace_re_all", haystack, re, replacement)
          ...
          case SmtlibReRepeatRange(from, to, arg) =>
            getExpr(e, s"(_ re.loop $from $to)", arg)
          ...
          case Z3ArrayConst(domain, codomain, value) =>
            getExpr(
              e,
              s"(as const ${smtTypeString(TSmtlibArray(domain, codomain))})",
              value,
            )
          case Z3ArrayOfFunction(ref) =>
            ...
            getExpr(
              e,
              s"(_ as-array ${ref.ref.decl.o.getPreferredNameOrElse().camel})",
            )
          case Z3ArrayMap(ref, args) =>
            getExpr(
              e,
              s"(_ map ${ref.ref.decl.o.getPreferredNameOrElse().camel})",
              args: _*
            )
          case Z3SeqEmpty(elementType) =>
            getExpr(e, s"(as seq.empty (Seq ${smtTypeString(elementType)}))")
          ...
          case Z3SeqExtract(seq, offset, len) =>
            getExpr(e, "seq.extract", seq, offset, len)
          ...
          case Z3SeqContains(seq, subseq) =>
            getExpr(e, "seq.contains", seq, subseq)
          ...
          case Z3SeqMapI(f, offset, seq) =>
            getExpr(e, "seq.mapi", f, offset, seq)
          case Z3SeqFoldl(f, base, seq) => getExpr(e, "seq.foldl", f, base, seq)
          case Z3SeqFoldlI(f, offset, base, seq) =>
            getExpr(e, "seq.foldli", f, offset, base, seq)
          case Z3TransitiveClosure(ref, args) =>
            getExpr(
              e,
              s"(_ transitive-closure ${ref.ref.decl.o.getPreferredNameOrElse().camel})",
              args: _*
            )
        }
      case other => rewriteDefault(other)
