...
import vct.col.origin.{
  ...
  Blame,
  CallableFailure,
  InterpretedOriginVariable,
  KernelBarrierInconsistent,
  KernelBarrierInvariantBroken,
  KernelBarrierNotEstablished,
  KernelPostconditionFailed,
  KernelPredicateNotInjective,
  Origin,
  PanicBlame,
  ParBarrierFailure,
  ParBarrierInconsistent,
  ParBarrierInvariantBroken,
  ParBarrierMayNotThrow,
  ParBarrierNotEstablished,
  ParBlockContractFailure,
  ParBlockFailure,
  ParBlockMayNotThrow,
  ParBlockPostconditionFailed,
  ParPreconditionFailed,
  ParPredicateNotInjective,
...
import vct.col.resolve.ctx.{
  ...
  CNameTarget,
  RefADTFunction,
  RefAxiomaticDataType,
  RefCFunctionDefinition,
  RefCGlobalDeclaration,
  RefCLocalDeclaration,
  RefCParam,
  RefCudaBlockDim,
  RefCudaBlockIdx,
  RefCudaGridDim,
...
case class LangCToCol[Pre <: Generation](rw: LangSpecificToCol[Pre])
  ...
  case class CudaIndexVariableOrigin(dim: RefCudaVecDim[_]) extends Origin {
    override def preferredName: String = dim.vec.name + dim.name.toUpperCase

    override def context: String =
      s"At: [Variable for dimension ${dim.name} of ${dim.vec.name}]"
    override def inlineContext: String =
      s"[Variable for dimension ${dim.name} of ${dim.vec.name}]"
  ...
  class CudaVec(ref: RefCudaVec[Pre])(implicit val o: Origin) {
    val indices: ListMap[RefCudaVecDim[Pre], Variable[Post]] = ListMap(
      Seq(RefCudaVecX[Pre](ref), RefCudaVecY[Pre](ref), RefCudaVecZ[Pre](ref))
        .map(dim =>
          dim -> new Variable[Post](TInt())(CudaIndexVariableOrigin(dim))
        ): _*
    )
  }
  ...
  def rewriteGPUParam(
    ...
    val varO = InterpretedOriginVariable(
      C.getDeclaratorInfo(cParam.declarator).name,
      cParam.o,
    )
  ...
  def rewriteFunctionDef(func: CFunctionDefinition[Pre]): Unit = {
    ...
    val namedO = InterpretedOriginVariable(
      C.getDeclaratorInfo(func.declarator).name,
      func.o,
    )
    val proc =
      ...
      Starall(
        ...
        Implies(
          foldAnd(filteredIdx.map { case (idx, dim) =>
            const[Post](0) <= idx.get && idx.get < dim.get
          }),
          body,
        ),
      )(blame)
  ...
  def declareSharedMemory(): (Seq[Variable[Post]], Seq[Statement[Post]]) =
    rw.variables.collect {
      ...
      dynamicSharedMemNames.foreach(d => {
        ...
        val varO: Origin = InterpretedOriginVariable(
          s"${C.getDeclaratorInfo(getCDecl(d)).name}_size",
          o,
        )
  ...
  def kernelProcedure(
    ...
    cudaCurrentBlockDim.having(blockDim) {
      cudaCurrentGridDim.having(gridDim) {
        ...
        val (sharedMemSizes, sharedMemInit: Seq[Statement[Post]]) =
          declareSharedMemory()
        ...
        val nonZeroThreads: Expr[Post] =
          foldStar(
            (blockDim.indices.values ++ gridDim.indices.values)
              .map(v => Less(IntegerValue(0)(o), v.get(o))(o)).toSeq
          )(o)
                    ...
                    ParBlock(
                      ...
                      iters =
                        threadIdx.indices.values.zip(blockDim.indices.values)
                          .map { case (index, dim) =>
                            IterVariable(index, const(0), dim.get)
                          }.toSeq,
  ...
  def isShared(s: Statement[Pre]): Boolean = {
    ...
    val varO = InterpretedOriginVariable(
      C.getDeclaratorInfo(init.decl).name,
      decl.o,
    )
  ...
  def isNumeric(t: Type[Pre]): Boolean =
    t match {
      ...
      case CPrimitiveType(specs) =>
        specs.collectFirst { case CSpecificationType(_: NumericType[Pre]) => }
          .nonEmpty
      case _ => false
    }
  ...
  def cudaGlobalThreadId(global: GlobalThreadId[Pre]): Expr[Post] = {
    ...
    Plus(
      Plus(
        Mult(
          Mult(
            getCudaGroupThread(2, o),
            getCudaGroupSize(1, o),
          ), // (get_global_id(2) * get_global_size(1)
          getCudaGroupSize(0, o),
        ), //   * get_global_size(0)
        Mult(getCudaGroupThread(1, o), getCudaGroupSize(0, o)),
