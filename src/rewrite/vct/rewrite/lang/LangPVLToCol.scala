...
case object LangPVLToCol {
  case class ModelConstructorNotSupported() extends SystemError {
    override def text: String =
      "VerCors attempted to get the constructor of a model, but this is not yet supported"
  }
}
...
case class LangPVLToCol[Pre <: Generation](
  ...
  val pvlDefaultConstructor: SuccessionMap[Class[Pre], Constructor[Post]] =
    SuccessionMap()
  val pvlConstructor: SuccessionMap[PVLConstructor[Pre], Constructor[Post]] =
    SuccessionMap()

  def constructorSucc(
      ref: PVLConstructorTarget[Pre]
  ): Ref[Post, Constructor[Post]] =
    ref match {
  ...
  def maybeDeclareDefaultConstructor(cls: Class[Pre]): Unit = {
    if (cls.decls.collectFirst { case _: PVLConstructor[Pre] => () }.isEmpty) {
      ...
      val checkRunnable =
        cls.decls.collectFirst { case _: RunMethod[Pre] => () }.nonEmpty

      pvlDefaultConstructor(cls) = rw.classDeclarations.declare(
        new Constructor[Post](
          rw.succ(cls),
          Nil,
          Nil,
  ...
  def local(local: PVLLocal[Pre]): Expr[Post] = {
    ...
    local.ref.get match {
      ...
      case endpoint: RefPVLEndpoint[Pre] =>
        rw.veymont.rewriteEndpointUse(endpoint, local)
    }
  }
  ...
  def branch(branch: PVLBranch[Pre]): Statement[Post] =
    Branch(branch.branches.map { case (e, s) =>
      (rw.dispatch(e), rw.dispatch(s))
    })(branch.o)

  def loop(loop: PVLLoop[Pre]): Statement[Post] =
    loop match {
  ...
  def rewriteMainMethod(main: VeSUVMainMethod[Pre]): Unit = {
    ...
    val contract: ApplicableContract[Post] =
      ApplicableContract(
        empty_pred,
        empty_pred,
        BooleanValue(value = true),
