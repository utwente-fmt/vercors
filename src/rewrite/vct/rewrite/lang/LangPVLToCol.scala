...
import vct.col.resolve.ctx.{
  ...
  ImplicitDefaultPVLConstructor,
  PVLBuiltinInstanceMethod,
  RefADTFunction,
...
case class LangPVLToCol[Pre <: Generation](rw: LangSpecificToCol[Pre])
  ...
  val pvlDefaultConstructor: SuccessionMap[Class[Pre], Procedure[Post]] =
    SuccessionMap()
  val pvlConstructor: SuccessionMap[PVLConstructor[Pre], Procedure[Post]] =
    SuccessionMap()
  ...
  def rewriteConstructor(cons: PVLConstructor[Pre]): Unit = {
    ...
    pvlConstructor(cons) = rw.globalDeclarations
      .declare(withResult((result: Result[Post]) =>
        new Procedure[Post](
          ...
          typeArgs = Nil,
          body =
            rw.currentThis.having(resVar.get) {
              cons.body.map(body =>
                Scope(
                  Seq(resVar),
                  Block(Seq(
                    assignLocal(
                      resVar.get,
                      NewObject[Post](rw.succ(rw.currentClass.top)),
                    ),
                    rw.dispatch(body),
                    Return(resVar.get),
                  )),
  ...
  def maybeDeclareDefaultConstructor(cls: Class[Pre]): Unit = {
    if (
      cls.declarations.collectFirst { case _: PVLConstructor[Pre] => () }
        .isEmpty
    ) {
      ...
      val defaultBlame = PanicBlame(
        "The postcondition of a default constructor cannot fail."
      )

      val checkRunnable =
        cls.declarations.collectFirst { case _: RunMethod[Pre] => () }.nonEmpty

      pvlDefaultConstructor(cls) = rw.globalDeclarations
        .declare(withResult((result: Result[Post]) =>
          new Procedure(
            t,
            Nil,
  ...
  def local(local: PVLLocal[Pre]): Expr[Post] = {
    ...
    local.ref.get match {
      ...
      case RefField(decl) =>
        Deref[Post](rw.currentThis.top, rw.succ(decl))(local.blame)
      case RefVeyMontThread(decl) => DerefVeyMontThread[Post](rw.succ(decl))
      case RefEnum(decl) => throw NotAValue(local)
      case RefEnumConstant(enum, constant) =>
        EnumUse(rw.succ(enum.get), rw.succ(constant))
    }
  }
  ...
  def deref(deref: PVLDeref[Pre]): Expr[Post] = {
    ...
    deref.ref.get match {
      ...
      case RefField(decl) =>
        Deref[Post](rw.dispatch(deref.obj), rw.succ(decl))(deref.blame)
      case RefEnumConstant(_, const) =>
        deref.obj.t match {
          case TNotAValue(RefEnum(enum)) =>
            EnumUse(rw.succ(enum), rw.succ(const))
        }
    }
  }
  ...
  def invocation(inv: PVLInvocation[Pre]): Expr[Post] = {
    ...
    inv.ref.get match {
      ...
      case PVLBuiltinInstanceMethod(f) => rw.dispatch(f(obj.get)(args))
  ...
  def newClass(inv: PVLNew[Pre]): Expr[Post] = {
    ...
    inv.ref.get match {
      ...
      case ImplicitDefaultPVLConstructor() =>
        ProcedureInvocation[Post](
          pvlDefaultConstructor.ref(t.asInstanceOf[TClass[Pre]].cls.decl),
          args.map(rw.dispatch),
          Nil,
          Nil,
          givenMap.map { case (Ref(v), e) => (rw.succ(v), rw.dispatch(e)) },
