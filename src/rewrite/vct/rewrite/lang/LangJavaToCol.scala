...
case object LangJavaToCol {
  ...
  case class JavaLocalOrigin(locals: JavaLocalDeclaration[_], idx: Int)
      extends Origin {
  ...
  case class JavaAnnotationMethodOrigin(method: JavaAnnotationMethod[_])
      extends Origin {
    override def preferredName: String = method.name
    override def shortPosition: String = method.o.shortPosition
    override def context: String = method.o.context
  ...
  case class JavaInstanceClassOrigin(cls: JavaClassOrInterface[_])
      extends Origin {
  ...
  case class JavaStaticsClassOrigin(cls: JavaClassOrInterface[_])
      extends Origin {
  ...
  case class JavaStaticsClassSingletonOrigin(cls: JavaClassOrInterface[_])
      extends Origin {
...
case class LangJavaToCol[Pre <: Generation](rw: LangSpecificToCol[Pre])
  ...
  val javaMethod: SuccessionMap[JavaMethod[Pre], InstanceMethod[Post]] =
    SuccessionMap()
  val javaConstructor: SuccessionMap[JavaConstructor[Pre], Procedure[Post]] =
    SuccessionMap()
  ...
  def makeJavaClass(
    ...
    declsDefault.foreach {
      case cons: JavaConstructor[Pre] =>
        ...
        rw.labelDecls.scope {
          javaConstructor(cons) = rw.globalDeclarations
    ...
    if (BipData.get(param).isDefined) { rw.bip.rewriteParameter(param) }
      ...
      javaParamSuccessor(param) = rw.variables
        .declare(new Variable(rw.dispatch(param.t))(
          SourceNameOrigin(param.name, param.o)
        ))
    }
              ...
              cls match {
                case cls: JavaClass[Pre] if BipComponent.get(cls).isDefined =>
                  val inputConstructorRefs: Seq[Ref[Post, Procedure[Post]]] =
                    cls.decls.collect({ case c: JavaConstructor[Pre] => c })
                      .map(javaConstructor.ref(_))
                  val syntheticConstructorRefs
                      : Seq[Ref[Post, Procedure[Post]]] =
                    javaDefaultConstructor.get(cls) match {
                      case Some(constructor) =>
                        Seq(
                          javaConstructor
                            .ref[Post, Procedure[Post]](constructor)
                        )
                      case None => Seq()
                    }
                  rw.bip.generateComponent(
                    cls,
                    inputConstructorRefs ++ syntheticConstructorRefs,
                  )
                case _ =>
              }
            }._1,
  ...
  def deref(deref: JavaDeref[Pre]): Expr[Post] = {
    ...
    deref.ref.get match {
      ...
      case RefModelField(decl) =>
        ModelDeref[Post](rw.dispatch(deref.obj), rw.succ(decl))(deref.blame)
      ...
      case RefEnumConstant(_, constant) =>
        deref.obj.t match {
  ...
  def invocation(inv: JavaInvocation[Pre]): Expr[Post] = {
    ...
    inv.ref.get match {
      ...
      case BuiltinInstanceMethod(f) => rw.dispatch(f(obj.get)(args))
    }
  }
  ...
  def newClass(inv: JavaNewClass[Pre]): Expr[Post] = {
    ...
    inv.ref.get match {
      ...
      case ImplicitDefaultJavaConstructor() =>
        ...
        val ref =
          new LazyRef[Post, Procedure[Post]](javaConstructor(
  ...
  def classType(t: JavaTClass[Pre]): Type[Post] =
    t.ref.decl match {
      case classOrInterface: JavaClassOrInterface[Pre] =>
        TClass(javaInstanceClassSuccessor.ref(classOrInterface))
    }
}
