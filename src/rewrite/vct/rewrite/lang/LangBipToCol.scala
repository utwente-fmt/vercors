...
import vct.col.origin.{
  BipComponentInvariantNotEstablished,
  BipConstructorFailure,
...
case object LangBipToCol {
  ...
  case class ImproperConstructor(c: JavaConstructor[_], msg: String)
      extends UserError {
  ...
  private def BipConstructorOrigin(
      cls: JavaClassOrInterface[_],
      c: JavaConstructor[_],
  ): Origin = { c.o.where(prefix = "constructor") }

  case class UntangleBipConstructorFailure(constructor: JavaConstructor[_])
      extends Blame[BipConstructorFailure] {
    override def blame(error: BipConstructorFailure): Unit =
      error match {
        case err: NontrivialUnsatisfiable =>
          constructor.contract.blame.blame(err)
        case err: BipComponentInvariantNotEstablished =>
          constructor.blame.blame(err)
        case err: BipStateInvariantNotEstablished =>
          constructor.blame.blame(err)
      }
  }
}
...
case class LangBipToCol[Pre <: Generation](rw: LangSpecificToCol[Pre])
  ...
  def rewriteTransition(
    ...
    val trans =
      ...
      )(annotation.blame)(m.o.where(name = m.name))
  ...
  def rewriteConstructor(
      constructor: JavaConstructor[Pre],
      annotation: JavaAnnotation[Pre],
      data: jad.BipComponent[Pre],
      generateInit: Expr[Post] => Statement[Post],
  ): Unit = {
    implicit val o: Origin = constructor.o
    val contractWithoutRequires =
      constructor.contract
        .copy(requires = UnitAccountedPredicate[Pre](tt))(blame =
          constructor.contract.blame
        )(o = constructor.o)
    if (contractWithoutRequires.nonEmpty) {
      throw ImproperConstructor(
        constructor,
        "Only precondition is allowed on JavaBIP component constructors",
      )
    }

    logger.debug(
      s"JavaBIP component constructor for ${constructor.o.inlineContextText}"
    )
    rw.currentThis.having(ThisObject(
      rw.java.javaInstanceClassSuccessor.ref(rw.java.currentJavaClass.top)
    )) {
      rw.labelDecls.scope {
        rw.classDeclarations.declare(
          new BipConstructor(
            args =
              rw.variables.collect { constructor.parameters.map(rw.dispatch) }
                ._1,
            body = Block(Seq(
          ...
          )(UntangleBipConstructorFailure(constructor))(
            BipConstructorOrigin(rw.java.currentJavaClass.top, constructor)
          )
        )
      }
    }
  ...
  def rewriteGuard(
      method: JavaMethod[Pre],
      annotation: JavaAnnotation[Pre],
      guard: jad.BipGuard[Pre],
  ): Unit = {
    if (method.returnType != TBool[Pre]()) {
      throw LangBipToCol.WrongGuardReturnType(method)
    }

    javaMethodSuccGuard(method) = rw.classDeclarations.declare(
      new BipGuard[Post](
  ...
  def rewriteOutgoingData(
      method: JavaMethod[Pre],
      annotation: JavaAnnotation[Pre],
      data: jad.BipData[Pre],
  ): Unit = {
    if (!jad.BipPure.isPure(method)) { throw ImpureData(method); }
    ...
    javaMethodSuccOutgoingData(method) = rw.classDeclarations.declare(
      new BipOutgoingData(
        ...
        BipDataOrigin(rw.java.namespace.top, currentClass(), data)
      )
    )
