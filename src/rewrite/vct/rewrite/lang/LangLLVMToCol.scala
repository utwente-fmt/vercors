...
case class LangLLVMToCol[Pre <: Generation](rw: LangSpecificToCol[Pre])
    ...
    val procedure = rw.labelDecls.scope {
      rw.globalDeclarations.declare(
        new Procedure[Post](
          ...
          body =
            if (func.pure)
              Some(GotoEliminator(func.functionBody match {
                case scope: Scope[Pre] => scope
              }).eliminate())
            else
              Some(rw.dispatch(func.functionBody)),
  ...
  def rewriteGlobal(decl: LlvmGlobal[Pre]): Unit = {
    ...
    rw.globalDeclarations.declare(decl.data match {
      case Some(data) =>
        data match {
  ...
  def result(ref: RefLlvmFunctionDefinition[Pre])(implicit
      o: Origin
    ...
    val labelDeclMap: Map[LabelDecl[Pre], Label[Pre]] =
      bodyScope.body match {
        case block: Block[Pre] =>
          block.statements.map { case label: Label[Pre] => (label.decl, label) }
            .toMap
      }
      ...
      label.stat match {
        case block: Block[Pre] =>
          block.statements.last match {
            ...
            case _: Return[Pre] =>
              rw.dispatch(block) match { case block: Block[Post] => block }
