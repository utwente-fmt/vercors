...
case class LangLLVMToCol[Pre <: Generation](rw: LangSpecificToCol[Pre])
  ...
  def rewriteFunctionInvocation(
    ...
    implicit val o: Origin = inv.o
    val fDecl = inv.ref.decl

    val newArgs = inv.args.zip(fDecl.llvmArgs).map { case (e, arg) =>
      val c = addCast(e, arg.v)
      if (arg.byValType.nonEmpty)
        DerefPointer(c)(InvocationBlameAdapter(inv.blame))(arg.o)
      else
    ...
    if (!fDecl.isPredicate) {
      ...
      val newGiven = inv.givenMap.map { case (Ref(v), e) =>
        (rw.succ[Variable[Post]](v), addCast(e, v))
      }

      val newYields =
        fDecl.contract match {
          case c: PallasFunctionContract[Pre] =>
            val byVals = c.llvmYieldsArgs.filter(_.isByVal).map(_.v).toSet
            inv.yields.map { case (e, Ref(v)) =>
              val target =
                if (byVals.contains(v)) {
                  // Byval yields-arg
          ...
          case c: VCLLVMFunctionContract[Pre] => Seq.empty
        }
