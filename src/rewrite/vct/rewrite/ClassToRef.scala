...
case class ClassToRef[Pre <: Generation]() extends Rewriter[Pre] {
      ...
      body = Some(
        ((sub.get === const(0)) ==> tt) &&
          foldAnd(typeNumberStore.map { case (cls, subNum) =>
            val supNums = (cls +: cls.transSupportArrows.map(_._2)).distinct
              .map(typeNumber)
    ...
    stat match {
      case Assign(Local(Ref(v)), NewObject(Ref(cls))) =>
        ...
        Block(Seq(
          SilverNewRef[Post](
            succ(v),
            cls.declarations.collect { case field: InstanceField[Pre] =>
              fieldSucc.ref(field)
            },
          ),
          Inhale(
  ...
  override def dispatch(e: Expr[Pre]): Expr[Post] =
    e match {
      ...
      case ThisObject(_) => Local[Post](diz.top.ref)(e.o)
