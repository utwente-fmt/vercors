...
case object RewriteTriggerADTFunctions extends RewriterBuilder {
  ...
  override def desc: String =
    "Rewrite adt forwarding functions in triggers to axiomatic ones."
}

case class RewriteTriggerADTFunctions[Pre <: Generation]()
  ...
  override def dispatch(e: Expr[Pre]): Expr[Post] =
    e match {
      case f @ Forall(_, triggers, _) =>
        f.rewrite(triggers =
          inTrigger.having(()) { triggers.map(_.map(dispatch)) }
        )
