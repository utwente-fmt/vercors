...
case class ParBlockEncoder[Pre <: Generation]() extends Rewriter[Pre] {
  ...
  def requires(region: ParRegion[Pre], nonEmpty: Boolean)(implicit
      o: Origin
  ): Expr[Post] =
    region match {
  ...
  def ensures(region: ParRegion[Pre], nonEmpty: Boolean)(implicit
      o: Origin
  ): Expr[Post] =
    region match {
      case ParParallel(regions) =>
  ...
  def constantExpression[A](
      e: Expr[A],
      nonConstVars: Set[Variable[A]],
  ): Boolean =
    e match {
      case _: Constant[_, _] => true
      case op: BinExpr[_] =>
        constantExpression(op.left, nonConstVars) &&
        constantExpression(op.right, nonConstVars)
      case Local(v) => !nonConstVars.contains(v.decl)
      case _ => false
    }

  def applyFunc[A, B](f: A => Option[Set[B]], xs: Iterable[A]): Option[Set[B]] =
    xs.foldLeft(Some(Set()): Option[Set[B]]) { case (res, x) =>
      res.flatMap(r => f(x).map(s => r ++ s))
    }
   ...
   */
  def scanForAssign(s: Statement[Pre]): Option[Set[Variable[Pre]]] =
    s match {
      ...
      case Switch(expr, body) =>
        combine(scanForAssignE(expr), scanForAssign(body))
      case Loop(init, cond, update, _, body) =>
        scanForAssign(init).flatMap(r1 =>
          scanForAssignE(cond).flatMap(r2 =>
            scanForAssign(update).flatMap(r3 =>
              scanForAssign(body).map(r4 => r1 ++ r2 ++ r3 ++ r4)
            )
          )
        )
      case Assign(target, value) =>
        combine(scanAssignTarget(target), scanForAssignE(value))
  ...
  def scanForAssignP(par: ParRegion[Pre]): Option[Set[Variable[Pre]]] =
    par match {
  ...
  def scanForAssignE(e: Expr[Pre]): Option[Set[Variable[Pre]]] =
    e match {
      case _: Constant[Pre, _] => Some(Set())
      case Local(_) => Some(Set())
      case op: BinExpr[Pre] =>
        combine(scanForAssignE(op.left), scanForAssignE(op.right))
      case ProcedureInvocation(_, args, outArgs, _, givenMap, yields) =>
        ...
        applyFunc(scanAssignTarget, args.filterNot(e => isPrimitive(e.t)))
          .flatMap(r1 =>
            scanIterE(givenMap.map(_._2)).flatMap(r2 =>
              scanIterE(outArgs).flatMap(r3 =>
                scanIterE(yields.map(_._1)).map(r4 => r1 ++ r2 ++ r3 ++ r4)
              )
            )
          )
      case FunctionInvocation(_, args, _, givenMap, yields) =>
        applyFunc(scanAssignTarget, args.filterNot(e => isPrimitive(e.t)))
          .flatMap(r1 =>
            scanIterE(givenMap.map(_._2)).flatMap(r2 =>
              scanIterE(yields.map(_._1)).map(r3 => r1 ++ r2 ++ r3)
            )
          )
      case PreAssignExpression(target, value) =>
        combine(scanAssignTarget(target), scanForAssignE(value))
      case PointerSubscript(pointer, index) =>
        combine(scanForAssignE(pointer), scanForAssignE(index))
      case ArraySubscript(pointer, index) =>
        combine(scanForAssignE(pointer), scanForAssignE(index))
      case SeqSubscript(seq, index) =>
        combine(scanForAssignE(seq), scanForAssignE(index))
      case _ => None
    }

  def scanAssignTarget(e: Expr[Pre]): Option[Set[Variable[Pre]]] =
    e match {
      case Local(v) => Some(Set(v.decl))
      case ArraySubscript(arr, index) =>
        combine(scanAssignTarget(arr), scanForAssignE(index))
      case PointerSubscript(pointer, index) =>
        combine(scanAssignTarget(pointer), scanForAssignE(index))
      case SeqSubscript(seq, index) =>
        combine(scanAssignTarget(seq), scanForAssignE(index))
      case _ => None
    }

  def isPrimitive(t: Type[_]): Boolean =
    t match {
    ...
    region match {
      ...
      case block @ ParBlock(decl, iters, _, _, _, _) =>
        ...
        Block(iters.foldLeft(Seq[Statement[Post]]()) { case (res, v) =>
          ...
          if (
            nonConstVars.nonEmpty &&
            constantExpression(v.from, nonConstVars.get) &&
            constantExpression(v.to, nonConstVars.get)
          ) {
        ...
        val (vars, init) = variables.collect {
          Block(iters.map { v =>
            ...
            assignLocal(
              ...
              IndeterminateInteger(from(v.variable), to(v.variable)),
            )
          })
        }
        Scope(
