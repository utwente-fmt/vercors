...
case class JavaLibraryLoader(blameProvider: BlameProvider)
  ...
  override def load[G](
    ...
    try {
      val f = RWFile(
        base
          .resolve((name.init :+ name.last + ".java").mkString(File.separator))
          .toFile
      )
      ColJavaParser(ReadableOriginProvider(f), blameProvider).parse[G](f)
        .decls match {
