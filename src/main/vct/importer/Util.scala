...
case object Util extends LazyLogging {
  ...
  def loadPVLLibraryFile[G](
    ...
    if (
      ...
      RWFile(pinnedLibrary, doWatch = false).readToCompletion() != text
    ) {
      val res = parser
        .ColPVLParser(debugOptions, ConstantBlameProvider(LibraryFileBlame))
        .parse(readable)
      val context = Resolution(
    ...
    Using(Files.newInputStream(result)) { in =>
      Deserialize.deserializeProgram[G](
        ...
        o => ConstantBlameProvider(LibraryFileBlame).apply(),
      )
    }.get
  }
  ...
  def loadJavaClass[G](
    ...
    parser.ColJavaParser(debugOptions, ConstantBlameProvider(LibraryFileBlame))
