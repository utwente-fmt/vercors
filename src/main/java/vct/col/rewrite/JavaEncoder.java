    ...
    if (m.getParent() instanceof ASTClass){
      ...
      case Plain:
        if (direct){
          ...
          Method res=create.method_kind(kind, returns, throwy, external_contract, name, args, varArgs, body);
          res.copyMissingFlags(m);
          currentTargetClass.add(res);         
        } else {
          currentTargetClass.add(create.method_kind(kind, returns, throwy, initial_contract, name, args, varArgs, null));
          args=copy_rw.rewrite(args);
          internal_mode=true;
          ASTNode body=rewrite(m.getBody());
          internal_mode=false;
          currentTargetClass.add(create.method_kind(kind, returns, throwy, internal_contract, internal_name, args, varArgs, body));
        }
        break;
      case Predicate:
        if (throwy.length != 0) {
      ...
      default:{
        ASTNode body=rewrite(m.getBody());
        result=create.method_kind(kind, returns, throwy, external_contract, name, args, varArgs, body);
      }}
    } else {
      ASTNode body=rewrite(m.getBody());
      result=create.method_kind(kind, returns, throwy, external_contract, name, args, varArgs, body);
