...
//    );
  ...
  public void visit(TryCatchBlock tcb){
      Abort("TCB should have been handled earlier");
...
//    result=block;
  ...
  public void visit(ReturnStatement s){
      super.visit(s);
//      Abort("Should have been handled earlier");
...
//      if (!Arrays.stream(m.getArgs()).anyMatch(decl -> decl.name().equals(CreateReturnParameter.RETURN_VAR))) {
////        Fail("unexpected invokation of non-void method %s at %s",e.method(),e.getOrigin());
//        Fail("Non-void method does not have return parameter, %s at %s",e.method(),e.getOrigin());
//      }
//    }
...
//    args[0]=create.local_name(IntroExcVar.excVar);
//    for(int i=0;i<N;i++){
