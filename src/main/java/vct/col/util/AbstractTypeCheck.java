...
public class AbstractTypeCheck extends RecursiveVisitor<Type> {
    ...
    public void visit(OperatorExpression e) {
        ...
        switch (op) {
            ...
            case IFF: {
                if (!tt[0].isBoolean())
                    Fail("type of left argument is %s rather than boolean at %s", tt[0], e.getOrigin());
                if (!tt[1].isBoolean())
                    Fail("type of right argument is %s rather than boolean at %s", tt[1], e.getOrigin());
                e.setType(new PrimitiveType(PrimitiveSort.Boolean));
                break;
            }
            case Member: {
            ...
            case MapBuild:
                ...
                if (!tt[0].firstarg().equals(tt[1]))
                    Fail("Type of key %s to add does not match the key type of the map %s at %s", tt[1], tt[0].firstarg(), e.getOrigin());
                if (!tt[0].secondarg().equals(tt[2]))
                    Fail("Type of value %s to add does not match the value type of the map %s at %s", tt[1], tt[0].firstarg(), e.getOrigin());
                e.setType(tt[0]);
                break;
            case MapDisjoint:
            case MapEquality:
