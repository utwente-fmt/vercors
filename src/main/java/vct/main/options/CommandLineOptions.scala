...
object CommandLineOptions {
  ...
  val help_passes = new BooleanSetting(false)
  ...
  val show_before = new StringListSetting
  val show_after = new StringListSetting
  val debugBefore = new CollectSetting
  val debugAfter = new CollectSetting
  val show_file = new StringSetting(null)
  val notifySetting = new BooleanSetting(false)

  val pass_list = new StringListSetting
  val pass_list_option: config.Option = pass_list.getAppendOption("add to the custom list of compilation passes")
  ...
  val check_defined = new BooleanSetting(false)
  val check_axioms = new BooleanSetting(false)
  val check_history = new BooleanSetting(false)
  val separate_checks = new BooleanSetting(false)
  val sequential_spec = new BooleanSetting(false)
  val global_with_field = new BooleanSetting(false)
  val no_context = new BooleanSetting(false)
  val gui_context = new BooleanSetting(false)
  val sat_check = new BooleanSetting(true)
  val abruptTerminationViaExceptions = new BooleanSetting(false)
  val trigger_generation = new IntegerSetting(0)
  ...
  def addOptions(parser: OptionParser): Unit = {
    ...
    parser.add(CommandLineOptions.check_defined.getEnable("Check if the process-algebraic specification itself satisfies its contract."), "check-defined")
    parser.add(CommandLineOptions.check_axioms.getEnable("Check if defined processes satisfy their contracts."), "check-axioms")
    parser.add(CommandLineOptions.check_history.getEnable("Check if the program correctly implements the process-algebraic specification."), "check-history")
    parser.add(CommandLineOptions.separate_checks.getEnable("validate classes separately"), "separate")
    parser.add(CommandLineOptions.help_passes.getEnable("print help on available passes"), "help-passes")
    parser.add(CommandLineOptions.sequential_spec.getEnable("sequential specification instead of concurrent"), "sequential")
    parser.add(CommandLineOptions.pass_list_option, "passes")
    parser.add(CommandLineOptions.show_before.getAppendOption("Show source code before given passes"), "show-before")
    parser.add(CommandLineOptions.show_after.getAppendOption("Show source code after given passes"), "show-after")
    parser.add(CommandLineOptions.show_file.getAssign("redirect show output to files instead of stdout"), "save-show")
    ...
    parser.add(CommandLineOptions.global_with_field.getEnable("Encode global access with a field rather than a parameter. (expert option)"), "global-with-field")
    parser.add(CommandLineOptions.no_context.getEnable("disable printing the context of errors"), "no-context")
    parser.add(CommandLineOptions.gui_context.getEnable("enable the gui extension of the context"), "gui")
    parser.add(CommandLineOptions.sat_check.getDisable("Disable checking if method pre-conditions are satisfiable"), "disable-sat")
    ...
    parser.add(CommandLineOptions.trigger_generation.getOptionalAssign("Try to simplify universal quantifiers and generate triggers for them."), "triggers")
