...
/** Utility class that forwards all calls to an underlying ByteString Idea:
  * either underlying is lazily computed so some convenient methods can be
  * specialized efficiently, or this class just wraps an already existing
  * ByteString.
  */
abstract class ForwardingByteString extends ByteString {
  def underlying: ByteString

  override def byteAt(index: Int): Byte = underlying.byteAt(index)
  ...
  override def copyTo(target: ByteBuffer): Unit = underlying.copyTo(target)

  override def writeTo(out: OutputStream): Unit = underlying.writeTo(out)

  override def writeTo(out: ByteOutput): Unit = underlying.writeTo(out)

  override def writeToInternal(
      out: OutputStream,
      sourceOffset: Int,
      numberToWrite: Int,
  ): Unit = underlying.writeTo(out, sourceOffset, numberToWrite)
  ...
  override def isValidUtf8: Boolean = underlying.isValidUtf8

  override def partialIsValidUtf8(state: Int, offset: Int, length: Int): Int =
    underlying.partialIsValidUtf8(state, offset, length)

  override def newInput(): InputStream = underlying.newInput()

  override def newCodedInput(): CodedInputStream = underlying.newCodedInput()

  override def getTreeDepth: Int = underlying.getTreeDepth

  override def isBalanced: Boolean = underlying.isBalanced
