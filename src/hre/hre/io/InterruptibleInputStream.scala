...
class InterruptibleInputStream(val underlying: InputStream) extends InputStream { outer =>
    ...
    override def run(): Unit =
      while(!eof) {
        if(outer.synchronized(spaceAvailable) >= BLOCK_SIZE) {
          val toRead = outer.synchronized(Math.min(BLOCK_SIZE, BUFFER_SIZE - writeCursor))
          val res = underlying.read(buffer, writeCursor, toRead)

          if(res == -1) {
            eof = true
          } else {
            writeCursor = (writeCursor + res) % BUFFER_SIZE
          }

          outer.synchronized(outer.notifyAll())
        } else {
          outer.synchronized(outer.wait())
        }
      }
  }

  override final def read: Int = synchronized {
    while(true) {
      if(available > 0) {
  ...
  override def read(bufferOut: Array[Byte], off: Int, len: Int): Int = synchronized {
    var written = 0
