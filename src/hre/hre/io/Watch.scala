  ...
  def booleanWithWatch[T](doWatch: Boolean, default: T)(f: => T): T =
    if (doWatch) {
      ...
      val previousHandler = Signal
        .handle(sigint, _ => Watch.currentWatch.get.mainThread.interrupt())
...
class Watch(val mainThread: Thread) extends LazyLogging {
  watch =>
              ...
              breakable {
                while (true) {
                  ...
                  for (ws <- watchServices) {
                    val key = Option(ws.poll(
                      100_000 / watchServices.size,
  ...
  private def stdinWatchThread() =
    new Thread {
      override def run(): Unit =
        try {
          var res = System.in.read()

          while (res != -1 && res != '\n'.toInt) { res = System.in.read() }
  ...
  private def disableDebounceThread() =
    new Thread {
      override def run(): Unit =
        try {
          Thread.sleep(1000)
  ...
  private def reset(): Unit = {
    for (ws <- watchServices.values) { ws.close() }

    watchServices.clear()
    watchedDirs.clear()
    watchedFiles.clear()

    for (inv <- toInvalidate) { inv.invalidate() }
  ...
  def await(): Unit =
    synchronized {
      ...
      logger.info(
        s"[Waiting for $watchCount external inputs to change - press ENTER to run again manually]"
      )
