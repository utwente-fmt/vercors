...
case object ResourceUtil {
  case class NoSuchResource(path: String) extends SystemError {
    override def text: String =
      s"Could not find required resource at path $path"
  }

  def getResource(path: String): Path =
    try {
      Option(getClass.getResource(path)) match {
        case Some(url) => new File(url.toURI).toPath
        case None => throw NoSuchResource(path)
      }
    } catch { case _: URISyntaxException => throw NoSuchResource(path) }

  def getPlatformDep(tail: String): Path =
  ...
  def getPlatformBinary(tail: String): Path =
    Platform.getCurrent match {
