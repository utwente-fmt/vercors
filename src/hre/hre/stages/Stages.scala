...
trait ContextStage[-Input, Ctx, +Output]
    extends Stage[(Input, Ctx), (Output, Ctx)] {
  def run(in: (Input, Ctx)): (Output, Ctx) = (runWithoutContext(in._1), in._2)
...
case class UnitStages[-Input, +Output](stage: Stage[Input, Output])
  ...
  override def runUnsafely(in: Input, progressNext: () => Unit): Output =
    stage.run(in)
  override def flatNames: Seq[(String, Int)] =
    Seq((stage.friendlyName, stage.progressWeight))
}
