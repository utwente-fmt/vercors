...
case class DataRecordTask(superTask: AbstractTask, record: DataRecord)
    extends Task {
  def renderMaybeShort(short: Boolean): ProgressRender =
    record match {
      case r: FunctionRecord =>
        Util.renderOrigin(r.value, "Verifying", short = true)
      case r: PredicateRecord =>
        Util.renderOrigin(r.value, "Verifying", short = true)
      case r: MethodRecord =>
        Util.renderOrigin(r.value, "Verifying", short = true)
      ...
      case r: DeciderAssertRecord =>
        ProgressRender(
          s"Asserting ${r.term.toString.replaceAll("[\n ]+", " ")}"
        )
      case r: ProverAssertRecord =>
        ProgressRender(
          s"Asserting ${r.term.toString.replaceAll("[\n ]+", " ")}"
        )
    }
  ...
  override def profilingBreadcrumb: String =
    record match {
      ...
      case r: ExecuteRecord =>
        s"${CachedExpRender(r.value).replaceAll("[\n ]+", " ")}"
      case r: ConsumeRecord =>
        s"Exhale ${CachedExpRender(r.value).replaceAll("[\n ]+", " ")}"
      case r: ProduceRecord =>
        s"Inhale ${CachedExpRender(r.value).replaceAll("[\n ]+", " ")}"
      case r: CommentRecord => s"/*${r.comment}*/"
      case r: DeciderAssertRecord =>
        s"Assert ${r.term.toString.replaceAll("[\n ]+", " ")}"
      case r: ProverAssertRecord =>
        s"Assert ${r.term.toString.replaceAll("[\n ]+", " ")}"
    }
