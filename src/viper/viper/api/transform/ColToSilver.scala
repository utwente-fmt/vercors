...
case class ColToSilver(program: col.Program[_]) {
  ...
  def pos(node: col.Node[_]): silver.Position = {
    ...
    silver.VirtualPosition(
      s"${node.o.shortPosition.replace(':', '-')};unique_id=$uniquePosId"
    )
  }
  ...
  def transform(): silver.Program = {
    ...
    program.declarations.collect { case adt: col.AxiomaticDataType[_] => adt }
      .flatMap(_.typeArgs).foreach(name)
    program.declarations.collect { case adt: col.AxiomaticDataType[_] => adt }
        ...
        scoped {
          val labelDecls = procedure.body.toSeq
            .flatMap(_.transSubnodes.collect { case l: col.LabelDecl[_] =>
              silver.Label(name(l), Seq())(pos = pos(l), info = NodeInfo(l))
            })
