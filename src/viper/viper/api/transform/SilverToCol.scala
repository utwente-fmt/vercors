...
case object SilverToCol {
  case class SilverPositionOrigin(node: silver.Positioned) extends Origin {
    ...
    override def shortPosition: String =
      node.pos match {
        case pos: AbstractSourcePosition =>
          s"${pos.start.line}:${pos.start.column}"
        case _ => "unknown"
      }
    ...
    override def inlineContext: String =
      InputOrigin.compressInlineText(node.toString)
  }
...
case class SilverToCol[G](
  ...
  def origin(node: silver.Positioned, sourceName: String = ""): Origin =
    if (sourceName.nonEmpty)
      SourceNameOrigin(sourceName, SilverPositionOrigin(node))
    else
      node match {
        case node: silver.Declaration =>
          SourceNameOrigin(node.name, SilverPositionOrigin(node))
  ...
  def transform(e: silver.Exp): col.Expr[G] = {
    ...
    e match {
      ...
      case silver.FractionalPerm(left, right) =>
        col.Div(f(left), f(right))(blame(e))
