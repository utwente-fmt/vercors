...
class Deserialize extends NodeGenerator {
  override def generate(out: Path, node: NodeDefinition): Unit =
    ResultStream.write(out.resolve("Deserialize" + node.name.base + ".scala"), deserialize(node))
  ...
  def deserializeTerm(term: Term, protoType: Proto.Type, structuralType: ST): Term =
    (protoType, structuralType) match {
      case (Proto.Required(pt), st) =>
        deserializeTerm(term, pt, st)
  ...
  def deserializeTerm(term: Term, protoType: Proto.PrimitiveType, structuralType: ST): Term =
    (protoType, structuralType) match {
      case (_, ST.Seq(ST.ExpectedError)) =>
        q"$SeqObj.empty"
      ...
      case (Proto.AuxType(_), ST.Tuple(args)) =>
        q"(..${
          args.zipWithIndex.map {
            case (structuralType, index) =>
              deserializeTerm(
                q"$term.${Term.Name(s"v${index + 1}")}",
                ProtoNaming.getType(structuralType).t,
                structuralType
              )
          }.toList
        })"
      ...
      case (Proto.StandardType(_), ST.ExpectedError) =>
        q"???"
      case (Proto.Bool, ST.Boolean) =>
        term
      case (Proto.Int, ST.Byte) =>
        q"$term.toByte"
      case (Proto.Int, ST.Short) =>
        q"$term.toShort"
      case (Proto.Int, ST.Int) =>
        term
      case (Proto.Long, ST.Long) =>
        term
      case (Proto.Float, ST.Float) =>
        term
      case (Proto.Double, ST.Double) =>
        term
      case (Proto.Int, ST.Char) =>
        q"$term.toChar"
      case (pt, st) =>
        err(pt, st)
    }
