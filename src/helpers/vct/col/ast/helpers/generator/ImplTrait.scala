...
class ImplTrait extends ImplTraitGenerator {
  ...
  val selectors =
    s"(?:$selectorRule|$acc_open$selectorRule(?:$comma$selectorRule)*$acc_close)"
  ...
  val traitRule =
    s"(?:$kwd_trait$id$brack_open$kwd_gen$brack_close(($kwd_extends$traitParents)?$acc_open))"
  ...
  def fix(p: Path, opsNames: Seq[String]): Unit = {
    ...
    val exts: Seq[String] = Option(traitMatch.group(2)).map(
      _.strip().split("\\s+").to(List).map(_.strip()).filterNot(_ == "extends")
        .filterNot(_ == "with")
    ).getOrElse(Nil)
    ...
    allImportLines.foreach { line =>
      importExpr.findAllIn(line.group(1)).foreach { imp =>
        ...
        if (
          pkg == Seq("vct", "col", "ast", "ops") &&
          (select.contains("_") ||
            opsNames.forall(name => select.contains(name)))
        ) { mkImport = false }
      }
    }
  ...
  def generate(
    ...
    Using(Files.newBufferedWriter(p)) { writer =>
      val ext =
        if (concrete && family)
          s"${node}Ops[G] with ${node}FamilyOps[G]"
        else if (concrete)
          s"${node}Ops[G]"
        else if (family)
          s"${node}FamilyOps[G]"
      ...
      writer.write(s"""|package vct.col.ast.unsorted
            |
