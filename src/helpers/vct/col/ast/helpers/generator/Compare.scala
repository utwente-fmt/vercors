...
import vct.col.ast.helpers.defn.Constants.{
  ...
  LazyList,
  LazyListObj,
  MatchingDeclarationObj,
  MatchingReferenceObj,
  Node,
  StructuralDifferenceObj,
}
...
class Compare extends NodeGenerator {
  override def generate(out: Path, node: NodeDefinition): Unit =
    ResultStream
      .write(out.resolve(s"${node.name.base}Compare.scala"), getTree(node))
  ...
  def compare(left: Term, right: Term, node: NodeDefinition): Term = {
    ...
    simplify(q"""
        if($valuesEqual) ${if (node.kind == DeclaredNode)
        q"$MatchingDeclarationObj($left, $right) #:: $refs #::: $subnodes"
      else
        q"$refs #::: $subnodes"} else $LazyListObj($StructuralDifferenceObj($left, $right))
      """)
  }

  def compareValues(left: Term, right: Term, node: NodeDefinition): Term =
    node.fields.map { case (fieldName, t) =>
  ...
  def compareValues(left: Term, right: Term, t: structure.Type): Term =
    t match {
      case structure.Type.Node(_) | structure.Type.Declaration(_) => q"true"
      ...
      case structure.Type.Option(t) =>
        q"$left.isEmpty == $right.isEmpty && ($left.isEmpty || ${compareValues(q"$left.get", q"$right.get", t)})"
      ...
      case structure.Type.Seq(t) =>
        q"$left.size == $right.size && $left.zip($right).forall { case (l0, r0) => ${compareValues(q"l0", q"r0", t)} }"
      case structure.Type.DeclarationSeq(_) => q"$left.size == $right.size"
    }

  def compareRefs(left: Term, right: Term, node: NodeDefinition): Term =
    node.fields.foldRight[Term](q"$LazyListObj()") { case ((field, t), acc) =>
      val fieldValues = compareRefs(
        q"$left.${Term.Name(field)}",
  ...
  def compareRefs(left: Term, right: Term, t: structure.Type): Term =
    t match {
      case structure.Type.Node(_) | structure.Type.Declaration(_) | structure
            .Type.DeclarationSeq(_) =>
        q"$LazyListObj()"
      case structure.Type.Ref(_) | structure.Type.MultiRef(_) =>
        q"$LazyListObj($MatchingReferenceObj($left.decl, $right.decl))"
      case _: structure.Type.PrimitiveType => q"$LazyListObj()"
      case structure.Type.Option(t) =>
        q"if($left.isDefined) ${compareRefs(q"$left.get", q"$right.get", t)} else $LazyListObj()"
      case structure.Type.Either(l, r) =>
      ...
      case structure.Type.Seq(t) =>
        q"$left.zip($right).to($LazyListObj).flatMap { case (l0, r0) => ${compareRefs(q"l0", q"r0", t)} }"
    }

  def compareSubnodes(left: Term, right: Term, node: NodeDefinition): Term =
    node.fields.foldRight[Term](q"$LazyListObj()") { case ((field, t), acc) =>
      val fieldValues = compareSubnodes(
        q"$left.${Term.Name(field)}",
        q"$right.${Term.Name(field)}",
        t,
      )
  ...
  def compareSubnodes(left: Term, right: Term, t: structure.Type): Term =
    t match {
      case structure.Type.Node(_) | structure.Type.Declaration(_) =>
        q"$left.compare($right)"
      case structure.Type.Ref(_) | structure.Type.MultiRef(_) =>
        q"$LazyListObj()"
      case _: structure.Type.PrimitiveType => q"$LazyListObj()"
      case structure.Type.Option(t) =>
        q"if($left.isDefined) ${compareSubnodes(q"$left.get", q"$right.get", t)} else $LazyListObj()"
      case structure.Type.Either(l, r) =>
      ...
      case structure.Type.Seq(t) =>
        q"$left.zip($right).to($LazyListObj).flatMap { case (l0, r0) => ${compareSubnodes(q"l0", q"r0", t)} }"
      case structure.Type.DeclarationSeq(t) =>
        q"$left.zip($right).to($LazyListObj).flatMap { case (l0, r0) => l0.compare(r0) }"
    }
}
