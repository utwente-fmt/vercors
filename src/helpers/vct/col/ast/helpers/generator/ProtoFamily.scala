...
class ProtoFamily extends FamilyGenerator {
  def message(family: Name, nodes: Seq[Name]): Option[Proto.Message] = {
    ...
    Some(Proto.Message(
      ...
      Proto.MessageOneOf(
        oneOfName = "v",
        fields = fqNames.zipWithIndex.map { case (name, idx) =>
          Proto.Field(
  ...
  override def generate(
    ...
    message(family, nodes).foreach { message =>
      ...
      Using(
        Files.newBufferedWriter(dir.resolve(message.name.last + ".proto"))
      ) { writer =>
        Proto.Source(
          imports = Seq("scalapb", "scalapb") +: message.imports,
          options = Proto.renderOptions(Proto.STANDARD_OPTIONS.updated(
            "package_name",
            ProtoNaming.scalaPackageOption(message.name),
          )),
          message = message,
        ).write(writer)
      }
