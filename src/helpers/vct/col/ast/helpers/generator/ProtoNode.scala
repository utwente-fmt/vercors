...
class ProtoNode extends NodeGenerator {
  def message(node: NodeDefinition): Proto.Message = {
    ...
    val blame =
      "blame" -> Proto.Required(Proto.StandardType(Proto.auxBase :+ "Blame"))
    val origin =
      "origin" -> Proto.Required(Proto.StandardType(Proto.auxBase :+ "Origin"))
    ...
    val allFields = allFieldDecls.zipWithIndex.map { case (name -> t, idx) =>
      Proto.Field(name, idx + 1, t)
    }
  ...
  override def generate(out: Path, node: NodeDefinition): Unit = {
    ...
    val source = Proto.Source(
      ...
      Proto.renderOptions(
        Proto.OPAQUE_SUBMESSAGES_OPTIONS
          .updated("package_name", ProtoNaming.scalaPackageOption(message.name))
      ),
      message,
    )
    ...
    Using(Files.newBufferedWriter(
      dir.resolve(message.name.last + ".proto"),
      StandardCharsets.UTF_8,
    )) { writer => source.write(writer) }
  }
}
