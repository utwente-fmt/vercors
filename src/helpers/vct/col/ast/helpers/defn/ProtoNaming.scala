...
object ProtoNaming {
  ...
  def explodeCamel(name: String, acc: Seq[String] = Nil): Seq[String] = {
    ...
    if (leftUpper.length <= 1) {
      ...
      explodeCamel(
        s"${leftUpper.last}${rem0}",
        acc :+ leftUpper.init.toLowerCase,
      )
    }
  }
  ...
  def snake(name: String): String = parts(name).mkString("_")
  ...
  def ucamel(name: String): String = camel(name).capitalize
  ...
  def typeText(t: structure.Type): String =
    t match {
      case structure.Type.Node(name) =>
        name.tailName.parts.flatMap(parts).map(_.capitalize).mkString("")
      case structure.Type.Declaration(name) =>
        typeText(structure.Type.Node(name))
      case structure.Type.DeclarationSeq(name) =>
        typeText(structure.Type.Seq(structure.Type.Node(name)))
      case structure.Type.Ref(node) => s"Ref_${typeText(node)}"
      case structure.Type.MultiRef(node) => s"Ref_${typeText(node)}"
      case structure.Type.Tuple(args) =>
        s"Tuple${args.size}_${args.map(typeText).mkString("_")}"
      ...
      case structure.Type.Either(left, right) =>
        s"Either_${typeText(left)}_${typeText(right)}"
      case other: structure.Type.PrimitiveType =>
        other.getClass.getSimpleName.replace("$", "")
    }

  private val _getTupleAux = mutable
    .Map[Seq[structure.Type], PrimitiveTypeResult]()

  def getTupleAux(ts: Seq[structure.Type]): PrimitiveTypeResult =
    _getTupleAux.getOrElseUpdate(
      ts, {
        val fieldTypeResults = ts.map(getType)
        val auxs = fieldTypeResults.flatMap(_.auxs)
        val fieldTypes = fieldTypeResults.map(_.t)
        val fields = MessageFields(fieldTypes.zipWithIndex.map { case (t, i) =>
          Field(s"v${i + 1}", i + 1, t)
        })
        val message = Message(
          auxBase :+ typeText(structure.Type.Tuple(ts)),
          fields,
        )
  ...
  def getOptionAux(t: structure.Type): PrimitiveTypeResult =
    _getOptionAux.getOrElseUpdate(
      t, {
        val typeResult = getPrimitiveType(t)
        val field = Field("value", 1, Option(typeResult.t))
        val message = Message(
          auxBase :+ ("Option_" + typeText(t)),
          MessageFields(Seq(field)),
        )
  ...
  def getSeqAux(t: structure.Type): PrimitiveTypeResult =
    _getSeqAux.getOrElseUpdate(
      t, {
        val typeResult = getPrimitiveType(t)
        val field = Field("value", 1, Repeated(typeResult.t))
        val message = Message(
          auxBase :+ ("Seq_" + typeText(t)),
          MessageFields(Seq(field)),
        )
        PrimitiveTypeResult(AuxType(message.name), typeResult.auxs :+ message)
      },
  ...
  private val _getEitherAux = mutable
    .Map[(structure.Type, structure.Type), PrimitiveTypeResult]()

  def getEitherAux(
      left: structure.Type,
      right: structure.Type,
  ): PrimitiveTypeResult =
    _getEitherAux.getOrElseUpdate(
      (left, right), {
        ...
        val message = Message(
          auxBase :+ s"Either_${typeText(left)}_${typeText(right)}",
          fields,
        )
        PrimitiveTypeResult(
  ...
  private val _getPrimitiveType = mutable
    .Map[structure.Type, PrimitiveTypeResult]()

  def getPrimitiveType(t: structure.Type): PrimitiveTypeResult =
    _getPrimitiveType.getOrElseUpdate(
      t, {
        t match {
          case structure.Type.Node(name) =>
          ...
          case structure.Type.Declaration(name) =>
            PrimitiveTypeResult(FamilyType(getTypeName(name)))
          case structure.Type.Ref(_) | structure.Type.MultiRef(_) =>
            getStandardType("Ref")
          ...
          case structure.Type.DeclarationSeq(name) =>
            getSeqAux(structure.Type.Declaration(name))
  ...
  def getType(t: structure.Type): TypeResult =
    _getType.getOrElseUpdate(
      t,
      t match {
        case structure.Type.Seq(t) => getPrimitiveType(t).result(Repeated)
        case structure.Type.DeclarationSeq(name) =>
          getPrimitiveType(structure.Type.Declaration(name)).result(Repeated)
