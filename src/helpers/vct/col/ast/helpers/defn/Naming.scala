...
object Naming {
  ...
  def typ(name: structure.Name): Type =
    if (name.parts.size == 1)
      ...
      Type.Select(term(name.initName), Type.Name(name.base))

  def typ(node: structure.NodeDefinition): Type = typ(node.name)

  def typ(t: structure.Type, g: Type): Type =
    t match {
      ...
      case structure.Type.DeclarationSeq(name) =>
        t"_root_.scala.Seq[${typ(name)}[$g]]"
      case structure.Type.Ref(node) => t"$RefType[$g, ${typ(node.name)}[$g]]"
      case structure.Type.MultiRef(node) =>
        t"$RefType[$g, ${typ(node.name)}[$g]]"
      ...
      case structure.Type.Either(left, right) =>
        t"_root_.scala.util.Either[${typ(left, g)}, ${typ(right, g)}]"
      ...
      case structure.Type.ExpectedError =>
        t"_root_.vct.col.origin.ExpectedError"
  ...
  def packageFromRoot(t: Term): Term.Ref =
    t match {
  ...
  def scalapbType(node: structure.Name): Type = typ(scalapbName(node))
  ...
  def compareType(node: NodeDefinition) =
    t"${packageFromRoot(ComparePackage)}.${compareTrait(node)}"
  def rewriteTrait(node: NodeDefinition) = Type.Name(node.name.base + "Rewrite")
  def rewriteType(node: NodeDefinition) =
    t"${packageFromRoot(RewritePackage)}.${rewriteTrait(node)}"
  def subnodesTrait(node: NodeDefinition) =
    Type.Name(node.name.base + "Subnodes")
  def subnodesType(node: NodeDefinition) =
    t"${packageFromRoot(SubnodesPackage)}.${subnodesTrait(node)}"
  def serializeTrait(node: NodeDefinition) =
    Type.Name(node.name.base + "Serialize")
  def serializeType(node: NodeDefinition) =
    t"${packageFromRoot(SerializePackage)}.${serializeTrait(node)}"
  def deserializeName(node: structure.Name) = "Deserialize" + node.base
  def deserializeFamilyName(node: structure.Name) =
    "DeserializeFamily" + node.base
  def deserializeType(node: NodeDefinition) =
    Type.Name(deserializeName(node.name))
  def deserializeObjectName(node: NodeDefinition) =
    Term.Name(deserializeName(node.name))
  def deserializeObject(node: structure.Name) =
    q"${packageFromRoot(DeserializePackage)}.${Term.Name(deserializeName(node))}"
  def deserializeFamilyObject(name: structure.Name) =
    q"${packageFromRoot(DeserializePackage)}.${Term.Name(deserializeFamilyName(name))}"
  def declareTrait(node: structure.Name) =
