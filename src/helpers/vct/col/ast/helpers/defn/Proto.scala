...
object Proto {
  ...
  val OPAQUE_SUBMESSAGES_OPTIONS: Map[String, String] =
    STANDARD_OPTIONS ++
      ListMap("bytes_type" -> "\"_root_.vct.serialize.OpaqueMessageBytes\"")
  ...
  sealed trait NamedType extends PrimitiveType {
    def fqName: Seq[String]
  }
  ...
  sealed trait Type {
    ...
    def writeOption(out: Appendable): Unit =
      this match {
    ...
    def map(f: PrimitiveType => PrimitiveType): Type =
      this match {
  ...
  case class Field(name: String, index: Int, t: Type) {
    ...
    def opaqueNodes: Field = copy(t = t.map(_.opaqueNodes))
  }
  ...
  sealed trait MessageBody {
    ...
    def namedTypes: Seq[NamedType] =
      fields.collect { case Field(_, _, Type(name: NamedType)) => name }
    ...
    def write(out: Appendable): Unit =
      this match {
        case MessageFields(_) => writeFields(out, "  ")
  ...
  case class MessageOneOf(oneOfName: String, fields: Seq[Field])
      extends MessageBody {
  ...
  case class Message(name: Seq[String], body: MessageBody) {
    ...
    def opaqueNodes: Message = copy(body = body.opaqueNodes)
  ...
  object Source {
    def apply(
        ...
        options: String,
        message: Message,
    ): Source = Source(imports, options, Seq(message))
  }
  ...
  case class Source(
    ...
    def write(out: Appendable): Unit = {
      ...
      if (messages.exists(_.name.size > 1)) {
        val pkg =
          messages.collectFirst {
            case msg if msg.name.size > 1 => msg.name.init
          }.get

        val wrongPackageMessages = messages
          .filter(msg => msg.name.size != pkg.size + 1 || msg.name.init != pkg)
        require(
          wrongPackageMessages.isEmpty,
          out.append(
            s"Messages rendered jointly in one source must have the same package. Wrong: ${wrongPackageMessages
                .map(_.name.mkString(".")).mkString("{", ", ", "}")}"
          ),
        )
      ...
      if (imports.nonEmpty) {
        for (imp <- imports) {
          out.append("import \"").append(imp.mkString("/"))
            .append(".proto\";\n")
        }
        out.append("\n")
      }
    ...
    def opaqueNodes: Source = copy(messages = messages.map(_.opaqueNodes))
  }
}
