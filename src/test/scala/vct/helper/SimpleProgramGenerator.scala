...
object SimpleProgramGenerator {
  ...
  def generateSimpleApplicableContract[G]()(implicit origin: Origin = generateSimpleInputOrigin()): ApplicableContract[G] ={
    ApplicableContract(UnitAccountedPredicate(BooleanValue(value = true)), UnitAccountedPredicate(BooleanValue(value = true)), BooleanValue(value = true),Seq(),Seq(),Seq())(TrueSatisfiable)
  }
