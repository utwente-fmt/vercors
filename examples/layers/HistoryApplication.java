...
class Sender extends Thread {
  ...
  /*@
    ...
    resource postJoin(frac p)=p==write
      ...
      ** (\forall* int i; 0 <= i && i < |vals| ; Perm(input[i],1))
      ** (\forall  int i; 0 <= i && i < |vals| ; input[i]==vals[i])
      ** Hist(queue.hist,1f/2,queue.hist.put_all(vals))
      ** true;
  @*/
  ...
  /*@
    ...
    requires input!=null ** input.length==|vals| ** queue!=null
      ** Value(queue.hist) ** Hist(queue.hist,1f/2,empty)
      ** PointsTo(queue.hist_active,1f/4,true)
      ** (\forall* int i; 0 <= i && i < |vals| ; Perm(input[i],1))
      ** (\forall  int i; 0 <= i && i < |vals| ; input[i]==vals[i]);
  ...
  public void run(){
    ...
    //@ assert Hist(queue.hist,1f/2,queue.hist.put_all(take(vals,i)));
    ...
    /*@
      ...
      ** Hist(queue.hist,1f/2,queue.hist.put_all(take(vals,i)));
    @*/
    while(i<N){
      queue.put(input[i])
      /*@ with { p=1f/2 ; P = queue.hist.put_all(take(vals,i));} @*/;
      //@ assert Hist(queue.hist,1f/2,
      //@    queue.hist.put_all(take(vals,i)+seq<int>{vals[i]}));
      //@ assert take(vals,i)+seq<int>{vals[i]}==take(vals,i+1);
      i=i+1;
      //@ assert Hist(queue.hist,1f/2,queue.hist.put_all(take(vals,i)));
    }
    //@ assert Hist(queue.hist,1f/2,queue.hist.put_all(take(vals,N)));
...
class Receiver extends Thread {
  ...
  /*@
    resource preFork(frac p)=p==write
      ** Value(this.queue) ** queue!=null ** Value(queue.hist)
      ** Value(this.output) ** PointsTo(queue.hist_active,1f/4,true)
    ...
    resource postJoin(frac p)=p==write
      ...
      ** Hist(queue.hist,1f/2,queue.hist.get_all(vals))
      ** true;
  @*/
  
  /*@
    requires queue!=null ** Value(queue.hist)
      ** Hist(queue.hist,1f/2,empty) ** output != null
      ** PointsTo(queue.hist_active,1f/4,true)
    ...
    ** Hist(queue.hist,1f/2,queue.hist.get_all(vals)); @*/
    while(i<N){
      output[i]=queue.get()/*@ with {
          p=1f/2 ; P = queue.hist.get_all(vals);} @*/;
...
/*@
  ...
  ensures Value(hist)  ** PointsTo(hist_active,1f/2,false)** HPerm(hist.q,1);
  void end_history();
@*/
...
public class Main {
  ...
  public static void main(int[] input,int[] output){
    ...
    /*@ with { hist=hist; } @*/;
    //@ split q.hist, 1f/2, empty, 1f/2, empty;
    ...
    //@ assert Hist(hist,1f/2,hist.put_all(s.vals));
    //@ assert Hist(hist,1f/2,hist.get_all(r.vals));
    //@ merge hist, 1f/2, hist.put_all(s.vals), 1f/2, hist.get_all(r.vals);
    //@ q.end_history();
