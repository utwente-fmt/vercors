...
/*@
  context \pointer(a, len, write) ** \pointer(b, len, 1f/2) ** \pointer(c, len, 1f/2);
...
void vector_add(int a[],int b[],int c[],int len){
  ...
  /*@
    ...
    context Perm(a[i],write) ** Perm(b[i],1f/2) ** Perm(c[i],1f/2);
...
/*@
  context \pointer(a, N, write) ** \pointer(b, N, 1f/2) ** \pointer(c, N, write);
@*/
void indep_drf(int a[],int b[],int c[],int N){
  for(int i=0;i<N;i++) /*@
    context a != NULL && b != NULL && c != NULL;
    context Perm(a[i],write) ** Perm(c[i],write) ** Perm(b[i],1f/2);
  @*/ {
    a[i] = b[i] + 1;
    c[i] = a[i] + 2;
  }
...
void forward_drf(int a[],int b[],int c[],int N){
  for(int i=0;i < N;i++) /*@
    context a != NULL && b != NULL && c != NULL;
    requires Perm(a[i],write) ** Perm(b[i],1f/2) ** Perm(c[i],write);
    ensures  Perm(a[i],1f/2) ** Perm(b[i],1f/2) ** Perm(c[i],write);
    ensures  (i>0 ==> Perm(a[i-1],1f/2)) ** (i==N-1 ==> Perm(a[i],1f/2));
  @*/ {
    a[i]=b[i]+1;
    /*@
      S1:if (i< N-1) {
        send a != NULL ** 0 <= i ** i < N - 1 ** Perm(a[i],1f/2) to S2,1;
      }
    @*/
    S2:if (i>0) {
      //@ recv a != NULL ** 0 < i ** i < N ** Perm(a[i-1],1f/2) from S1,1;
      c[i]=a[i-1]+2;
    }
  }
}
...
/*@
  context \pointer(a, len, write) ** \pointer(b, len, 1f/2) ** \pointer(c, len, write);
...
void forward_full(int a[],int b[],int c[],int len){
  for(int i=0;i < len;i++) /*@
    ...
    ensures  Perm(a[i],1f/2) ** Perm(b[i],1f/2) ** Perm(c[i],write);
    ensures  i>0 ==> Perm(a[i-1],1f/2);
    ensures  i==len-1 ==> Perm(a[i],1f/2);
    ensures  a[i]==i+1 && b[i]==i && (i>0 ==> c[i]==i+2);
  @*/ {
    a[i]=b[i]+1;
    /*@
      FS1:if (i< len-1) {
        send a != NULL ** 0 <= i ** i < len - 1 ** Perm(a[i],1f/2) ** a[i]==i+1 to FS2,1;
      }
    @*/
    FS2:if (i>0) {
      //@ recv a != NULL ** 0 < i ** i < len ** Perm(a[i-1],1f/2) ** a[i-1]==i from FS1,1;
      c[i]=a[i-1]+2;
    }
  }
}
...
void backward_drf(int a[],int b[],int c[],int N){
  for(int i=0;i < N;i++)
   /*@
    context a != NULL && b != NULL && c != NULL;
    requires Perm(a[i],1f/2) ** Perm(b[i],1f/2) ** Perm(c[i],write);
    requires (i==0 ==> Perm(a[i],1f/2)) ** (i < N-1 ==> Perm(a[i+1],1f/2));
    ensures  Perm(a[i],1f/2) ** Perm(a[i],1f/2) ** Perm(b[i],1f/2) ** Perm(c[i],write);
   @*/
    {
    /*@
      T1:if (i>0) {
        recv a != NULL ** 0 < i ** i < N ** Perm(a[i],1f/2) from T2,1;
      }
    @*/
    a[i]=b[i]+1;
    T2:if (i < N-1) {
      c[i]=a[i+1]+2;
      //@ send a != NULL ** 0 <= i ** i < N - 1 ** Perm(a[i+1],1f/2) to T1,1;
    }
  }
}
...
void backward_full(int a[],int b[],int c[],int len){
  for(int i=0;i < len;i++)
   /*@
    ...
    requires Perm(a[i], 1f/2);
    requires i==0 ==> Perm(a[i], 1f/2);
    requires i < len-1 ==> Perm(a[i+1], 1f/2);
    context  Perm(b[i], 1f/2);
   ...
   @*/
    ...
    /*@
      FT1:if (i>0) {
        recv a != NULL ** 0 < i ** i < len ** i == (i-1)+1 ** Perm(a[i], 1f/2) from FT2,1;
      }
    @*/
    a[i]=b[i]+1;
    FT2:if (i < len-1) {
      c[i]=a[i+1]+2;
      //@ send a != NULL ** 0 <= i ** i < len - 1 ** Perm(a[i+1], 1f/2) to FT1,1;
    }
  }
}
