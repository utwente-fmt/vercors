...
requires 0 <= startIdx && startIdx < endIdx && endIdx <= _ptrLength(arr);
...
/*@
requires arr != NULL;
requires _inRange(0, idx1, _ptrLength(arr));
requires _inRange(0, idx2, _ptrLength(arr));
requires idx1 != idx2;
requires _sep(_Perm(&arr[idx1], _write),
              _Perm(&arr[idx2], _write));
ensures  _sep(_Perm(&arr[idx1], _write),
              _Perm(&arr[idx2], _write));
ensures arr[idx1] == _old(arr[idx2]);
ensures arr[idx2] == _old(arr[idx1]);
@*/
