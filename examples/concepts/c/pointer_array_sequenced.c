...
int *localInLoop(int n, int m) {
    ...
    // Assert only needed for sequence encoding
    //@ assert (\forall* int i_j = 0 .. n * m; &a[\euclidean_div(i_j, n)][\euclidean_mod(i_j, n)] == {:a + i_j:} ** Perm(a[\euclidean_div(i_j, n)][\euclidean_mod(i_j, n)], write));
    return a;
}
