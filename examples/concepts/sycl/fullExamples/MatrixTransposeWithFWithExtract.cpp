...
/*@
	...
	ensures (\forall int r, int c;
		...
		result[sycl::linearize2(c, r, cols, rows)] ==
		f(matrix[sycl::linearize2(r, c, rows, cols)])
	);
*/
			...
			[&](sycl::handler& cgh) {
				...
				cgh.parallel_for(sycl::nd_range(sycl::range(rows*cols), sycl::range(cols)),
					...
					/*@
                        ...
                        context 0 <= it.get_local_linear_id();
                        context temp_acc.get_range().get(0) == it.get_global_range(0);
