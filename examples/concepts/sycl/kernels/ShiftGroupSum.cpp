...
/*@
    ...
    context_everywhere (\forall int lid1=0 .. N, int gid1=0 .. T/N; ({:1:sycl::linearize2(gid1, lid1, T/N, N):}%4 == 0 ==>                      (lid1%sycl::h::warp_sizes()+sycl::h::warp_sizes() <= sycl::h::warp_sizes() ==>sycl::linearize2(gid1, lid1, T/N, N)+sycl::h::warp_sizes() <= |fxGs| ==>fx[sycl::linearize2(gid1, lid1, T/N, N)] == sum(fxGs[sycl::linearize2(gid1, lid1, T/N, N)   .. sycl::linearize2(gid1, lid1, T/N, N)+sycl::h::warp_sizes()]))));
    context_everywhere (\forall int lid1=0 .. N, int gid1=0 .. T/N; ({:2:sycl::linearize2(gid1, lid1, T/N, N):}%4 == 1 ==> (0 <= lid1%sycl::h::warp_sizes() - 1 ==> lid1%sycl::h::warp_sizes()+sycl::h::warp_sizes()-1 <= sycl::h::warp_sizes() ==> sycl::linearize2(gid1, lid1, T/N, N)+sycl::h::warp_sizes()-1 <= |fyGs| ==>                             fx[sycl::linearize2(gid1, lid1, T/N, N)] == sum(fyGs[sycl::linearize2(gid1, lid1, T/N, N)-1 .. sycl::linearize2(gid1, lid1, T/N, N)+sycl::h::warp_sizes()-1]))));
    context_everywhere (\forall int lid1=0 .. N, int gid1=0 .. T/N; ({:3:sycl::linearize2(gid1, lid1, T/N, N):}%4 == 2 ==> (0 <= lid1%sycl::h::warp_sizes() - 2 ==> lid1%sycl::h::warp_sizes()+sycl::h::warp_sizes()-2 <= sycl::h::warp_sizes() ==> sycl::linearize2(gid1, lid1, T/N, N)+sycl::h::warp_sizes()-2 <= |fzGs| ==> fx[sycl::linearize2(gid1, lid1, T/N, N)] == sum(fzGs[sycl::linearize2(gid1, lid1, T/N, N)-2 .. sycl::linearize2(gid1, lid1, T/N, N)+sycl::h::warp_sizes()-2]))));
    ...
    for (gid=0; gid < T/N; gid++){
        ...
        /*@ loop_invariant 0 <= gid && gid < T/N;
            ...
            loop_invariant (lid == N ) ==> (resultx == sum(fxGs[0 .. sycl::linearize2(gid, N-sycl::h::warp_sizes(), T/N, N)+sycl::h::warp_sizes()]) &&
                                            resulty == sum(fyGs[0 .. sycl::linearize2(gid, N-sycl::h::warp_sizes(), T/N, N)+sycl::h::warp_sizes()]) &&
                                            resultz == sum(fzGs[0 .. sycl::linearize2(gid, N-sycl::h::warp_sizes(), T/N, N)+sycl::h::warp_sizes()])); */
...
ensures N == sycl::h::warp_sizes() ==> \result == 5;
