    ...
    //@ label bK;
    {
        ...
        sycl::event e0 = q.submit([&](sycl::handler& h)
        {
            ...
            /*@ context Perm(fxAcc[it.get_global_id(0)], write);
                ...
                ensures (0 <= it.get_sub_group().get_local_id()[0] - 1 && it.get_global_id(0) + 1 < |fxGs|) ==>
            ...
            [=](sycl::nd_item<1> it) {
                ...
                int d1 = 1;
                fxAcc[gid] = sycl::shift_group_right(it.get_sub_group(), fxAcc[gid], d1) /*@ sub_group_inv { \gtid < |fxGs| ==> \sg_val == fxGs[\gtid] } */;
                //@ assert (0 <= laneId - d1 && gid + d1 < |fxGs|) ==> fxAcc[gid] == fxGs[gid - d1];
            });});
        e0.wait();
    }
}
